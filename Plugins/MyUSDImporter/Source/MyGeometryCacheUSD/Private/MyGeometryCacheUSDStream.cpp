...
{
	// Do this before calling the ReadFunc as FMyUsdStreamDDCUtils::GetUsdStreamDDCKey also needs a valid stage to work with
	if (!bReadyForRead)
	{
...
void FGeometryCacheUsdStream::GetMeshData(int32 FrameIndex, int32 ConcurrencyIndex, FGeometryCacheMeshData& OutMeshData)
{
	...
	if (GUsdStreamCacheInDDC)
	{
		...
		const FString DerivedDataKey = FMyUsdStreamDDCUtils::GetUsdStreamDDCKey(
