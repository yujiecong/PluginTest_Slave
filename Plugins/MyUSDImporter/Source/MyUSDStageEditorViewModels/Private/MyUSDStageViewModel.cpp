...
void FMyUsdStageViewModel::NewStage()
{
...
void FMyUsdStageViewModel::OpenStage(const TCHAR* FilePath)
{
	FScopedUsdMessageLog ScopedLog;
	...
	{
		USD_LOG_ERROR(TEXT("Failed to find a AMyUsdStageActor that could open stage '%s'!"), FilePath);
	}
}

void FMyUsdStageViewModel::SetLoadAllRule()
{
#if USE_USD_SDK
	if (!UsdStageActor.IsValid())
	{
	...
	if (UE::FMyUsdStage Stage = UsdStageActor->GetOrOpenUsdStage())
	{
		...
		UsdStage->SetLoadRules(pxr::UsdStageLoadRules::LoadAll());
	}
#endif	  // USE_USD_SDK
}

void FMyUsdStageViewModel::SetLoadNoneRule()
{
#if USE_USD_SDK
	if (!UsdStageActor.IsValid())
	{
	...
	if (UE::FMyUsdStage Stage = UsdStageActor->GetOrOpenUsdStage())
	{
		...
		pxr::UsdStageRefPtr UsdStage = pxr::UsdStageRefPtr(Stage);
		UsdStage->SetLoadRules(pxr::UsdStageLoadRules::LoadNone());
	}
#endif	  // USE_USD_SDK
}

bool FMyUsdStageViewModel::HasDefaultLoadRule()
{
#if USE_USD_SDK
	if (!UsdStageActor.IsValid())
	{
	...
	if (UE::FMyUsdStage Stage = UsdStageActor->GetOrOpenUsdStage())
	{
		...
		pxr::UsdStageLoadRules Rules = UsdStage->GetLoadRules();
		Rules.Minimize();

		return Rules == pxr::UsdStageLoadRules::LoadNone() || Rules == pxr::UsdStageLoadRules::LoadAll();
	}
#endif	  // USE_USD_SDK
...
void FMyUsdStageViewModel::ReloadStage()
{
	if (!UsdStageActor.IsValid())
	{
		return;
...
void FMyUsdStageViewModel::ResetStage()
{
#if USE_USD_SDK
	if (!UsdStageActor.IsValid())
	{
...
void FMyUsdStageViewModel::CloseStage()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMyUsdStageViewModel::CloseStage);
	if (AMyUsdStageActor* StageActor = UsdStageActor.Get())
	{
		StageActor->Reset();
	}
...
void FMyUsdStageViewModel::SaveStage()
{
#if USE_USD_SDK
	if (UsdStageActor.IsValid())
	{
...
void FMyUsdStageViewModel::SaveStageAs(const TCHAR* FilePath)
{
#if USE_USD_SDK
	...
	if (StageActorPtr)
	{
		if (UE::FMyUsdStage UsdStage = UsdStageActor->GetOrOpenUsdStage())
		{
			...
			{
				if (pxr::SdfLayerRefPtr(RootLayer)->Export(TCHAR_TO_UTF8(FilePath)))
				{
...
void FMyUsdStageViewModel::ImportStage(const TCHAR* TargetContentFolder, UMyUsdStageImportOptions* Options)
{
#if USE_USD_SDK
	AMyUsdStageActor* StageActor = UsdStageActor.Get();
	if (!StageActor)
	{
		return;
	}
	...
	// Import directly from stage
	{
		...
		// Preload some settings according to MyUSDStage options. These will overwrite whatever is loaded from config
		...
		ImportContext.ImportOptions->FallbackCollisionType = StageActor->FallbackCollisionType;
		...
		if (ImportContext.Init(StageName, RootPath, TEXT("/Game/"), RF_Public | RF_Transactional | RF_Standalone, bIsAutomated))
		{
			...
			if (bIsAutomated)
			{
				// Apply same conversion that FMyUsdStageImportContext::Init does on our received path
