...
FMyUsdObjectFieldViewModel::FMyUsdObjectFieldViewModel(FMyUsdObjectFieldsViewModel* InOwner)
	: Owner(InOwner)
{
}

TArray<TSharedPtr<FString>> FMyUsdObjectFieldViewModel::GetDropdownOptions() const
{
#if USE_USD_SDK
	if (Label == TEXT("kind"))
	{
...
void FMyUsdObjectFieldViewModel::SetAttributeValue(const UsdUtils::FConvertedVtValue& InValue)
{
	Owner->SetFieldValue(Label, InValue);
}

template<typename T>
void FMyUsdObjectFieldsViewModel::CreateField(
...
)
{
	...
	FMyUsdObjectFieldViewModel Property(this);
	Property.Type = Type;
	...
	Fields.Add(MakeSharedUnreal<FMyUsdObjectFieldViewModel>(MoveTemp(Property)));
}

void FMyUsdObjectFieldsViewModel::CreateField(EObjectFieldType Type, const FString& FieldName, const UsdUtils::FConvertedVtValue& Value, bool bReadOnly)
{
	FMyUsdObjectFieldViewModel Property(this);
	Property.Type = Type;
	Property.Label = FieldName;
...
void FMyUsdObjectFieldsViewModel::SetFieldValue(const FString& FieldName, const UsdUtils::FConvertedVtValue& InValue)
{
	bool bSuccess = false;
	...
	//  - We respond to the notice on AMyUsdStageActor::OnUsdObjectsChanged;
	...
	if (!bSuccess)
	{
		...
		USD_LOG_USERWARNING(ErrorMessage);
	}
#endif	  // #if USE_USD_SDK
}
	...
	if (UsdStage)
	{
		...
		if (ObjectPath.Equals(TEXT("/")) || ObjectPath.IsEmpty())
		{
			...
			for (const pxr::TfToken& Token : TokenVector)
			{
				...
				if (Token == pxr::SdfFieldKeys->TimeCodesPerSecond)
				{
					// There is some fallback logic here (e.g. using framesPerSecond), so don't fetch
					// the raw metadata value generically as that will ignore it
					VtValue = Stage->GetTimeCodesPerSecond();
				}
				else
				{
					bool bSuccess = Stage->GetMetadata(Token, &VtValue);
					if (!bSuccess)
					{
						VtValue = pxr::VtValue{};
					}
...
void FMyUsdObjectFieldsViewModel::Sort()
{
	...
	if (CurrentSortColumn == ObjectFieldColumnIds::TypeColumn)
	{
		...
		{
			Fields.Sort(
				[](const TSharedPtr<FMyUsdObjectFieldViewModel>& A, const TSharedPtr<FMyUsdObjectFieldViewModel>& B)
				{
					if (A->Type == B->Type)
					{
...
FString FMyUsdObjectFieldsViewModel::GetObjectPath() const
{
	return ObjectPath;
}
