		...
		for (const pxr::UsdPrimCompositionQueryArc& CompositionArc : PrimCompositionQuery.GetCompositionArcs())
		{
			...
			else if (CompositionArc.GetArcType() == pxr::PcpArcTypeReference)
			{
				...
				if (CompositionArc.GetIntroducingListEditor(&ReferenceEditor, &UsdReference))
				{
					...
					References.Add(MakeSharedUnreal<FMyUsdReference>(MoveTemp(Reference)));
				}
			}
...
void FMyUsdReferencesViewModel::RemoveReference(const TSharedPtr<FMyUsdReference>& Reference)
{
#if USE_USD_SDK
	FScopedUsdAllocs UsdAllocs;
...
void FMyUsdReferencesViewModel::ReloadReference(const TSharedPtr<FMyUsdReference>& Reference)
{
#if USE_USD_SDK
	FScopedUsdAllocs UsdAllocs;
