...
#include "MyUSDStageEditorSettings.h"
...
#include "Widgets/SMyUSDObjectFieldList.h"
...
#define LOCTEXT_NAMESPACE "SMyUsdStage"

#if USE_USD_SDK

namespace SMyUSDStageConstants
{
	static const FMargin SectionPadding(1.f, 1.f, 1.f, 1.f);
}

namespace SMyUSDStageImpl
{
#if PLATFORM_LINUX
	...
	void SelectGeneratedComponentsAndActors(AMyUsdStageActor* StageActor, const TArray<FString>& PrimPaths)
	{
		if (!StageActor)
		{
			return;
	...
	public:
		...
		static FString GetStageForItem(const ISceneOutlinerTreeItem* Item)
		{
			if (Item)
			{
				...
				{
					if (const AMyUsdStageActor* StageActor = Cast<const AMyUsdStageActor>(ActorTreeItem->Actor.Get()))
					{
						if (const UE::FMyUsdStage& Stage = StageActor->GetUsdStage())
						{
...
void SMyUsdStage::Construct(const FArguments& InArgs)
{
	const bool bFlashButton = true;
	OnActorLoadedHandle = AMyUsdStageActor::OnActorLoaded.AddSP(SharedThis(this), &SMyUsdStage::AttachToStageActor, bFlashButton);

	OnViewportSelectionChangedHandle = USelection::SelectionChangedEvent.AddSP(SharedThis(this), &SMyUsdStage::OnViewportSelectionChanged);

	PostPIEStartedHandle = FEditorDelegates::PostPIEStarted.AddSP(SharedThis(this), &SMyUsdStage::OnPostPIEStarted);
	EndPIEHandle = FEditorDelegates::EndPIE.AddSP(SharedThis(this), &SMyUsdStage::OnEndPIE);

	IMyUsdStageModule& UsdStageModule = FModuleManager::Get().LoadModuleChecked<IMyUsdStageModule>("UsdStage");
	ViewModel.UsdStageActor = UsdStageModule.FindUsdStageActor(IMyUsdClassesModule::GetCurrentWorld());
	...
	const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (StageActor)
	{
		UsdStage = static_cast<const AMyUsdStageActor*>(StageActor)->GetUsdStage();
	}
	...
	[
		...
		[
			...
			+SVerticalBox::Slot()
			.FillHeight(1.f)
			.Padding(SMyUSDStageConstants::SectionPadding)
			[
				SNew(SSplitter)
				...
				[
					...
					[
						...
						[
							SAssignNew(UsdStageTreeView, SMyUsdStageTreeView)
							.OnPrimSelectionChanged(this, &SMyUsdStage::OnPrimSelectionChanged)
						]
					]
					...
					[
						...
						[
							SAssignNew(UsdPrimInfoWidget, SMyUsdPrimInfo)
						]
					]
				]
				...
				[
					...
					.BorderImage(FAppStyle::GetBrush(TEXT("ToolPanel.GroupBorder")))
					[
						SAssignNew(UsdLayersTreeView, SMyUsdLayersTreeView)
						.OnLayerIsolated(this, &SMyUsdStage::OnLayerIsolated)
					]
				]
			]
		]
	...
	FSlateApplication::Get().OnPreTick().AddSP(this, &SMyUsdStage::OnSlateTick);
...
AMyUsdStageActor* SMyUsdStage::GetAttachedStageActor() const
{
	return ViewModel.UsdStageActor.Get();
}

TArray<UE::FSdfLayer> SMyUsdStage::GetSelectedLayers() const
{
...
void SMyUsdStage::SetSelectedLayers(const TArray<UE::FSdfLayer>& NewSelection) const
{
	if (UsdLayersTreeView)
	{
...
TArray<UE::FMyUsdPrim> SMyUsdStage::GetSelectedPrims() const
{
	if (UsdStageTreeView)
	{
...
void SMyUsdStage::SetSelectedPrims(const TArray<UE::FMyUsdPrim>& NewSelection) const
{
	if (UsdStageTreeView)
	{
...
TArray<FString> SMyUsdStage::GetSelectedPropertyNames() const
{
...
void SMyUsdStage::SetSelectedPropertyNames(const TArray<FString>& NewSelection)
{
	if (UsdPrimInfoWidget && UsdPrimInfoWidget->PropertiesList)
	{
...
TArray<FString> SMyUsdStage::GetSelectedPropertyMetadataNames() const
{
...
void SMyUsdStage::SetSelectedPropertyMetadataNames(const TArray<FString>& NewSelection)
{
	if (UsdPrimInfoWidget && UsdPrimInfoWidget->PropertyMetadataPanel)
	{
...
void SMyUsdStage::AttachToStageActor(AMyUsdStageActor* InUsdStageActor, bool bFlashButton)
{
	...
	if (InUsdStageActor)
	{
		UE::FMyUsdStage UsdStage = static_cast<const AMyUsdStageActor*>(InUsdStageActor)->GetUsdStage();
...
void SMyUsdStage::SetupStageActorDelegates()
{
	ClearStageActorDelegates();
	...
	if (ViewModel.UsdStageActor.IsValid())
	{
		...
		OnStageEditTargetChangedHandle = ViewModel.UsdStageActor->GetUsdListener().GetOnStageEditTargetChanged().AddSP(
			this,
			&SMyUsdStage::RequestLayersTreeViewRefresh
		);
...
void SMyUsdStage::ClearStageActorDelegates()
{
	const bool bEvenIfPendingKill = true;
	if (AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get(bEvenIfPendingKill))
	{
...
SMyUsdStage::~SMyUsdStage()
{
	...
	AMyUsdStageActor::OnActorLoaded.Remove(OnActorLoadedHandle);
...
TSharedRef<SWidget> SMyUsdStage::MakeMainMenu()
{
	FMenuBarBuilder MenuBuilder(nullptr);
	{
		...
		MenuBuilder.AddPullDownMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillFileMenu)
		);
		...
		MenuBuilder.AddPullDownMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillActionsMenu)
		);
		...
		MenuBuilder.AddPullDownMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillOptionsMenu)
		);
	}
...
TSharedRef<SWidget> SMyUsdStage::MakeActorPickerMenu()
{
	// clang-format off
	return
		SNew(SOverlay)
		...
		[
			...
			.OnGetMenuContent(this, &SMyUsdStage::MakeActorPickerMenuContent)
			...
			[
				...
				.Text_Lambda([this]()
				{
					if (AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
					{
		...
		[
			...
			.BorderImage(FMyUsdStageEditorStyle::Get()->GetBrush("UsdStageEditor.ActorPickerFlashBrush"))
...
TSharedRef<SWidget> SMyUsdStage::MakeActorPickerMenuContent()
{
	if (!ActorPickerMenu.IsValid())
	{
		...
		FSceneOutlinerColumnInfo StageColumnInfo{
			...
			FCreateSceneOutlinerColumn::CreateLambda(
				...
				{
					return MakeShareable(new SMyUSDStageImpl::FStageNameOutlinerColumn(InSceneOutliner));
				}
			),
			bCanBeHidden,
			FillSize
		...
		InitOptions.ColumnMap.Add(SMyUSDStageImpl::FStageNameOutlinerColumn::GetID(), StageColumnInfo);
		InitOptions.Filters->AddFilterPredicate<FActorTreeItem>(FActorTreeItem::FFilterPredicate::CreateLambda(
			[](const AActor* Actor)
			{
				return Actor && Actor->IsA<AMyUsdStageActor>();
			}
		));
...
TSharedRef<SWidget> SMyUsdStage::MakeIsolateWarningButton()
{
	// clang-format off
	return SNew(SButton)
		...
		{
			if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
			{
		...
		.ToolTipText_Lambda([this]() -> FText
		{
			if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
			{
				if (const UE::FMyUsdStage& CurrentStage = StageActor->GetUsdStage())
				{
					return FText::Format(
...
void SMyUsdStage::FillFileMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.BeginSection("File", LOCTEXT("File", "File"));
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SMyUsdStage::FileNew), FCanExecuteAction()),
			NAME_None,
			...
			FUIAction(
				...
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
						{
							if (UE::FMyUsdStage Stage = StageActor->GetUsdStage())
							{
								return true;
							}
						}
		...
		MenuBuilder.AddSubMenu(LOCTEXT("Export", "Export"), FText::GetEmpty(), FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillExportSubMenu));
	}
	...
	MenuBuilder.BeginSection("Reload", LOCTEXT("Reload", "Reload"));
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SMyUsdStage::FileReload),
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SMyUsdStage::FileReset),
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
	...
	MenuBuilder.BeginSection("Close", LOCTEXT("Close", "Close"));
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SMyUsdStage::FileClose),
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
...
void SMyUsdStage::FillActionsMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.BeginSection("Actions", LOCTEXT("Actions", "Actions"));
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SMyUsdStage::ActionsImportWithDialog),
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SMyUsdStage::ActionsRegenerate),
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
	...
	MenuBuilder.BeginSection("Stage Actor", LOCTEXT("Stage Actor", "Stage Actor"));
	{
		MenuBuilder.AddMenuEntry(
			LOCTEXT("SelectStageActor", "Select stage actor"),
			LOCTEXT("SelectStageActor_ToolTip", "Selects the currently attached USD Stage Actor"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SMyUsdStage::ActionsSelectStageActor),
				FCanExecuteAction::CreateLambda(
					[this]()
					{
		...
		MenuBuilder.AddMenuEntry(
			...
			LOCTEXT("OpenLevelSequence_ToolTip", "Opens the generated LevelSequence asset for this stage"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SMyUsdStage::ActionsOpenLevelSequence),
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						if (const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get())
						{
							return static_cast<bool>(StageActor->GetUsdStage()) && (StageActor->GetLevelSequence() != nullptr);
						}
...
void SMyUsdStage::FillOptionsMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.BeginSection(TEXT("StageOptions"), LOCTEXT("StageOptions", "Stage options"));
	{
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillStageStateSubMenu)
		);

		MenuBuilder.AddSubMenu(
			LOCTEXT("Payloads", "Payloads"),
			LOCTEXT("Payloads_ToolTip", "What to do with payloads when initially opening the stage"),
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillPayloadsSubMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillPurposesToLoadSubMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillRenderContextSubMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillMaterialPurposeSubMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillRootMotionSubMenu)
		);

		MenuBuilder.AddSubMenu(
			LOCTEXT("FallbackCollisionType", "Fallback collision type"),
			LOCTEXT(
				"FallbackCollisionType_ToolTip",
				"What type of collision to use for static meshes generated from Prims that don't have physics schemas applied"
			),
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillFallbackCollisionTypeMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillSubdivisionLevelSubMenu)
		);

		MenuBuilder.AddSubMenu(
			LOCTEXT("Metadata", "Metadata"),
			LOCTEXT("Metadata_ToolTip", "How to collect and handle metadata from USD prims"),
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillMetadataSubMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillCollapsingSubMenu)
		);

		MenuBuilder.AddSubMenu(
			LOCTEXT("AssetReuse", "Asset sharing"),
			LOCTEXT("AssetReuse_ToolTip", "How to behave when generating assets from identical prims"),
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillShareAssetsSubMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillInterpolationTypeSubMenu)
		);
		...
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillGeometryCacheImportSubMenu)
		);
	}
	MenuBuilder.EndSection();
	...
	MenuBuilder.BeginSection(TEXT("EditorOptions"), LOCTEXT("EditorOptions", "Editor options"));
	{
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillSelectionSubMenu)
		);

		MenuBuilder.AddSubMenu(
			LOCTEXT("NaniteSettings", "Nanite"),
			LOCTEXT("NaniteSettings_ToolTip", "Configure how to use Nanite for generated assets"),
			FNewMenuDelegate::CreateSP(this, &SMyUsdStage::FillNaniteThresholdSubMenu),
			false,
			FSlateIcon(),
			false
		);
...
void SMyUsdStage::FillExportSubMenu(FMenuBuilder& MenuBuilder)
{
...
void SMyUsdStage::FillStageStateSubMenu(FMenuBuilder& MenuBuilder)
{
	TFunction<void(EMyUsdStageState, const FText&, const FText&)> AddStageStateEntry =
		[this, &MenuBuilder](EMyUsdStageState NewState, const FText& EntryText, const FText& EntryToolTipText)
	{
		MenuBuilder.AddMenuEntry(
			EntryText,
			EntryToolTipText,
			...
			FUIAction(
				FExecuteAction::CreateLambda(
					...
					{
						if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
						{
							...
							// c.f. comment in SMyUsdStage::FillCollapsingSubMenu
	...
	AddStageStateEntry(
		EMyUsdStageState::OpenedAndLoaded,
...
void SMyUsdStage::FillPayloadsSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
		LOCTEXT("LoadAll", "Load all"),
		LOCTEXT("LoadAll_ToolTip", "Loads all payloads, and make new payloads start out loaded"),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda(
				[this]()
				{
					...
					{
						FScopedTransaction Transaction(FText::Format(
							LOCTEXT("SetLoadAllTransaction", "Set USD stage actor '{0}' actor to load all payloads"),
							FText::FromString(StageActor->GetActorLabel())
						));
			...
			FIsActionChecked::CreateLambda(
				[this]()
				{
					if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
					{
						if (!StageActor->GetUsdStage() || ViewModel.HasDefaultLoadRule())
						{
							return StageActor->InitialLoadSet == EMyUsdInitialLoadSet::LoadAll;
						}
					}
					return false;
				}
	...
	MenuBuilder.AddMenuEntry(
		...
		LOCTEXT("LoadNone_ToolTip", "Unload all payloads, and make new payloads start out unloaded"),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda(
				[this]()
				{
					...
					{
						FScopedTransaction Transaction(FText::Format(
							LOCTEXT("SetLoadNoneTransaction", "Set USD stage actor '{0}' actor to load no payloads"),
							FText::FromString(StageActor->GetActorLabel())
						));
			...
			FIsActionChecked::CreateLambda(
				[this]()
				{
					if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
					{
						if (!StageActor->GetUsdStage() || ViewModel.HasDefaultLoadRule())
						{
							return StageActor->InitialLoadSet == EMyUsdInitialLoadSet::LoadNone;
						}
					}
					return false;
				}
...
void SMyUsdStage::FillPurposesToLoadSubMenu(FMenuBuilder& MenuBuilder)
{
	auto AddPurposeEntry = [&](const EMyUsdPurpose& Purpose, const FText& Text)
	{
		MenuBuilder.AddMenuEntry(
			Text,
			FText::GetEmpty(),
			...
			FUIAction(
				FExecuteAction::CreateLambda(
					[this, Purpose]()
					{
						if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
						{
							...
							StageActor->SetPurposesToLoad((int32)((EMyUsdPurpose)StageActor->PurposesToLoad ^ Purpose));
							if (StageActor->IsTemplate())
							{
								StageActor->SaveConfig();
							}
				...
				FIsActionChecked::CreateLambda(
					[this, Purpose]()
					{
						if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
						{
							return EnumHasAllFlags((EMyUsdPurpose)StageActor->PurposesToLoad, Purpose);
						}
						return false;
					}
				)
	...
	AddPurposeEntry(EMyUsdPurpose::Proxy, LOCTEXT("ProxyPurpose", "Proxy"));
	AddPurposeEntry(EMyUsdPurpose::Render, LOCTEXT("RenderPurpose", "Render"));
	AddPurposeEntry(EMyUsdPurpose::Guide, LOCTEXT("GuidePurpose", "Guide"));
}

void SMyUsdStage::FillRenderContextSubMenu(FMenuBuilder& MenuBuilder)
{
...
void SMyUsdStage::FillMaterialPurposeSubMenu(FMenuBuilder& MenuBuilder)
{
	MaterialPurposes = {
...
void SMyUsdStage::FillRootMotionSubMenu(FMenuBuilder& MenuBuilder)
{
	auto AddRootMotionEntry = [&](const EMyUsdRootMotionHandling& HandlingStrategy, const FText& Text)
	{
		UEnum* Enum = StaticEnum<EMyUsdRootMotionHandling>();
	...
	AddRootMotionEntry(EMyUsdRootMotionHandling::NoAdditionalRootMotion, LOCTEXT("NoAdditionalRootMotionText", "No additional root motion"));
	AddRootMotionEntry(EMyUsdRootMotionHandling::UseMotionFromSkelRoot, LOCTEXT("UseMotionFromSkelRootText", "Use motion from SkelRoot"));
	AddRootMotionEntry(EMyUsdRootMotionHandling::UseMotionFromSkeleton, LOCTEXT("UseMotionFromSkeletonText", "Use motion from Skeleton"));
}

void SMyUsdStage::FillFallbackCollisionTypeMenu(FMenuBuilder& MenuBuilder)
{
	auto AddFallbackCollisionTypeEntry = [&](const EMyUsdCollisionType& NewFallbackType, const FText& Text)
	{
		UEnum* Enum = StaticEnum<EMyUsdCollisionType>();
		FText ToolTip = Enum->GetToolTipTextByIndex(Enum->GetIndexByValue(static_cast<uint8>(NewFallbackType)));
		...
		MenuBuilder.AddMenuEntry(
			...
			FUIAction(
				FExecuteAction::CreateLambda(
					[this, NewFallbackType]()
					{
						if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
						{
							FScopedTransaction Transaction(FText::Format(
								LOCTEXT("FallbackCollisionTypeTransaction", "Change fallback collision type for USD stage actor '{0}'"),
								FText::FromString(StageActor->GetActorLabel())
							));
							...
							StageActor->SetFallbackCollisionType(NewFallbackType);
							if (StageActor->IsTemplate())
							{
								StageActor->SaveConfig();
							}
				...
				FIsActionChecked::CreateLambda(
					[this, NewFallbackType]()
					{
						if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
						{
							return StageActor->FallbackCollisionType == NewFallbackType;
						}
						return false;
					}
				)
	...
	AddFallbackCollisionTypeEntry(EMyUsdCollisionType::None, LOCTEXT("CollisionNone", "None"));
	AddFallbackCollisionTypeEntry(EMyUsdCollisionType::ConvexDecomposition, LOCTEXT("CollisionConvexDecomposition", "Convex decomposition"));
	AddFallbackCollisionTypeEntry(EMyUsdCollisionType::ConvexHull, LOCTEXT("CollisionConvexHull", "Convex hull"));
	AddFallbackCollisionTypeEntry(EMyUsdCollisionType::Sphere, LOCTEXT("CollisionSphere", "Sphere"));
	AddFallbackCollisionTypeEntry(EMyUsdCollisionType::Cube, LOCTEXT("CollisionCube", "Cube"));
	AddFallbackCollisionTypeEntry(EMyUsdCollisionType::MeshSimplification, LOCTEXT("CollisionMeshSimplification", "Mesh simplification"));
	AddFallbackCollisionTypeEntry(EMyUsdCollisionType::Capsule, LOCTEXT("CollisionCapsule", "Capsule"));
}

void SMyUsdStage::FillSubdivisionLevelSubMenu(FMenuBuilder& MenuBuilder)
{
	if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
	{
	...
	TSharedRef<SSpinBox<int32>> Slider = SNew(SSpinBox<int32>)
		...
		.Value(this, &SMyUsdStage::GetSubdivisionLevelValue)
		.OnValueChanged(this, &SMyUsdStage::OnSubdivisionLevelValueChanged)
		.SupportDynamicSliderMaxValue(true)
		.OnValueCommitted(this, &SMyUsdStage::OnSubdivisionLevelValueCommitted);
	// clang-format on
...
void SMyUsdStage::FillMetadataSubMenu(FMenuBuilder& MenuBuilder)
{
	AMyUsdStageActor* StageActor = GetStageActorOrCDO();
	if (!StageActor)
	{
		return;
	}
	...
	DetailView->SetIsPropertyVisibleDelegate(FIsPropertyVisible::CreateLambda(
		...
		{
			static const FName MetadataOptionsName = GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, MetadataOptions);
	...
	DetailView->SetIsCustomRowVisibleDelegate(FIsCustomRowVisible::CreateLambda(
		...
		{
			static const FName FilterPropertyName = GET_MEMBER_NAME_CHECKED(FMyUsdMetadataImportOptions, BlockedPrefixFilters);
...
void SMyUsdStage::FillCollapsingSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
	...
	auto AddKindToCollapseEntry = [&](const EMyUsdDefaultKind Kind, const FText& Text, FCanExecuteAction CanExecuteAction)
	{
		MenuBuilder.AddMenuEntry(
			Text,
			FText::GetEmpty(),
			...
			FUIAction(
				FExecuteAction::CreateLambda(
					[this, Text, Kind]()
					{
						if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
						{
							...
							// We'll take care to try to re-select the same prims within SMyUsdStage::Refresh after the refresh is complete,
							...
							StageActor->SetKindsToCollapse((int32)((EMyUsdDefaultKind)StageActor->KindsToCollapse ^ Kind));
							if (StageActor->IsTemplate())
							{
								StageActor->SaveConfig();
							}
				...
				FIsActionChecked::CreateLambda(
					[this, Kind]()
					{
						if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
						{
							return EnumHasAllFlags((EMyUsdDefaultKind)StageActor->KindsToCollapse, Kind);
						}
						return false;
					}
				)
	...
	AddKindToCollapseEntry(EMyUsdDefaultKind::Model, LOCTEXT("ModelKind", "Model"), FCanExecuteAction{});

	AddKindToCollapseEntry(
		EMyUsdDefaultKind::Component,
		...
		FCanExecuteAction::CreateLambda(
			[this]()
			{
				if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
				{
					...
					if (!EnumHasAnyFlags((EMyUsdDefaultKind)StageActor->KindsToCollapse, EMyUsdDefaultKind::Model))
					{
						return true;
					}
				}
	...
	AddKindToCollapseEntry(
		...
		FCanExecuteAction::CreateLambda(
			[this]()
			{
				...
				{
					if (!EnumHasAnyFlags((EMyUsdDefaultKind)StageActor->KindsToCollapse, EMyUsdDefaultKind::Model)
						&& !EnumHasAnyFlags((EMyUsdDefaultKind)StageActor->KindsToCollapse, EMyUsdDefaultKind::Group))
					{
						return true;
					}
				}
	...
	AddKindToCollapseEntry(EMyUsdDefaultKind::Subcomponent, LOCTEXT("ModelSubcomponent", "Subcomponent"), FCanExecuteAction{});

	MenuBuilder.EndSection();
}

void SMyUsdStage::FillShareAssetsSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
...
void SMyUsdStage::FillInterpolationTypeSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
		...
		FUIAction(
			FExecuteAction::CreateLambda(
				[this]()
				{
					if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
					{
						...
						StageActor->SetInterpolationType(EMyUsdInterpolationType::Linear);
						if (StageActor->IsTemplate())
						{
							StageActor->SaveConfig();
						}
			...
			FIsActionChecked::CreateLambda(
				[this]()
				{
					if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
					{
						return StageActor->InterpolationType == EMyUsdInterpolationType::Linear;
					}
					return false;
				}
			)
		...
		FUIAction(
			FExecuteAction::CreateLambda(
				[this]()
				{
					if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
					{
						...
						StageActor->SetInterpolationType(EMyUsdInterpolationType::Held);
						if (StageActor->IsTemplate())
						{
							StageActor->SaveConfig();
						}
			...
			FIsActionChecked::CreateLambda(
				[this]()
				{
					if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
					{
						return StageActor->InterpolationType == EMyUsdInterpolationType::Held;
					}
					return false;
				}
			)
...
void SMyUsdStage::FillGeometryCacheImportSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
...
void SMyUsdStage::FillSelectionSubMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
		...
		FUIAction(
			FExecuteAction::CreateLambda(
				[this]()
				{
					if (UMyUsdStageEditorSettings* Settings = GetMutableDefault<UMyUsdStageEditorSettings>())
					{
					...
					if (GEditor)
					{
						...
						else if (NumViewportSelected == 0)
						{
							...
							SMyUSDStageImpl::SelectGeneratedComponentsAndActors(
			...
			FIsActionChecked::CreateLambda(
				[]()
				{
					const UMyUsdStageEditorSettings* Settings = GetDefault<UMyUsdStageEditorSettings>();
...
void SMyUsdStage::FillNaniteThresholdSubMenu(FMenuBuilder& MenuBuilder)
{
	if (AMyUsdStageActor* StageActor = GetStageActorOrCDO())
	{
	...
	TSharedRef<SSpinBox<int32>> Slider = SNew(SSpinBox<int32>)
		...
		.Value(this, &SMyUsdStage::GetNaniteTriangleThresholdValue)
		.OnValueChanged(this, &SMyUsdStage::OnNaniteTriangleThresholdValueChanged)
		.SupportDynamicSliderMaxValue(true)
		.OnValueCommitted(this, &SMyUsdStage::OnNaniteTriangleThresholdValueCommitted);
	// clang-format on
...
void SMyUsdStage::FileNew()
{
	ViewModel.NewStage();
}

void SMyUsdStage::FileOpen(const FString& FilePath)
{
...
void SMyUsdStage::FileSave(const FString& FilePath)
{
	const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
...
void SMyUsdStage::FileExportAllLayers(const FString& OutputDirectory)
{
	const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
	...
	UE::FMyUsdStage UsdStage = StageActor->GetUsdStage();
	if (!UsdStage)
	{
		return;
	}
	...
	SMyUSDStageImpl::AppendAllExternalReferences(LayerStack);
	...
	for (UE::FSdfLayer& Layer : LayerStack)
	{
		...
		if (FString* TargetLayerPath = OldPathToSavedPath.Find(LayerPath))
		{
			...
			// Remap references to layers so that they point at the other newly saved files. Note that SMyUSDStageImpl::AppendAllExternalReferences
...
void SMyUsdStage::FileExportFlattenedStage(const FString& OutputLayer)
{
	const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
	...
	if (!bResult)
	{
		USD_LOG_USERERROR(FText::Format(
			LOCTEXT("FailedToExportFlattened", "Failed to export flattened USD Stage to path '{0}'!"),
			FText::FromString(OutputLayerCopy)
		));
	}
...
void SMyUsdStage::FileExportFlattenedLayerStack(const FString& OutputLayer)
{
	const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
	...
	{
		USD_LOG_USERERROR(FText::Format(
			LOCTEXT("FailedToExportFlattenedLayerStack", "Failed to export flattened layer stack to path '{0}'!"),
			FText::FromString(OutputLayerCopy)
		));
	}
...
void SMyUsdStage::FileReload()
{
	AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
		return;
	}
...
void SMyUsdStage::FileClose()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(SMyUsdStage::FileClose);
...
void SMyUsdStage::OnLayerIsolated(const UE::FSdfLayer& IsolatedLayer)
{
	FScopedTransaction Transaction(LOCTEXT("StopIsolatingButtonTransaction", "Changed layer isolation"));
...
void SMyUsdStage::ActionsImportWithDialog()
{
	ViewModel.ImportStage();
}

void SMyUsdStage::ActionsImport(const FString& OutputContentFolder, UMyUsdStageImportOptions* Options)
{
	ViewModel.ImportStage(*OutputContentFolder, Options);
}

void SMyUsdStage::ActionsRegenerate()
{
...
void SMyUsdStage::ActionsSelectStageActor()
{
	AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
	...
	const bool bSelected = true;
	const bool bNotifySelectionChanged = true;
	GEditor->SelectActor(StageActor, bSelected, bNotifySelectionChanged);
}

void SMyUsdStage::ActionsOpenLevelSequence()
{
	AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
	...
	ULevelSequence* LevelSequence = StageActor->GetLevelSequence();
	if (!LevelSequence)
	{
		return;
	}
...
void SMyUsdStage::ExportSelectedLayers(const FString& OutputLayerOrDirectory)
{
	if (UsdLayersTreeView)
	{
...
void SMyUsdStage::OnPrimSelectionChanged(const TArray<FString>& PrimPaths)
{
	...
	// AMyUsdStageActor::LoadUsdStage calls AMyUsdStageActor::OpenUsdStage, which triggers OnStageChanged
	// and gets us here. Ideally this wouldn't happen at the same time as AMyUsdStageActor::LoadUsdStage
	// because we need the InfoCache to be complete at this point. Strangely though, the SlowTask.EnterProgressFrame
	// within AMyUsdStageActor::LoadUsdStage triggers a full slate tick right there, that calls us before
	...
	if (Settings && Settings->bSelectionSynced && GEditor && !bIsRootSelection)
	{
		...
		SMyUSDStageImpl::SelectGeneratedComponentsAndActors(StageActor, PrimPaths);
	}
}

void SMyUsdStage::OpenStage(const TCHAR* FilePath)
{
...
void SMyUsdStage::RequestLayersTreeViewRefresh()
{
	if (UsdLayersTreeView)
	{
		FWriteScopeLock LayersTreeViewLock{UsdLayersTreeView->RefreshStateLock};

		UsdLayersTreeView->bUpdateState = (SMyUsdLayersTreeView::ELayersTreeViewState
		)FMath::Max((uint8)SMyUsdLayersTreeView::ELayersTreeViewState::RefreshNeeded, (uint8)UsdLayersTreeView->bUpdateState);
	}
}

void SMyUsdStage::RequestFullRefresh()
{
	if (UsdLayersTreeView)
	{
		FWriteScopeLock LayersTreeViewLock{UsdLayersTreeView->RefreshStateLock};

		UsdLayersTreeView->bUpdateState = SMyUsdLayersTreeView::ELayersTreeViewState::ResyncNeeded;
	}
...
void SMyUsdStage::OnSlateTick(float Time)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(SMyUsdStage::OnSlateTick);
	...
	SMyUsdLayersTreeView::ELayersTreeViewState LayersTreeViewState = SMyUsdLayersTreeView::ELayersTreeViewState::NoRefreshNeeded;
	...
	if (LayersTreeViewState != SMyUsdLayersTreeView::ELayersTreeViewState::NoRefreshNeeded)
	{
		// May be nullptr, but that is ok. Its how the widgets are reset
		AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();

		UE::FMyUsdStageWeak IsolatedStage = StageActor ? StageActor->GetIsolatedUsdStage() : UE::FMyUsdStage{};
		UE::FMyUsdStageWeak BaseStage = StageActor ? StageActor->GetBaseUsdStage() : UE::FMyUsdStage{};

		// The layers tree view always needs to receive both the full stage as well as the isolated
		const bool bResync = LayersTreeViewState == SMyUsdLayersTreeView::ELayersTreeViewState::ResyncNeeded;
...
void SMyUsdStage::OnViewportSelectionChanged(UObject* NewSelection)
{
	...
	AMyUsdStageActor* AttachedStageActor = ViewModel.UsdStageActor.Get();
	...
	AMyUsdStageActor* OwnerActor = nullptr;
	if (AllSelectedComponents.Num() > 0)
	{
		...
		if (AttachParent)
		{
			OwnerActor = Cast<AMyUsdStageActor>(AttachParent->GetOwner());
		}
	}
	if (!OwnerActor)
	{
...
void SMyUsdStage::OnPostPIEStarted(bool bIsSimulating)
{
	AMyUsdStageActor* CurrentStageActor = ViewModel.UsdStageActor.Get();
	if (!CurrentStageActor)
	{
		return;
	}

	AMyUsdStageActor* PIEStageActor = Cast<AMyUsdStageActor>(EditorUtilities::GetSimWorldCounterpartActor(CurrentStageActor));
	if (!PIEStageActor)
	{
		return;
	}
...
void SMyUsdStage::OnEndPIE(bool bIsSimulating)
{
	AMyUsdStageActor* CurrentStageActor = ViewModel.UsdStageActor.Get();
	if (!CurrentStageActor)
	{
	...
	AMyUsdStageActor* EditorStageActor = Cast<AMyUsdStageActor>(EditorUtilities::GetEditorWorldCounterpartActor(CurrentStageActor));
	if (!EditorStageActor)
	{
		return;
	}
...
int32 SMyUsdStage::GetNaniteTriangleThresholdValue() const
{
	return CurrentNaniteThreshold;
}

void SMyUsdStage::OnNaniteTriangleThresholdValueChanged(int32 InValue)
{
	CurrentNaniteThreshold = InValue;
}

void SMyUsdStage::OnNaniteTriangleThresholdValueCommitted(int32 InValue, ETextCommit::Type InCommitType)
{
	AMyUsdStageActor* StageActor = GetStageActorOrCDO();
	if (!StageActor)
	{
...
int32 SMyUsdStage::GetSubdivisionLevelValue() const
{
	return CurrentSubdivisionLevel;
}

void SMyUsdStage::OnSubdivisionLevelValueChanged(int32 InValue)
{
	CurrentSubdivisionLevel = InValue;
}

void SMyUsdStage::OnSubdivisionLevelValueCommitted(int32 InValue, ETextCommit::Type InCommitType)
{
	AMyUsdStageActor* StageActor = GetStageActorOrCDO();
	if (!StageActor)
	{
...
UE::FMyUsdStageWeak SMyUsdStage::GetCurrentStage() const
{
	const AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (StageActor)
	{
	...
	return UE::FMyUsdStageWeak{};
}

AMyUsdStageActor* SMyUsdStage::GetStageActorOrCDO() const
{
	AMyUsdStageActor* StageActor = ViewModel.UsdStageActor.Get();
	if (!StageActor)
	{
		StageActor = GetMutableDefault<AMyUsdStageActor>();
	}
	ensure(StageActor);
	return StageActor;
}
