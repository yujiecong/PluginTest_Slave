...
#include "Widgets/SMyUSDIntegrationsPanel.h"
...
void SMyUsdPrimInfo::Construct(const FArguments& InArgs)
{
	...
	[
		...
		.FillHeight(1.f)
		[
			SAssignNew(PropertiesList, SMyUsdObjectFieldList)
			.NameColumnText(LOCTEXT("NameColumnText", "Name"))
			.OnSelectionChanged_Lambda([this](const TSharedPtr<FMyUsdObjectFieldViewModel>& NewSelection, ESelectInfo::Type SelectionType)
			{
		...
		[
			SAssignNew(PropertyMetadataPanel, SMyUsdObjectFieldList)
		...
		[
			SAssignNew(IntegrationsPanel, SMyUsdIntegrationsPanel)
		]
		...
		[
			SAssignNew(ReferencesList, SMyUsdReferencesList)
		]
	];
	// clang-format on
}
