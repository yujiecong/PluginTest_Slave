...
#include "MyUSDUtilitiesModule.h"
...
{
	const static FTabId UsdStageEditorTabID{TEXT("MyUSDStage")};

	void SaveStageActorLayersForWorld(UWorld* World, bool bForClosing, AMyUsdStageActor* TargetStageActor = nullptr)
	{
#if USE_USD_SDK
		if (!World)
		{
		...
		TArray<AMyUsdStageActor*> StageActorsToVisit;
		if (TargetStageActor)
		{
		...
		{
			for (TActorIterator<AMyUsdStageActor> It{World}; It; ++It)
			{
		...
		TMap<FString, FMyUsdSaveDialogRowData> RowsByIdentifier;
		for (AMyUsdStageActor* StageActor : StageActorsToVisit)
		{
			if (!StageActor)
			{
				continue;
			...
			for (const UE::FSdfLayer& UsedLayer : UsedLayers)
			{
				...
				{
					FMyUsdSaveDialogRowData& RowData = RowsByIdentifier.FindOrAdd(UsedLayer.GetIdentifier());
		...
		TArray<FMyUsdSaveDialogRowData> Rows;
		RowsByIdentifier.GenerateValueArray(Rows);

		if (bPrompt)
		{
			Rows.Sort(
				[](const FMyUsdSaveDialogRowData& Left, const FMyUsdSaveDialogRowData& Right)
				{
		...
		for (const FMyUsdSaveDialogRowData& ReturnedRow : Rows)
		{
				...
				if (PinnedLayer.IsAnonymous())
				{
					...
					if (UsdFilePath)
					{
						...
						if (bSaved && !bForClosing)
						{
							...
							for (AMyUsdStageActor* StageActor : ReturnedRow.ConsumerActors)
							{
								if (StageActor->RootLayer.FilePath == ExpectedIdentifier
									&& static_cast<const AMyUsdStageActor*>(StageActor)->GetUsdStage())
								{
...
#if USE_USD_SDK
	TSharedPtr<SMyUsdStage> GetUsdStageEditor(bool bOpenIfNeeded = true)
	{
		...
		if (Tab)
		{
			...
			{
				if (TSharedPtr<SMyUsdStage> UsdStageEditor = StaticCastSharedRef<SMyUsdStage>(ContentBorder->GetContent()))
				{
...
class FMyUsdStageEditorModule : public IMyUsdStageEditorModule
{
public:
#if USE_USD_SDK
	...
	virtual void StartupModule() override
	{
		...
		StageActorLoadedHandle = AMyUsdStageActor::OnActorLoaded.AddLambda(
			[this](AMyUsdStageActor* StageActor)
			{
				if (!StageActor)
				{
					return;
				...
				TWeakObjectPtr<AMyUsdStageActor> WeakActor = StageActor;
				StageActor->OnPreStageChanged.AddLambda(
					[this, WeakActor]()
					{
						AMyUsdStageActor* StageActor = WeakActor.Get();
						if (!bUndoRedoing && StageActor && static_cast<const AMyUsdStageActor*>(StageActor)->GetUsdStage())
						{
							const bool bForClosing = true;
		...
		PreSaveWorldEditorDelegateHandle = FEditorDelegates::PreSaveWorldWithContext.AddLambda(
			[](UWorld* World, FObjectPreSaveContext InContext)
			{
				...
				if (InContext.IsFromAutoSave() || InContext.IsProceduralSave())
				{
					return;
				}
		...
		OpenStageEditorClickedHandle = AMyUsdStageActor::OnOpenStageEditorClicked.AddLambda(
			[](AMyUsdStageActor* StageActor)
			{
				FLevelEditorModule& LevelEditorModule = FModuleManager::GetModuleChecked<FLevelEditorModule>("LevelEditor");
				TSharedPtr<FTabManager> LevelEditorTabManager = LevelEditorModule.GetLevelEditorTabManager();
				if (TSharedPtr<SDockTab> Tab = LevelEditorTabManager->TryInvokeTab(FTabId{TEXT("MyUSDStage")}))
				{
					if (TSharedPtr<SBorder> ContentBorder = StaticCastSharedRef<SBorder>(Tab->GetContent()))
					{
						if (TSharedPtr<SMyUsdStage> UsdStageEditor = StaticCastSharedRef<SMyUsdStage>(ContentBorder->GetContent()))
	...
	{
		AMyUsdStageActor::OnOpenStageEditorClicked.Remove(OpenStageEditorClickedHandle);

		FEditorDelegates::PreSaveWorldWithContext.Remove(PreSaveWorldEditorDelegateHandle);

		AMyUsdStageActor::OnActorLoaded.Remove(StageActorLoadedHandle);
...
bool IMyUsdStageEditorModule::OpenStageEditor() const
{
#if USE_USD_SDK
...
bool IMyUsdStageEditorModule::CloseStageEditor() const
{
#if USE_USD_SDK
	FLevelEditorModule& LevelEditorModule = FModuleManager::GetModuleChecked<FLevelEditorModule>("LevelEditor");
	TSharedPtr<FTabManager> LevelEditorTabManager = LevelEditorModule.GetLevelEditorTabManager();
...
bool IMyUsdStageEditorModule::IsStageEditorOpened() const
{
#if USE_USD_SDK
...
AMyUsdStageActor* IMyUsdStageEditorModule::GetAttachedStageActor() const
{
#if USE_USD_SDK
...
bool IMyUsdStageEditorModule::SetAttachedStageActor(AMyUsdStageActor* NewActor) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
TArray<UE::FSdfLayer> IMyUsdStageEditorModule::GetSelectedLayers() const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::SetSelectedLayers(const TArray<UE::FSdfLayer>& NewSelection) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
TArray<FString> IMyUsdStageEditorModule::GetSelectedPropertyNames() const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::SetSelectedPropertyNames(const TArray<FString>& NewSelection) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
TArray<FString> IMyUsdStageEditorModule::GetSelectedPropertyMetadataNames() const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::SetSelectedPropertyMetadataNames(const TArray<FString>& NewSelection) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileNew() const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileOpen(const FString& FilePath) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileSave(const FString& OutputFilePathIfUnsaved) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileExportAllLayers(const FString& OutputDirectory) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileExportFlattenedStage(const FString& OutputLayer) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileExportFlattenedLayerStack(const FString& OutputLayer) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileReload() const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileReset() const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::FileClose() const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::ActionsImport(const FString& OutputContentFolder, UMyUsdStageImportOptions* Options) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
void IMyUsdStageEditorModule::ExportSelectedLayers(const FString& OutputLayerOrDirectory) const
{
#if USE_USD_SDK
	if (TSharedPtr<SMyUsdStage> UsdStageEditor = UE::UsdStageEditorModule::Private::GetUsdStageEditor())
	{
...
IMPLEMENT_MODULE_USD(FMyUsdStageEditorModule, MyUSDStageEditor);
