...
#include "SMyUSDObjectFieldList.h"
...
#define LOCTEXT_NAMESPACE "SMyUsdObjectFieldList"
...
namespace USDObjectFieldListImpl
{
	...
	TArray<TSharedPtr<FString>>* GetTokenDropdownOptions(const FMyUsdObjectFieldViewModel& ViewModel)
	{
...
class SMyUsdObjectFieldRow : public SMultiColumnTableRow<TSharedPtr<FMyUsdObjectFieldViewModel>>
{
	SLATE_BEGIN_ARGS(SMyUsdObjectFieldRow)
	{
	}
	SLATE_END_ARGS()
...
public:
	void Construct(
		...
		const TSharedPtr<FMyUsdObjectFieldViewModel>& InUsdObjectField,
		const TSharedRef<STableViewBase>& OwnerTable
	);
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName) override;

	void SetUsdObjectField(const TSharedPtr<FMyUsdObjectFieldViewModel>& InUsdObjectField);
...
private:
	TSharedPtr<FMyUsdObjectFieldViewModel> Field;
};

void SMyUsdObjectFieldRow::Construct(
	const FArguments& InArgs,
	const TSharedPtr<FMyUsdObjectFieldViewModel>& InUsdObjectField,
	const TSharedRef<STableViewBase>& OwnerTable
)
{
	...
	SMultiColumnTableRow<TSharedPtr<FMyUsdObjectFieldViewModel>>::Construct(
		SMultiColumnTableRow<TSharedPtr<FMyUsdObjectFieldViewModel>>::FArguments(),
		OwnerTable
	);
}

TSharedRef<SWidget> SMyUsdObjectFieldRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	{
		ColumnWidget = GenerateTextWidget({this, &SMyUsdObjectFieldRow::GetType}, {this, &SMyUsdObjectFieldRow::GetTypeToolTip}, ETextJustify::Center);
	}
	else if (ColumnName == ObjectFieldColumnIds::NameColumn)
	{
		ColumnWidget = GenerateTextWidget({this, &SMyUsdObjectFieldRow::GetLabel});
	}
	else
	{
		...
		switch (WidgetType)
		{
			case EWidgetType::Bool:
				ColumnWidget = GenerateCheckboxWidget({this, &SMyUsdObjectFieldRow::GetValueBool});
				break;
			case EWidgetType::U8:
			...
			case EWidgetType::Dropdown:
			{
				...
				if (Options)
				{
					...
					[
						...
						{
							return SMyUsdObjectFieldRow::GenerateTextWidget(FText::FromString(*Option));
						})
						.OnSelectionChanged(this, &SMyUsdObjectFieldRow::OnComboBoxSelectionChanged)
						[
							...
							.Text(this, &SMyUsdObjectFieldRow::GetValueText)
							...
							.OnTextCommitted(this, &SMyUsdObjectFieldRow::OnTextBoxTextCommitted)
						]
					];
					// clang-format on
			...
			{
				ColumnWidget = SMyUsdObjectFieldRow::GenerateEditableTextWidget({this, &SMyUsdObjectFieldRow::GetValueText}, Field->bReadOnly);
				break;
			}
...
void SMyUsdObjectFieldRow::SetUsdObjectField(const TSharedPtr<FMyUsdObjectFieldViewModel>& InUsdObjectField)
{
	Field = InUsdObjectField;
}

template<typename T>
TArray<TAttribute<TOptional<T>>> SMyUsdObjectFieldRow::GetAttributeArray()
{
	...
	if (Field->Value.Entries.Num() == 1)
	{
		...
		{
			Attributes.Add(AttrType::Create(AttrType::FGetter::CreateSP(this, &SMyUsdObjectFieldRow::template GetValue<T>, ComponentIndex)));
		}
	}
...
TSharedRef<SWidget> SMyUsdObjectFieldRow::GenerateTextWidget(
	const TAttribute<FText>& Attribute,
	const TAttribute<FText>& ToolTipAttribute,
...
TSharedRef<SWidget> SMyUsdObjectFieldRow::GenerateEditableTextWidget(const TAttribute<FText>& Attribute, bool bIsReadOnly)
{
	// clang-format off
	return SNew(SBox)
		.HeightOverride(FMyUsdStageEditorStyle::Get()->GetFloat("UsdStageEditor.ListItemHeight"))
...
TSharedRef<SWidget> SMyUsdObjectFieldRow::GenerateCheckboxWidget(const TAttribute<ECheckBoxState>& Attribute)
{
	// clang-format off
	return SNew(SBox)
		.HeightOverride(FMyUsdStageEditorStyle::Get()->GetFloat("UsdStageEditor.ListItemHeight"))
		...
		[
			SNew(SCheckBox)
			.IsChecked(Attribute)
			.OnCheckStateChanged(this, &SMyUsdObjectFieldRow::OnCheckBoxCheckStateChanged)
		];
	// clang-format on
}

template<typename T>
TSharedRef<SWidget> SMyUsdObjectFieldRow::GenerateSpinboxWidgets(
	...
	for (int32 RowIndex = 0; RowIndex <NumRows; ++RowIndex)
	{
		...
		for (int32 ColumnIndex = 0; ColumnIndex <NumColumns; ++ColumnIndex)
		{
			...
			TSharedRef<SWidget> EntryBox = SNew(SNumericEntryBox<T>)
				...
				.OnValueChanged(this, &SMyUsdObjectFieldRow::OnSpinboxValueChanged<T>, ComponentIndex)
				.OnValueCommitted(this, &SMyUsdObjectFieldRow::OnSpinboxValueCommitted<T>, ComponentIndex)
				.Value(Attribute)
...
template<typename T>
void SMyUsdObjectFieldRow::OnSpinboxValueChanged(T NewValue, int32 ComponentIndex)
{
	Field->Value.Entries[0][ComponentIndex].Set<T>(NewValue);
}

template<typename T>
void SMyUsdObjectFieldRow::OnSpinboxValueCommitted(T InNewValue, ETextCommit::Type CommitType, int32 ComponentIndex)
{
...
void SMyUsdObjectFieldRow::OnComboBoxSelectionChanged(TSharedPtr<FString> ChosenOption, ESelectInfo::Type SelectInfo)
{
	if (Field->bReadOnly)
	{
		return;
...
void SMyUsdObjectFieldRow::OnTextBoxTextCommitted(const FText& NewText, ETextCommit::Type CommitType)
{
	if (CommitType == ETextCommit::OnCleared || Field->bReadOnly)
	{
		return;
...
void SMyUsdObjectFieldRow::OnCheckBoxCheckStateChanged(ECheckBoxState NewState)
{
...
void SMyUsdObjectFieldList::Construct(const FArguments& InArgs)
{
	...
	.FixedWidth(24.0f)
	.SortMode(this, &SMyUsdObjectFieldList::GetColumnSortMode, ObjectFieldColumnIds::TypeColumn)
	.OnSort(this, &SMyUsdObjectFieldList::Sort)

	+SHeaderRow::Column(ObjectFieldColumnIds::NameColumn)
	.DefaultLabel(InArgs._NameColumnText)
	.FillWidth(40.f)
	.SortMode(this, &SMyUsdObjectFieldList::GetColumnSortMode, ObjectFieldColumnIds::NameColumn)
	.OnSort(this, &SMyUsdObjectFieldList::Sort)
	...
	(
		...
		.OnGenerateRow(this, &SMyUsdObjectFieldList::OnGenerateRow)
...
TSharedRef<ITableRow> SMyUsdObjectFieldList::OnGenerateRow(
	TSharedPtr<FMyUsdObjectFieldViewModel> InDisplayNode,
	const TSharedRef<STableViewBase>& OwnerTable
)
{
...
void SMyUsdObjectFieldList::Sort(const EColumnSortPriority::Type SortPriority, const FName& ColumnId, const EColumnSortMode::Type NewSortMode)
{
...
EColumnSortMode::Type SMyUsdObjectFieldList::GetColumnSortMode(const FName ColumnId) const
{
...
TArray<FString> SMyUsdObjectFieldList::GetSelectedFieldNames() const
{
	TArray<FString> SelectedProperties;

	TArray<TSharedPtr<FMyUsdObjectFieldViewModel>> SelectedViewModels = GetSelectedItems();
	SelectedProperties.Reserve(SelectedViewModels.Num());

	for (const TSharedPtr<FMyUsdObjectFieldViewModel>& SelectedItem : SelectedViewModels)
	{
		if (SelectedItem)
		{
...
void SMyUsdObjectFieldList::SetSelectedFieldNames(const TArray<FString>& NewSelection)
{
	TSet<FString> NewSelectionSet{NewSelection};

	Private_ClearSelection();

	TArray<TSharedPtr<FMyUsdObjectFieldViewModel>> NewItemSelection;
	NewItemSelection.Reserve(NewSelection.Num());

	for (const TSharedPtr<FMyUsdObjectFieldViewModel>& Item : ViewModel.Fields)
	{
...
FString SMyUsdObjectFieldList::GetObjectPath() const
{
