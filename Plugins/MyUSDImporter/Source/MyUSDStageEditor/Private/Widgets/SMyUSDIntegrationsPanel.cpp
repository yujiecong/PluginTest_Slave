...
TSharedRef<SWidget> SMyUsdIntegrationsPanelRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	FOptionalSize RowHeight = FMyUsdStageEditorStyle::Get()->GetFloat("UsdStageEditor.ListItemHeight");
	...
	else
	{
		...
		{
			if (UMyUsdIntegrationsPanelPropertyDummy* Dummy = GetMutableDefault<UMyUsdIntegrationsPanelPropertyDummy>())
			{
				...
				TSharedPtr<class ISinglePropertyView> PropertyView = PropertyEditor.CreateSingleProperty(
					Dummy,
					GET_MEMBER_NAME_CHECKED(UMyUsdIntegrationsPanelPropertyDummy, AnimBPProperty),
					Params
				);
				...
				FSimpleDelegate PropertyChanged = FSimpleDelegate::CreateLambda(
					[AttributeCopy]()
					{
						...
						UMyUsdIntegrationsPanelPropertyDummy* Dummy = GetMutableDefault<UMyUsdIntegrationsPanelPropertyDummy>();
		...
		else if (AttributeName == *UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigPath))
		{
			if (UMyUsdIntegrationsPanelPropertyDummy* Dummy = GetMutableDefault<UMyUsdIntegrationsPanelPropertyDummy>())
			{
				...
				TSharedPtr<class ISinglePropertyView> PropertyView = PropertyEditor.CreateSingleProperty(
					Dummy,
					GET_MEMBER_NAME_CHECKED(UMyUsdIntegrationsPanelPropertyDummy, ControlRigProperty),
					Params
				);
...
void SMyUsdIntegrationsPanel::Construct(const FArguments& InArgs)
{
	// clang-format off
	SAssignNew(HeaderRowWidget, SHeaderRow)

	+SHeaderRow::Column(FName(TEXT("PropertyName")))
	.DefaultLabel(NSLOCTEXT("SMyUsdIntegrationsPanel", "Integrations", "Integrations"))
	.FillWidth(30.0f)
	...
	(
		...
		.OnGenerateRow(this, &SMyUsdIntegrationsPanel::OnGenerateRow)
