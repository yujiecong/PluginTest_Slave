...
bool FMyUsdStageImportContext::Init(
...
)
{
	...
	if (!bIsAutomated)
	{
		...
		bool bProceedWithImport = SMyUsdOptionsWindow::ShowImportOptions(*ImportOptions, &Stage);
...
void FMyUsdStageImportContext::Reset()
{
	World = nullptr;
	...
	PackagePath = FString{};
	FilePath = FString{};
	ImportOptions = NewObject<UMyUsdStageImportOptions>();
