...
bool UMyUsdStageAssetImportFactory::DoesSupportClass(UClass* Class)
{
	return (Class == UStaticMesh::StaticClass() || Class == USkeletalMesh::StaticClass());
}

UClass* UMyUsdStageAssetImportFactory::ResolveSupportedClass()
{
	return UStaticMesh::StaticClass();
}

UObject* UMyUsdStageAssetImportFactory::FactoryCreateFile(
	UClass* InClass,
	UObject* InParent,
	FName InName,
	EObjectFlags Flags,
...
bool UMyUsdStageAssetImportFactory::FactoryCanImport(const FString& Filename)
{
...
void UMyUsdStageAssetImportFactory::CleanUp()
{
	ImportContext.Reset();
	Super::CleanUp();
}

bool UMyUsdStageAssetImportFactory::CanReimport(UObject* Obj, TArray<FString>& OutFilenames)
{
...
void UMyUsdStageAssetImportFactory::SetReimportPaths(UObject* Obj, const TArray<FString>& NewReimportPaths)
{
...
EReimportResult::Type UMyUsdStageAssetImportFactory::Reimport(UObject* Obj)
{
	if (!Obj)
	{
	...
	bool bSuccess = MyUSDImporter->ReimportSingleAsset(ImportContext, Obj, OriginalPrimPath, ReimportedAsset);
...
int32 UMyUsdStageAssetImportFactory::GetPriority() const
{
