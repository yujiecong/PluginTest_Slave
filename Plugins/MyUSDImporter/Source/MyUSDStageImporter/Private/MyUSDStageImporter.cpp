...
#include "MyUSDNaniteAssemblyTranslator.h"
...
#include "MyUSDStageImportContext.h"
...
#define LOCTEXT_NAMESPACE "MyUSDStageImporter"

namespace UE::MyUSDStageImporter::Private
{
	void OpenStage(FMyUsdStageImportContext& ImportContext, bool bNeedsMasking)
	{
		...
		{
			for (const UE::FMyUsdStage& OpenedStage : UnrealUSDWrapper::GetAllStagesFromCache())
			{
		...
		{
			ImportContext.Stage = UE::FMyUsdStage();
	...
	void SetupSceneActor(FMyUsdStageImportContext& ImportContext)
	{
	...
	AActor* GetExistingSceneActor(FMyUsdStageImportContext& ImportContext)
	{
	...
	void SetupStageForImport(FMyUsdStageImportContext& ImportContext)
	{
#if USE_USD_SDK
	...
	void CreateAssetsForPrims(const TArray<UE::FMyUsdPrim>& Prims, FMyUsdSchemaTranslationContext& TranslationContext, const FText& ProgressMessage)
	{
	...
	void CreateNaniteAssemblyAssets(const TArray<UE::FMyUsdPrim>& Prims, FMyUsdSchemaTranslationContext& TranslationContext)
	{
		...
		for (const UE::FMyUsdPrim& Prim : Prims)
		{
			FMyUsdNaniteAssemblyTranslator AssemblyTranslator(TranslationContext.AsShared(), UE::FMyUsdTyped(Prim));
	...
	void CacheCollapsingState(FMyUsdSchemaTranslationContext& TranslationContext)
	{
	...
	void ImportMaterials(FMyUsdStageImportContext& ImportContext, FMyUsdSchemaTranslationContext& TranslationContext)
	{
		...
		TArray<UE::FMyUsdPrim> MaterialPrims = UsdUtils::GetAllPrimsOfType(ImportContext.Stage.GetPseudoRoot(), TEXT("UsdShadeMaterial"));

		CreateAssetsForPrims(MaterialPrims, TranslationContext, LOCTEXT("CreateMaterials", "Creating materials"));
	}

	void ImportMeshes(FMyUsdStageImportContext& ImportContext, FMyUsdSchemaTranslationContext& TranslationContext)
	{
		...
		auto PruneCollapsedMeshes = [&TranslationContext](const UE::FMyUsdPrim& UsdPrim) -> bool
		{
			if (TSharedPtr<FMyUsdSchemaTranslator> SchemaTranslator = FMyUsdSchemaTranslatorRegistry::Get().CreateTranslatorForSchema(
					TranslationContext.AsShared(),
		...
		TArray<UE::FMyUsdPrim> MeshPrims = UsdUtils::GetAllPrimsOfType(
	...
	)
	{
		...
		UMyUsdPrimTwin* UsdPrimTwin = NewObject<UMyUsdPrimTwin>();
	...
	)
	{
		...
		// Refresh our prim reference because FMyUsdGeomXformableTranslator::CreateComponents can potentially flip through
		...
		if (bExpandChilren)
		{
			...
			for (UE::FMyUsdPrim Child : Prim.GetFilteredChildren(bTraverseInstanceProxies))
			{
		...
		if (Component)
		{
			...
			else if (EMyUsdDrawMode DrawMode = UsdUtils::GetAppliedDrawMode(Prim); DrawMode != EMyUsdDrawMode::Default)
			{
	...
	void ImportActors(FMyUsdStageImportContext& ImportContext, FMyUsdSchemaTranslationContext& TranslationContext)
	{
		if (!ImportContext.ImportOptions->bImportActors)
		{
			return;
		...
		UE::FMyUsdPrim RootPrim = ImportContext.Stage.GetPseudoRoot();
	...
	void UpdateAssetImportData(UObject* Asset, const FString& MainFilePath, UMyUsdStageImportOptions* ImportOptions)
	{
		if (!Asset)
		{
			return;
		...
		UMyUsdAssetImportData* ImportData = UsdUnreal::ObjectUtils::GetAssetImportData(Asset);
		if (!ImportData)
		{
			ImportData = NewObject<UMyUsdAssetImportData>(Asset);
	...
	void UpdateAssetUserData(const TSet<UObject*>& UsedAssetsAndDependencies, const FString& MainFilePath, UMyUsdStageImportOptions* ImportOptions)
	{
	...
	void PrunePrimMetadata(UObject* Asset, const UE::FMyUsdStage& Stage)
	{
		...
		UMyUsdAssetUserData* UserData = UserDataInterface->GetAssetUserData<UMyUsdAssetUserData>();
		if (!UserData)
		{
			return;
		}
		...
		for (TMap<FString, FMyUsdCombinedPrimMetadata>::TIterator Iter = UserData->StageIdentifierToMetadata.CreateIterator(); Iter; ++Iter)
		{
	...
	void CopyOriginalMaterialAssignment(FMyUsdStageImportContext& ImportContext, UObject* ExistingAsset, UObject* NewAsset)
	{
	...
	void CopySkeletonAssignment(FMyUsdStageImportContext& ImportContext, UObject* ExistingAsset, UObject* NewAsset)
	{
		USkeletalMesh* ExistingSkeletalMesh = Cast<USkeletalMesh>(ExistingAsset);
		USkeletalMesh* NewSkeletalMesh = Cast<USkeletalMesh>(NewAsset);
		if (ExistingSkeletalMesh && NewSkeletalMesh)
	...
	void CloseStageIfNeeded(FMyUsdStageImportContext& ImportContext)
	{
#if USE_USD_SDK
		...
		// the non-static data member of UMyUsdStageImportFactory
		ImportContext.Stage = UE::FMyUsdStage();
		ImportContext.LevelSequenceHelper.Init(UE::FMyUsdStage());
#endif	  // #if USE_USD_SDK
	}

	/**
	 * UMyUsdAssetCache3 can track which assets are requested/added to itself during translation, but it may miss some dependencies
	 ...
	 */
	void CollectUsedAssetDependencies(FMyUsdStageImportContext& ImportContext, TSet<UObject*>& OutAssetsAndDependencies)
	{
		...
		if (ImportContext.UsdAssetCache)
		{
			...
			for (int32 AssetIndex = 0; AssetIndex < AssetQueue.Num(); ++AssetIndex)
			{
				...
				TSet<UObject*> Dependencies = IMyUsdClassesModule::GetAssetDependencies(Asset);
	...
	void RemapSoftReferences(
		const FMyUsdStageImportContext& ImportContext,
	...
	)
	{
		...
		if (FEngineAnalytics::IsAvailable())
		{
			...
			IMyUsdClassesModule::AddAssetCountAttributes(ImportedAssets, EventAttributes);
	...
	// UE::MyUSDStageImporter::Private::ImportMeshes, because we may want to reuse meshes we already got from the asset cache.
...
void UMyUsdStageImporter::ImportFromFile(FMyUsdStageImportContext& ImportContext)
{
#if USE_USD_SDK
	...
	// We could also GetMutableDefault<UMyUsdStageImportOptions>() instead, but I think in this case it's expected that these
	// should be the default value and not the config
	if (!ImportContext.ImportOptions)
	{
		ImportContext.ImportOptions = NewObject<UMyUsdStageImportOptions>();
	}
	...
	{
		UE::MyUSDStageImporter::Private::OpenStage(ImportContext, bNeedsMasking);
	}
	...
	UE::MyUSDStageImporter::Private::SetupSceneActor(ImportContext);
	...
	FMyUsdDelegates::OnPreUsdImport.Broadcast(ImportContext.FilePath);

	AActor* ExistingSceneActor = UE::MyUSDStageImporter::Private::GetExistingSceneActor(ImportContext);

	UE::MyUSDStageImporter::Private::SetupStageForImport(ImportContext);
	...
	TStrongObjectPtr<UMyUsdAssetCache3> AssetCachePin;
	if (!ImportContext.UsdAssetCache && ImportContext.ImportOptions->bUseExistingAssetCache)
	{
		...
		ImportContext.UsdAssetCache = Cast<UMyUsdAssetCache3>(ImportContext.ImportOptions->ExistingAssetCache.TryLoad());
	}
	if (!ImportContext.UsdAssetCache)
	{
		...
		ImportContext.UsdAssetCache = NewObject<UMyUsdAssetCache3>();
	...
	TStrongObjectPtr<UMyUsdPrimLinkCache> PrimLinkCache{NewObject<UMyUsdPrimLinkCache>(Outer, Name, Flags)};
	TStrongObjectPtr<UMyUsdInfoCache> UsdInfoCache{NewObject<UMyUsdInfoCache>(Outer, Name, Flags)};
	...
	EMyUsdPurpose PurposesToImport = static_cast<EMyUsdPurpose>(ImportContext.ImportOptions->PurposesToImport);
	...
	if (!ImportContext.BBoxCache)
	{
		...
		ImportContext.BBoxCache = MakeShared<UE::FMyUsdGeomBBoxCache>(ImportTime, PurposesToImport, bUseExtentsHint, bIgnoreVisibility);
	}

	TOptional<EMyUsdPurpose> IncludedPurposesToRevertBBoxCacheTo;
	...
	TranslationContext->KindsToCollapse = (EMyUsdDefaultKind)ImportContext.ImportOptions->KindsToCollapse;
	...
	{
		UE::MyUSDStageImporter::Private::CacheCollapsingState(TranslationContext.Get());
		UE::MyUSDStageImporter::Private::ImportMaterials(ImportContext, TranslationContext.Get());
		UE::MyUSDStageImporter::Private::ImportMeshes(ImportContext, TranslationContext.Get());
		UE::MyUSDStageImporter::Private::ImportActors(ImportContext, TranslationContext.Get());
	}
	TranslationContext->CompleteTasks();

	UE::MyUSDStageImporter::Private::CollectUsedAssetDependencies(ImportContext, UsedAssetsAndDependencies);
	UE::MyUSDStageImporter::Private::PruneUnwantedAssets(ImportContext, UsedAssetsAndDependencies, ObjectsToRemap, SoftObjectsToRemap);
	UE::MyUSDStageImporter::Private::UpdateAssetUserData(UsedAssetsAndDependencies, ImportContext.FilePath, ImportContext.ImportOptions);
	UE::MyUSDStageImporter::Private::ResolveActorConflicts(ImportContext, ExistingSceneActor, ObjectsToRemap, SoftObjectsToRemap);
	UE::MyUSDStageImporter::Private::PublishAssets(
		ImportContext,
	...
	UE::MyUSDStageImporter::Private::RemapReferences(ImportContext, PublishedAssets, ObjectsToRemap);
	UE::MyUSDStageImporter::Private::RemapSoftReferences(ImportContext, PublishedAssets, SoftObjectsToRemap);
	UE::MyUSDStageImporter::Private::Cleanup(
		ImportContext.SceneActor,
		ExistingSceneActor,
		ImportContext.ImportOptions->ExistingActorPolicy,
		AssetsToFinalize
	);
	UE::MyUSDStageImporter::Private::PostRemapAssets(PublishedAssets);
	UE::MyUSDStageImporter::Private::BroadcastImportEvents(PublishedAssets);
	UE::MyUSDStageImporter::Private::RefreshComponents(ImportContext.SceneActor, ImportContext.ImportOptions->bImportAtSpecificTimeCode);
	...
	FMyUsdDelegates::OnPostUsdImport.Broadcast(ImportContext.FilePath);

	// Analytics
	{
		double ElapsedSeconds = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - StartTime);
		UE::MyUSDStageImporter::Private::SendAnalytics(ImportContext, nullptr, TEXT("Import"), PublishedAssets, ElapsedSeconds);
	...
	UE::MyUSDStageImporter::Private::CloseStageIfNeeded(ImportContext);
#endif	  // #if USE_USD_SDK
}

bool UMyUsdStageImporter::ReimportSingleAsset(
	FMyUsdStageImportContext& ImportContext,
...
)
{
	...
	TOptional<EMyUsdPurpose> IncludedPurposesToRevertTo;
	if (ImportContext.BBoxCache->GetIncludedPurposes() != PurposesToImport)
	{
	...
	TranslationContext->GroomInterpolationSettings = ImportContext.ImportOptions->GroomInterpolationSettings;
	{
		...
		UE::FMyUsdPrim TargetPrim = ImportContext.Stage.GetPrimAtPath(UE::FSdfPath(*OriginalPrimPath));
		if (TargetPrim)
		{
			UE::MyUSDStageImporter::Private::CreateAssetsForPrims({TargetPrim}, TranslationContext.Get(), LOCTEXT("CreateAssets", "Creating assets"));
		}
	}
	TranslationContext->CompleteTasks();
	...
	{
		UMyUsdAssetUserData* UserData = nullptr;
		if (IInterface_AssetUserData* UserDataInterface = Cast<IInterface_AssetUserData>(Asset))
		{
			UserData = UserDataInterface->GetAssetUserData<UMyUsdAssetUserData>();
		}
	...
	if (ReimportedObject)
	{
		UE::MyUSDStageImporter::Private::UpdateAssetImportData(ReimportedObject, ImportContext.FilePath, ImportContext.ImportOptions);

		// Assign things from the original assets before we publish the reimported asset, overwriting it
		UE::MyUSDStageImporter::Private::CopyOriginalMaterialAssignment(ImportContext, OriginalAsset, ReimportedObject);
		UE::MyUSDStageImporter::Private::CopySkeletonAssignment(ImportContext, OriginalAsset, ReimportedObject);
		...
		OutReimportedAsset = UE::MyUSDStageImporter::Private::PublishAsset(
			ImportContext,
			ReimportedObject,
		...
		UE::MyUSDStageImporter::Private::RemapReferences(ImportContext, PublishedAssetsAndDependencies, ObjectsToRemap);
		UE::MyUSDStageImporter::Private::RemapSoftReferences(ImportContext, PublishedAssetsAndDependencies, SoftObjectsToRemap);
		UE::MyUSDStageImporter::Private::PostRemapAssets(PublishedAssetsAndDependencies);

		bSuccess = OutReimportedAsset != nullptr && ActiveAssets.Contains(ReimportedObject);
	}

	const bool bIsReimport = true;
	UE::MyUSDStageImporter::Private::Cleanup(ImportContext.SceneActor, nullptr, ImportContext.ImportOptions->ExistingActorPolicy, AssetsToFinalize);
	UE::MyUSDStageImporter::Private::BroadcastImportEvents({ReimportedObject}, bIsReimport);
	UE::MyUSDStageImporter::Private::RefreshComponents(ImportContext.SceneActor, ImportContext.ImportOptions->bImportAtSpecificTimeCode);
	...
	{
		double ElapsedSeconds = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - StartTime);
		UE::MyUSDStageImporter::Private::SendAnalytics(ImportContext, ReimportedObject, TEXT("Reimport"), {ReimportedObject}, ElapsedSeconds);
