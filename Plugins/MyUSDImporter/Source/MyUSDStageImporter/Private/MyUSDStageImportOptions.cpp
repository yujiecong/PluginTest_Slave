...
// Deprecated
void UMyUsdStageImportOptions::EnableActorImport(bool bEnable)
{
}

void UMyUsdStageImportOptions::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UsdUtils::AddAnalyticsAttributes(const UMyUsdStageImportOptions& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes)
{
...
UsdUtils::FScopedSuppressActorImport::FScopedSuppressActorImport(UMyUsdStageImportOptions* InOptions)
	: Options{InOptions}
{
	for (TFieldIterator<FProperty> PropertyIterator(UMyUsdStageImportOptions::StaticClass()); PropertyIterator; ++PropertyIterator)
	{
		FProperty* Property = *PropertyIterator;
		if (Property && Property->GetFName() == GET_MEMBER_NAME_CHECKED(UMyUsdStageImportOptions, bImportActors))
		{
	...
	if (UMyUsdStageImportOptions* OptionsPtr = Options.Get())
	{
