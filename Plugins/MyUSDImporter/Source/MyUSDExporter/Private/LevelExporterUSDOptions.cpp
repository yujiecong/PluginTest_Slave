...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LevelExporterUSDOptions)
...
void UsdUtils::AddAnalyticsAttributes(const FLevelExporterUSDOptionsInner& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes)
{
	...
	InOutAttributes.Emplace(TEXT("bExportToNestedSubFolders"), Options.bExportToNestedSubFolders);
...
void UsdUtils::HashForLevelExport(const FLevelExporterUSDOptionsInner& Options, FSHA1& HashToUpdate)
{
	...
	{
		IMyUsdExporterModule::HashEditorSelection(HashToUpdate);
	}
	...
	HashToUpdate.Update(reinterpret_cast<const uint8*>(&Options.bExportToNestedSubFolders), sizeof(Options.bExportToNestedSubFolders));
