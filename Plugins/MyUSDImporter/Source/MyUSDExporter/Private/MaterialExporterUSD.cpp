...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MaterialExporterUSD)
...
namespace UE::MaterialExporterUSD::Private
{
	void SendAnalytics(
		...
		const FMyUsdMaterialBakingOptions& Options,
		bool bAutomated,
	...
	void HashMaterial(const UMaterialInterface& Material, FSHA1& VersionHash)
	{
		if (const UMaterial* Parent = Material.GetMaterial())
		{
			...
			if (const FMaterialResource* MaterialResource = Parent->GetMaterialResource(GMaxRHIShaderPlatform))
			{
				MaterialResource = Parent->GetMaterialResource(GMaxRHIShaderPlatform);
	...
	void HashOptions(const FMyUsdMaterialBakingOptions& Options, FSHA1& VersionHash)
	{
...
bool UMaterialExporterUsd::ExportBinary(UObject* Object, const TCHAR* Type, FArchive& Ar, FFeedbackContext* Warn, int32 FileIndex, uint32 PortFlags)
{
	...
	if (!Options)
	{
		...
		if (Options && (!ExportTask || !ExportTask->bAutomated))
		{
			...
			const bool bContinue = SMyUsdOptionsWindow::ShowExportOptions(*Options);
			if (!bContinue)
			{
				return false;
			}
	...
	{
		const FMaterialResource* MaterialResource = Parent->GetMaterialResource(GMaxRHIShaderPlatform);
...
bool UMaterialExporterUsd::ExportMaterial(
	const UMaterialInterface& Material,
	const FMyUsdMaterialBakingOptions& Options,
	const FMyUsdMetadataExportOptions& MetadataOptions,
...
)
{
	...
	if (!IMyUsdExporterModule::CanExportToLayer(FilePath.FilePath))
	{
		return false;
	}
...
{
	static FMyUsdMetadataExportOptions DefaultOptions;
	return UMaterialExporterUsd::ExportMaterial(
		Material,
		Options,
		DefaultOptions,
