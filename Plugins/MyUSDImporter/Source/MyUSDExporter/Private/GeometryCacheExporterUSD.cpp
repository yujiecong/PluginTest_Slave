...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GeometryCacheExporterUSD)
...
)
{
	...
	if (!IMyUsdExporterModule::CanExportToLayer(UExporter::CurrentFilename)
		|| (Options->MeshAssetOptions.bUsePayload && !IMyUsdExporterModule::CanExportToLayer(PayloadFilename)))
	{
		return false;
	}
