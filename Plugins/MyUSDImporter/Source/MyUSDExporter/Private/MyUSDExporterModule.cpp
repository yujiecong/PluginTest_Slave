...
class FMyUsdExporterModule : public IMyUsdExporterModule
{
public:
	virtual void StartupModule() override
	{
		...
		if (UScriptStruct* ScriptStruct = FMyUsdMeshAssetOptions::StaticStruct())
		{
			if (FProperty* LowestMeshLODProperty = ScriptStruct->FindPropertyByName(GET_MEMBER_NAME_CHECKED(FMyUsdMeshAssetOptions, LowestMeshLOD)))
			{
				LowestMeshLODProperty->SetMetaData(TEXT("ClampMax"), LexToString(MAX_MESH_LOD_COUNT - 1));
			}

			if (FProperty* HighestMeshLODProperty = ScriptStruct->FindPropertyByName(GET_MEMBER_NAME_CHECKED(FMyUsdMeshAssetOptions, HighestMeshLOD)))
			{
...
void IMyUsdExporterModule::HashEditorSelection(FSHA1& HashToUpdate)
{
	if (!GEditor)
	{
		return;
...
bool IMyUsdExporterModule::CanExportToLayer(const FString& TargetFilePath)
{
		...
		if (FPaths::IsSamePath(LoadedLayer.GetRealPath(), TargetFilePath))
		{
			FText ErrorMessage = FText::Format(
				LOCTEXT(
					...
					"Failed to export to the USD layer '{0}' as a layer with this identifier is already open in another USD Stage. Please use a different file path or try closing that other USD stage and exporting again."
				),
				FText::FromString(TargetFilePath)
			);
			USD_LOG_USERERROR(ErrorMessage);
...
IMPLEMENT_MODULE_USD(FMyUsdExporterModule, MyUSDExporter);
