...
struct FLevelExporterUSDOptionsInner
{
	 ...
	 * Warning: This can produce very long file paths that could cause issues depending on the operating system settings.
	 */
	UPROPERTY(EditAnywhere, config, BlueprintReadWrite, Category = "Asset options", AdvancedDisplay)
	bool bExportToNestedSubFolders = false;
	...
	FMyUsdMeshAssetOptions AssetOptions;
 ...
 */
UCLASS(MinimalAPI, Config = Editor, Blueprintable, HideCategories = Hidden)
...
private:
	UFUNCTION()
	static UE_API TArray<FString> GetUsdExtensions();
};

namespace UsdUtils
{
	MYUSDEXPORTER_API void AddAnalyticsAttributes(const FLevelExporterUSDOptionsInner& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes);

	MYUSDEXPORTER_API void AddAnalyticsAttributes(const ULevelExporterUSDOptions& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes);

	MYUSDEXPORTER_API void HashForLevelExport(const FLevelExporterUSDOptionsInner& Options, FSHA1& HashToUpdate);

	MYUSDEXPORTER_API void HashForLevelExport(const ULevelExporterUSDOptions& Options, FSHA1& HashToUpdate);
}
