...
struct FMyUsdMaterialBakingOptions
{
	GENERATED_BODY()
...
struct FMyUsdMeshAssetOptions
{
	...
	UPROPERTY(
		...
		meta = (EditCondition = bUsePayload, GetOptions = "MyUSDExporter.LevelExporterUSDOptions.GetUsdExtensions")
	)
	...
	FMyUsdMaterialBakingOptions MaterialBakingOptions;
	...
	FMyUsdMeshAssetOptions() = default;
	FMyUsdMeshAssetOptions(const FMyUsdMeshAssetOptions&) = default;
	FMyUsdMeshAssetOptions& operator=(const FMyUsdMeshAssetOptions&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
};

namespace UsdUtils
{
	MYUSDEXPORTER_API void AddAnalyticsAttributes(const FMyUsdMaterialBakingOptions& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes);

	MYUSDEXPORTER_API void AddAnalyticsAttributes(const FMyUsdMeshAssetOptions& Options, TArray<FAnalyticsEventAttribute>& InOutAttributes);

	MYUSDEXPORTER_API void HashForMaterialExport(const FMyUsdMaterialBakingOptions& Options, FSHA1& HashToUpdate);

	MYUSDEXPORTER_API void HashForMeshExport(const FMyUsdMeshAssetOptions& Options, FSHA1& HashToUpdate);
}
