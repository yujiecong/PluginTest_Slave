...
#include "MyUsdWrappers/PcpLayerStack.h"
#include "MyUsdWrappers/PcpLayerStackIdentifier.h"
#include "MyUsdWrappers/PcpMapExpression.h"
#include "MyUsdWrappers/PcpNodeRef.h"
#include "MyUsdWrappers/SdfChangeBlock.h"
#include "MyUsdWrappers/SdfLayer.h"
#include "MyUsdWrappers/SdfPayloadsProxy.h"
#include "MyUsdWrappers/SdfPrimSpec.h"
#include "MyUsdWrappers/SdfReferencesProxy.h"
#include "UsdWrappers/MyUsdAttribute.h"
#include "UsdWrappers/MyUsdEditContext.h"
#include "UsdWrappers/MyUsdGeomBBoxCache.h"
#include "UsdWrappers/MyUsdGeomXformable.h"
#include "UsdWrappers/MyUsdPrim.h"
#include "UsdWrappers/MyUsdResolveInfo.h"
...
namespace UE::USDLevelSequenceHelper::Private
{
	...
	{
		FScopedReadOnlyDisable(UMovieScene* InMovieScene, const UE::FSdfLayer& InLayer, const UE::FMyUsdStage& InOwnerStage)
	...
	private:
		...
		const UE::FMyUsdStage OwnerStage;
	};
...
cleanup:
	...
	void ShowVisibilityWarningIfNeeded(const UMovieScenePropertyTrack* PropertyTrack, const UE::FMyUsdPrim& UsdPrim)
	{
		...
		UE::FMyUsdAttribute VisibilityAttr = UsdPrim.GetAttribute(*VisibilityAttrName);
...
class FMyUsdLevelSequenceHelperImpl : private FGCObject
{
public:
	FMyUsdLevelSequenceHelperImpl();
	~FMyUsdLevelSequenceHelperImpl();

	ULevelSequence* Init(const UE::FMyUsdStage& InUsdStage);
	bool Serialize(FArchive& Ar);
	void SetPrimLinkCache(UMyUsdPrimLinkCache* PrimLinkCache);
	void SetBBoxCache(TSharedPtr<UE::FMyUsdGeomBBoxCache> InBBoxCache);
	bool HasData() const;
	void Clear();
...
protected:
	...
	{
		return TEXT("FMyUsdLevelSequenceHelperImpl");
	}
...
public:
	...
	void BindToUsdStageActor(AMyUsdStageActor* InStageActor);
	void UnbindFromUsdStageActor();
	EMyUsdRootMotionHandling GetRootMotionHandling() const;
	void SetRootMotionHandling(EMyUsdRootMotionHandling NewValue);
	...
	FMyUsdLevelSequenceHelper::FOnSkelAnimationBaked& GetOnSkelAnimationBaked()
	{
...
private:
	ULevelSequence* FindOrAddSequenceForAttribute(const UE::FMyUsdAttribute& Attribute, UE::FSdfLayer* OutSequenceLayer = nullptr);
	...
	void RemoveSequenceForPrim(ULevelSequence& Sequence, const UMyUsdPrimTwin& PrimTwin);
...
private:
	...
	void CreateTimeTrack(const FMyUsdLevelSequenceHelperImpl::FLayerTimeInfo& Info);
	void RemoveTimeTrack(const FMyUsdLevelSequenceHelperImpl::FLayerTimeInfo* Info);

	void AddCommonTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim, bool bForceVisibilityTracks = false);
	void AddBoundsTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim, TOptional<bool> HasAnimatedBounds = {});
	void AddCameraTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim);
	void AddLightTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim, const TSet<FName>& PropertyPathsToRead = {});
	void AddSkeletalTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim);
	void AddGroomTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim);
	void AddGeometryCacheTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim);
	void AddVolumeTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim);
	void AddAudioTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim);

	template<typename TrackType>
	TrackType* AddTrack(
		const FName& PropertyPath,
		const UMyUsdPrimTwin& PrimTwin,
		USceneComponent& ComponentToBind,
		ULevelSequence& Sequence,
		bool bIsMuted = false
	);

	void RemovePossessable(const UMyUsdPrimTwin& PrimTwin);
...
public:
	...
	void AddPrim(UMyUsdPrimTwin& PrimTwin, bool bForceVisibilityTracks = false, TOptional<bool> HasAnimatedBounds = {});
	void RemovePrim(const UMyUsdPrimTwin& PrimTwin);

	// These functions assume the skeletal animation tracks (if any) were already added to the level sequence
	void UpdateControlRigTracks(UMyUsdPrimTwin& PrimTwin);
...
private:
	...
	friend FArchive& operator<<(FArchive& Ar, FMyUsdLevelSequenceHelperImpl::FPrimTwinBindings& Bindings)
	{
	...
	TMap<TWeakObjectPtr<const UMyUsdPrimTwin>, FPrimTwinBindings> PrimTwinToBindings;
...
private:
	...
	FGuid GetOrCreateComponentBinding(const UMyUsdPrimTwin& PrimTwin, USceneComponent& ComponentToBind, ULevelSequence& Sequence);
...
private:
	...
	FMyUsdLevelSequenceHelper::FOnSkelAnimationBaked OnSkelAnimationBaked;

	TWeakObjectPtr<AMyUsdStageActor> StageActor = nullptr;

	// We keep a pointer to these directly because we may be called via the
	// MyUSDStageImporter directly, when we don't have an available actor.
	...
	TWeakObjectPtr<UMyUsdPrimLinkCache> PrimLinkCache = nullptr;
	TSharedPtr<UE::FMyUsdGeomBBoxCache> BBoxCache = nullptr;
...
const EObjectFlags FMyUsdLevelSequenceHelperImpl::DefaultObjFlags = EObjectFlags::RF_Transactional | EObjectFlags::RF_Transient
																  | EObjectFlags::RF_Public;
const double FMyUsdLevelSequenceHelperImpl::DefaultFramerate = 24.0;
const TCHAR* FMyUsdLevelSequenceHelperImpl::TimeTrackName = TEXT("Time");
const double FMyUsdLevelSequenceHelperImpl::EmptySubSectionRange = 10.0;

FMyUsdLevelSequenceHelperImpl::FMyUsdLevelSequenceHelperImpl()
...
ULevelSequence* FMyUsdLevelSequenceHelperImpl::Init(const UE::FMyUsdStage& InUsdStage)
{
	UsdStage = InUsdStage;
...
bool FMyUsdLevelSequenceHelperImpl::Serialize(FArchive& Ar)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMyUsdLevelSequenceHelperImpl::Serialize);
...
void FMyUsdLevelSequenceHelperImpl::SetPrimLinkCache(UMyUsdPrimLinkCache* InPrimLinkCache)
{
	// PrimLinkCache.Reset(InPrimLinkCache);
	PrimLinkCache = InPrimLinkCache;
}

void FMyUsdLevelSequenceHelperImpl::SetBBoxCache(TSharedPtr<UE::FMyUsdGeomBBoxCache> InBBoxCache)
{
	BBoxCache = InBBoxCache;
}

bool FMyUsdLevelSequenceHelperImpl::HasData() const
{
...
void FMyUsdLevelSequenceHelperImpl::Clear()
{
	MainLevelSequence = nullptr;
...
void FMyUsdLevelSequenceHelperImpl::AddReferencedObjects(FReferenceCollector& Collector)
{
	Collector.AddReferencedObject(MainLevelSequence);
	Collector.AddReferencedObjects(LevelSequencesByIdentifier);
}

void FMyUsdLevelSequenceHelperImpl::CreateSequencesForLayerStack(const UE::FSdfLayer& RootLayer)
{
	if (!UsdStage)
	{
		return;
...
void FMyUsdLevelSequenceHelperImpl::BindToUsdStageActor(AMyUsdStageActor* InStageActor)
{
	...
	SetRootMotionHandling(InStageActor ? InStageActor->RootMotionHandling : EMyUsdRootMotionHandling::NoAdditionalRootMotion);
	...
	OnUsdObjectsChangedHandle = StageActor->GetUsdListener().GetOnObjectsChanged().AddRaw(this, &FMyUsdLevelSequenceHelperImpl::OnUsdObjectsChanged);
...
void FMyUsdLevelSequenceHelperImpl::UnbindFromUsdStageActor()
{
	if (UsdStage)
	{
	...
	SetRootMotionHandling(EMyUsdRootMotionHandling::NoAdditionalRootMotion);
}

EMyUsdRootMotionHandling FMyUsdLevelSequenceHelperImpl::GetRootMotionHandling() const
{
	return RootMotionHandling;
}

void FMyUsdLevelSequenceHelperImpl::SetRootMotionHandling(EMyUsdRootMotionHandling NewValue)
{
	RootMotionHandling = NewValue;
}

void FMyUsdLevelSequenceHelperImpl::OnStageActorRenamed()
{
...
void FMyUsdLevelSequenceHelperImpl::SubscribeToEditorEvents()
{
#if WITH_EDITOR
	UnsubscribeToEditorEvents();
	...
	if (GEditor)
	{
		OnObjectTransactedHandle = FCoreUObjectDelegates::OnObjectTransacted.AddRaw(this, &FMyUsdLevelSequenceHelperImpl::OnObjectTransacted);

		if (UTransBuffer* Transactor = Cast<UTransBuffer>(GEditor->Trans))
		{
			Transactor->OnTransactionStateChanged().AddRaw(this, &FMyUsdLevelSequenceHelperImpl::HandleTransactionStateChanged);
		}
	}
#endif	  // WITH_EDITOR
}

void FMyUsdLevelSequenceHelperImpl::UnsubscribeToEditorEvents()
{
#if WITH_EDITOR
	if (GEditor)
	{
...
void FMyUsdLevelSequenceHelperImpl::DirtySequenceHierarchyCache()
{
	bSequenceHierarchyCacheIsDirty = true;
}

const FMovieSceneSequenceTransform& FMyUsdLevelSequenceHelperImpl::GetSubsequenceTransform(const ULevelSequence* Sequence)
{
...
ULevelSequence* FMyUsdLevelSequenceHelperImpl::FindOrAddSequenceForAttribute(const UE::FMyUsdAttribute& Attribute, UE::FSdfLayer* OutSequenceLayer)
{
	using namespace UE;
	...
	FMyUsdResolveInfo ResolveInfo = Attribute.GetResolveInfo(UsdUtils::GetEarliestTimeCode());
...
ULevelSequence* FMyUsdLevelSequenceHelperImpl::FindOrAddSequenceForLayer(const UE::FSdfLayer& Layer, bool* bOutCreatedNew)
{
	...
	if (!Sequence)
	{
		// We only get a PrimLinkCache when importing (from UMyUsdStageImporter::ImportFromFile) or when BindToUsdStageActor is called, which also gives us
		...
		Sequence = NewObject<ULevelSequence>(GetTransientPackage(), UniqueSequenceName, FMyUsdLevelSequenceHelperImpl::DefaultObjFlags);
		Sequence->Initialize();
...
ULevelSequence* FMyUsdLevelSequenceHelperImpl::CreateSequenceAndParentsRecursively(const UE::FPcpNodeRef& CurrentNode, const UE::FSdfLayer& CurrentLayer)
{
	using namespace UE;
...
UE::FSdfLayer FMyUsdLevelSequenceHelperImpl::FindEditTargetForSubsequence(ULevelSequence* Sequence)
{
	FString LayerIdentifier = IdentifierByLevelSequence.FindRef(Sequence);
	return UE::FSdfLayer::FindOrOpen(*LayerIdentifier);
}

UMovieSceneSubSection* FMyUsdLevelSequenceHelperImpl::FindSubSequenceSection(ULevelSequence& Sequence, ULevelSequence& SubSequence)
{
	UMovieScene* MovieScene = Sequence.GetMovieScene();
	if (!MovieScene)
	{
...
void FMyUsdLevelSequenceHelperImpl::CreateSubSequenceSection(ULevelSequence& Sequence, ULevelSequence& SubSequence, const UE::FSdfLayerOffset& SubLayerOffset)
{
...
void FMyUsdLevelSequenceHelperImpl::RemoveSubSequenceSection(ULevelSequence& Sequence, ULevelSequence& SubSequence)
{
...
void FMyUsdLevelSequenceHelperImpl::UpdateSubSectionTimeRanges(bool bShowResizedSectionToast)
{
...
void FMyUsdLevelSequenceHelperImpl::CreateTimeTrack(const FLayerTimeInfo& Info)
{
	...
	UMovieSceneFloatTrack* TimeTrack = MovieScene->FindTrack<UMovieSceneFloatTrack>(
		StageActorBinding,
		FName(FMyUsdLevelSequenceHelperImpl::TimeTrackName)
	);
	if (TimeTrack)
	{
		TimeTrack->Modify();
	...
	else
	{
		...
		TimeTrack->SetPropertyNameAndPath(FName(FMyUsdLevelSequenceHelperImpl::TimeTrackName), "Time");
...
void FMyUsdLevelSequenceHelperImpl::RemoveTimeTrack(const FLayerTimeInfo* LayerTimeInfo)
{
...
void FMyUsdLevelSequenceHelperImpl::AddCommonTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim, bool bForceVisibilityTracks)
{
	...
	UE::FMyUsdGeomXformable Xformable(Prim);
	...
	{
		UE::FMyUsdPrim AncestorPrim = Prim.GetParent();
		while (AncestorPrim && !AncestorPrim.IsPseudoRoot())
		{
			if (UE::FMyUsdGeomXformable AncestorXformable{AncestorPrim})
			{
	...
	{
		default:
		case EMyUsdRootMotionHandling::NoAdditionalRootMotion:
		{
			break;
		}
		case EMyUsdRootMotionHandling::UseMotionFromSkelRoot:
		{
		...
		case EMyUsdRootMotionHandling::UseMotionFromSkeleton:
		{
	...
	if (Xformable.TransformMightBeTimeVarying() || bNeedTrackToCompensateResetXformOp || Prim.IsA(TEXT("Gprim")))
	{
		...
		TArray<UE::FMyUsdAttribute> Attrs = UsdUtils::GetAttributesForProperty(Prim, UnrealIdentifiers::TransformPropertyName);
		for (int32 Index = Attrs.Num() - 1; Index >= 0; --Index)
		{
			const UE::FMyUsdAttribute& Attr = Attrs[Index];
		...
		UE::FMyUsdAttribute StrongestAttr = UsdUtils::GetStrongestAttribute(Attrs, UsdUtils::GetEarliestTimeCode());
	...
	TArray<UE::FMyUsdAttribute> Attrs = UsdUtils::GetAttributesForProperty(Prim, UnrealIdentifiers::HiddenInGamePropertyName);
	if (Attrs.Num() > 0)
	{
		UE::FMyUsdAttribute VisibilityAttribute = Attrs[0];
		if (VisibilityAttribute)
		{
			...
			UE::FMyUsdAttribute FirstAnimatedVisibilityParentAttr;
			...
			if (bForceVisibilityTracks)
			{
				...
				while (ParentPrim && !ParentPrim.IsPseudoRoot())
				{
					...
					{
						TArray<UE::FMyUsdAttribute> ParentAttrs = UsdUtils::GetAttributesForProperty(
							ParentPrim,
							UnrealIdentifiers::HiddenInGamePropertyName
						);
						if (ParentAttrs.Num() > 0)
						{
							UE::FMyUsdAttribute ParentVisAttr = ParentAttrs[0];
			...
			UE::FMyUsdAttribute AttributeForSequence;
...
void FMyUsdLevelSequenceHelperImpl::AddBoundsTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim, TOptional<bool> HasAnimatedBounds)
{
	...
	{
		UE::FMyUsdAttribute ExtentAttr = Prim.GetAttribute(TEXT("extent"));
	...
	if (!TargetSequence)
	{
		...
		{
			UE::FMyUsdAttribute ExtentsHintAttr = Prim.GetAttribute(TEXT("extentsHint"));
...
void FMyUsdLevelSequenceHelperImpl::AddCameraTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim)
{
	...
	{
		TArray<UE::FMyUsdAttribute> Attrs = UsdUtils::GetAttributesForProperty(Prim, PropertyName);
		if (Attrs.Num() < 1)
		{
			continue;
		}
		...
		const UE::FMyUsdAttribute& Attr = Attrs[0];
...
void FMyUsdLevelSequenceHelperImpl::AddLightTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim, const TSet<FName>& PropertyPathsToRead)
{
	...
	for (const TPair<FName, ETrackType>& Pair : PropertyPathToTrackType)
	{
		...
		TArray<UE::FMyUsdAttribute> Attrs = UsdUtils::GetAttributesForProperty(Prim, PropertyPath);
		if (Attrs.Num() < 1)
		{
			continue;
		}
		...
		UE::FMyUsdAttribute MainAttr = Attrs[0];
		...
		{
			UE::FMyUsdAttribute& Attr = Attrs[AttrIndex];
		...
		if (Attrs.Num() == 0 || !UE::FMyUsdAttribute::GetUnionedTimeSamples(Attrs, UnionedTimeSamples))
		{
			continue;
		}
...
void FMyUsdLevelSequenceHelperImpl::AddSkeletalTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim)
{
	...
	UMyUsdPrimLinkCache* PrimLinkCachePtr = PrimLinkCache.Get();
	if (!PrimLinkCachePtr)
	{
		return;
	}

	UE::FMyUsdPrim SkelRootPrim = UsdUtils::GetClosestParentSkelRoot(Prim);
	...
	UE::FMyUsdPrim SkelAnimationPrim = UsdUtils::FindAnimationSource(SkelRootPrim, Prim);
	if (!SkelAnimationPrim)
	{
		return;
	}

	// Fetch the UAnimSequence asset from the asset cache. Ideally we'd call AMyUsdStageActor::GetGeneratedAssets,
	// but we may belong to a FMyUsdStageImportContext, and so there's no AMyUsdStageActor at all to use.
	...
	UE::FMyUsdAttribute TranslationsAttr = SkelAnimationPrim.GetAttribute(TEXT("translations"));
	UE::FMyUsdAttribute RotationsAttr = SkelAnimationPrim.GetAttribute(TEXT("rotations"));
	UE::FMyUsdAttribute ScalesAttr = SkelAnimationPrim.GetAttribute(TEXT("scales"));
	UE::FMyUsdAttribute BlendShapeWeightsAttr = SkelAnimationPrim.GetAttribute(TEXT("blendShapeWeights"));

	UE::FMyUsdAttribute StrongestAttribute = UsdUtils::GetStrongestAttribute(
	...
	{
		UE::FMyUsdResolveInfo Info = StrongestAttribute.GetResolveInfo(UsdUtils::GetEarliestTimeCode());
		...
		if (UMyUsdAnimSequenceAssetUserData* UserData = Sequence->GetAssetUserData<UMyUsdAnimSequenceAssetUserData>())
		{
...
void FMyUsdLevelSequenceHelperImpl::AddGeometryCacheTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim)
{
	...
	TArray<UE::FMyUsdAttribute> AnimatedMeshAttributes = UsdUtils::GetAnimatedMeshAttributes(Prim);
	UE::FMyUsdAttribute StrongestAttribute = UsdUtils::GetStrongestAttribute(AnimatedMeshAttributes, UsdUtils::GetEarliestTimeCode());
...
void FMyUsdLevelSequenceHelperImpl::AddGroomTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim)
{
	...
	TArray<UE::FMyUsdAttribute> AnimatedAttributes = UsdUtils::GetAnimatedAttributes(Prim);
	UE::FMyUsdAttribute StrongestAttribute = UsdUtils::GetStrongestAttribute(AnimatedAttributes, UsdUtils::GetEarliestTimeCode());
...
void FMyUsdLevelSequenceHelperImpl::AddVolumeTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim)
{
	using namespace UnrealIdentifiers;
	...
	UMyUsdSparseVolumeTextureAssetUserData* UserData = nullptr;
	...
	if (CurrentMaterial)
	{
		...
		if (ParameterInfo.Num() > 0)
		{
			...
			if (CurrentMaterial->GetSparseVolumeTextureParameterValue(Info, SparseVolumeTexture) && SparseVolumeTexture)
			{
				...
				{
					UserData = Cast<UMyUsdSparseVolumeTextureAssetUserData>(UsdUnreal::ObjectUtils::GetAssetUserData(SparseVolumeTexture));
				}
			}
		}
	}
	...
	UE::FMyUsdPrim PrimForOffsetCalculation = Prim;
	if (UserData->SourceOpenVDBAssetPrimPaths.Num() > 0)
	{
		const FString& FirstAssetPrimPath = UserData->SourceOpenVDBAssetPrimPaths[0];
		UE::FMyUsdPrim FirstAssetPrim = Prim.GetStage().GetPrimAtPath(UE::FSdfPath{*FirstAssetPrimPath});
		if (FirstAssetPrim)
		{
			PrimForOffsetCalculation = FirstAssetPrim;
		}
	}
	UE::FSdfLayerOffset CombinedOffset = UsdUtils::GetPrimToStageOffset(UE::FMyUsdPrim{PrimForOffsetCalculation});
	...
	{
		// Reference: FMyUsdVolVolumeTranslator::UpdateComponents
...
{
	template<typename T>
	TOptional<T> GetAuthoredValue(const UE::FMyUsdPrim& Prim, const FString& AttrName)
	{
		if (UE::FMyUsdAttribute Attr = Prim.GetAttribute(*AttrName))
		{
			UE::FVtValue VtValue;
...
void FMyUsdLevelSequenceHelperImpl::AddAudioTracks(const UMyUsdPrimTwin& PrimTwin, const UE::FMyUsdPrim& Prim)
{
	using namespace UnrealIdentifiers;
	...
	// See big comment within FMyUsdMediaSpatialAudioTranslator::UpdateComponents
	...
	// Set up the actual audio track
	{
		...
		UE::FMyUsdAttribute Attr = Prim.GetAttribute(*FilePathToken);
		...
		// values as retrieved by UE::FMyUsdAttribute::Get() are *already relative to the stage*.
				...
				UMovieSceneAudioTrack>(TEXT("Audio track"), PrimTwin, *AudioComponent, *AttributeSequence, bIsMuted))
		{
			...
			if (NewAudioSection)
			{
				...
				UE::FMyUsdAttribute GainAttr = Prim.GetAttribute(*GainToken);
...
void FMyUsdLevelSequenceHelperImpl::AddPrim(UMyUsdPrimTwin& PrimTwin, bool bForceVisibilityTracks, TOptional<bool> HasAnimatedBounds)
{
	if (!UsdStage)
	{
		return;
	...
	UE::FMyUsdPrim UsdPrim{UsdStage.GetPrimAtPath(PrimPath)};
	if (!UsdPrim)
	{
		return;
	}
...
template<typename TrackType>
TrackType* FMyUsdLevelSequenceHelperImpl::AddTrack(
...
void FMyUsdLevelSequenceHelperImpl::RemovePrim(const UMyUsdPrimTwin& PrimTwin)
{
	if (!UsdStage)
	{
		return;
...
void FMyUsdLevelSequenceHelperImpl::UpdateControlRigTracks(UMyUsdPrimTwin& PrimTwin)
{
#if WITH_EDITOR
	if (!UsdStage)
	{
	...
	UE::FSdfPath PrimPath{*PrimTwin.PrimPath};
	UE::FMyUsdPrim SkeletonPrim{UsdStage.GetPrimAtPath(PrimPath)};
	if (!SkeletonPrim)
	{
		return;
	}
	...
	UE::FMyUsdPrim SkelRootPrim = UsdUtils::GetClosestParentSkelRoot(SkeletonPrim);
	UE::FMyUsdPrim SkelAnimationPrim = UsdUtils::FindAnimationSource(SkelRootPrim, SkeletonPrim);
	...
	UE::FMyUsdEditContext EditContext{UsdStage, SkelAnimationLayer};
	...
	UE::FMyUsdAttribute JointsAttr = SkelAnimationPrim.CreateAttribute(TEXT("joints"), TEXT("token[]"));
	UE::FMyUsdAttribute TranslationsAttr = SkelAnimationPrim.CreateAttribute(TEXT("translations"), TEXT("float3[]"));
	UE::FMyUsdAttribute RotationsAttr = SkelAnimationPrim.CreateAttribute(TEXT("rotations"), TEXT("quatf[]"));
	UE::FMyUsdAttribute ScalesAttr = SkelAnimationPrim.CreateAttribute(TEXT("scales"), TEXT("half3[]"));
	UE::FMyUsdAttribute BlendShapeWeightsAttr = SkelAnimationPrim.GetAttribute(TEXT("blendShapeWeights"));
	...
	if (!ControlRigTrack)
	{
		...
		if (UE::FMyUsdAttribute Attr = PrimWithSchema.GetAttribute(*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigReduceKeys)))
		{
			UE::FVtValue Value;
		...
		if (UE::FMyUsdAttribute Attr = PrimWithSchema.GetAttribute(*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigReductionTolerance)))
		{
			UE::FVtValue Value;
			if (Attr.Get(Value) && !Value.IsEmpty())
			{
		...
		if (UE::FMyUsdAttribute Attr = PrimWithSchema.GetAttribute(*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealUseFKControlRig)))
		{
			UE::FVtValue Value;
			if (Attr.Get(Value))
			{
		...
		else
		{
			...
			if (UE::FMyUsdAttribute Attr = PrimWithSchema.GetAttribute(*UsdToUnreal::ConvertToken(UnrealIdentifiers::UnrealControlRigPath)))
			{
				UE::FVtValue Value;
				if (Attr.Get(Value) && !Value.IsEmpty())
				{
...
void FMyUsdLevelSequenceHelperImpl::RemoveSequenceForPrim(ULevelSequence& Sequence, const UMyUsdPrimTwin& PrimTwin)
{
...
void FMyUsdLevelSequenceHelperImpl::RemovePossessable(const UMyUsdPrimTwin& PrimTwin)
{
...
void FMyUsdLevelSequenceHelperImpl::RefreshSequencer()
{
#if WITH_EDITOR
...
void FMyUsdLevelSequenceHelperImpl::UpdateUsdLayerOffsetFromSection(const UMovieSceneSequence* Sequence, const UMovieSceneSubSection* Section)
{
	if (!Section || !Sequence)
	{
		return;
...
void FMyUsdLevelSequenceHelperImpl::UpdateMovieSceneTimeRanges(UMovieScene& MovieScene, const FLayerTimeInfo& LayerTimeInfo, bool bUpdateViewRanges)
{
...
void FMyUsdLevelSequenceHelperImpl::BlockMonitoringChangesForThisTransaction()
{
...
void FMyUsdLevelSequenceHelperImpl::OnObjectTransacted(UObject* Object, const class FTransactionObjectEvent& Event)
{
...
void FMyUsdLevelSequenceHelperImpl::OnUsdObjectsChanged(
...
)
{
	...
	UE::FMyUsdStage ActiveStage = StageActorPtr->GetUsdStage();
	if (!ActiveStage)
	{
		return;
	}
...
void FMyUsdLevelSequenceHelperImpl::HandleTransactionStateChanged(
...
double FMyUsdLevelSequenceHelperImpl::GetFramesPerSecond() const
{
	if (!UsdStage)
	{
...
double FMyUsdLevelSequenceHelperImpl::GetTimeCodesPerSecond() const
{
	if (!UsdStage)
	{
		return DefaultFramerate;
...
FGuid FMyUsdLevelSequenceHelperImpl::GetOrCreateComponentBinding(
	const UMyUsdPrimTwin& PrimTwin,
	USceneComponent& ComponentToBind,
	ULevelSequence& Sequence
)
...
void FMyUsdLevelSequenceHelperImpl::HandleMovieSceneChange(UMovieScene& MovieScene)
{
	using namespace UE::USDLevelSequenceHelper::Private;
	...
	auto RemoveTimeSamplesForAttr = [](const UE::FMyUsdAttribute& Attr)
	{
		if (!Attr || Attr.GetNumTimeSamples() == 0)
		{
			return;
	...
	auto RemoveTimeSamplesForPropertyIfNeeded =
		[&MovieScene, &RemoveTimeSamplesForAttr](const UE::FMyUsdPrim& Prim, const FGuid& Guid, const FName& PropertyPath)
	{
		if (!UE::USDLevelSequenceHelper::Private::FindTrackTypeOrDerived<UMovieScenePropertyTrack>(&MovieScene, Guid, PropertyPath))
		{
			for (UE::FMyUsdAttribute& Attr : UsdUtils::GetAttributesForProperty(Prim, PropertyPath))
			{
	...
	for (TMap<TWeakObjectPtr<const UMyUsdPrimTwin>, FPrimTwinBindings>::TIterator PrimTwinIt = PrimTwinToBindings.CreateIterator(); PrimTwinIt;
		 ++PrimTwinIt)
	{
		const UMyUsdPrimTwin* UsdPrimTwin = PrimTwinIt->Key.Get();
			 ...
			 ++BindingIt)
		{
			...
			if (UsdPrimTwin)
			{
				...
				if (UE::FMyUsdPrim UsdPrim = UsdStage.GetPrimAtPath(UE::FSdfPath(*UsdPrimTwin->PrimPath)))
				{
					...
					{
						for (UE::FMyUsdAttribute& Attr : UsdUtils::GetAttributesForProperty(UsdPrim, UnrealIdentifiers::HiddenInGamePropertyName))
						{
							RemoveTimeSamplesForAttr(Attr);
						}
					}
					...
					else if (bIsSkeletal)
					{
						...
						{
							UE::FMyUsdPrim SkelRootPrim = UsdUtils::GetClosestParentSkelRoot(UsdPrim);
							UE::FMyUsdPrim SkelAnimationPrim = UsdUtils::FindAnimationSource(SkelRootPrim, UsdPrim);
							if (SkelAnimationPrim)
							{
...
void FMyUsdLevelSequenceHelperImpl::HandleSubSectionChange(UMovieSceneSubSection& Section)
{
...
void FMyUsdLevelSequenceHelperImpl::HandleControlRigSectionChange(UMovieSceneControlRigParameterSection& Section)
{
#if WITH_EDITOR
	AMyUsdStageActor* StageActorValue = StageActor.Get();
	if (!StageActorValue)
	{
		return;
	}
	...
	UMyUsdPrimTwin* PrimTwin = StageActorValue->RootUsdTwin->Find(BoundComponent);
	if (!PrimTwin)
	{
		return;
	}

	UE::FMyUsdPrim SkeletonPrim = UsdStage.GetPrimAtPath(UE::FSdfPath(*PrimTwin->PrimPath));
	if (!SkeletonPrim)
	{
		return;
	}
	...
	UE::FMyUsdEditContext EditContext{UsdStage, EditTargetLayer};
...
void FMyUsdLevelSequenceHelperImpl::HandleTrackChange(UMovieSceneTrack& Track, bool bIsMuteChange)
{
	if (!StageActor.IsValid())
	{
		return;
	...
	{
		if (PropertyPath == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, Time))
		{
	...
	UMyUsdPrimTwin* PrimTwin = StageActor->RootUsdTwin->Find(BoundSceneComponent);
	...
	{
		FScopedBlockNoticeListening BlockNotices(StageActor.Get());
		UE::FMyUsdPrim UsdPrim = UsdStage.GetPrimAtPath(UE::FSdfPath(*PrimTwin->PrimPath));
		...
		if (bIsMuteChange)
		{
			if (PropertyTrack)
			{
				TArray<UE::FMyUsdAttribute> Attrs = UsdUtils::GetAttributesForProperty(UsdPrim, PropertyPath);
				if (Attrs.Num() > 0)
				{
					...
					UE::FMyUsdAttribute& Attr = Attrs[0];
			...
			else if (const UMovieSceneSkeletalAnimationTrack* SkeletalTrack = Cast<const UMovieSceneSkeletalAnimationTrack>(&Track))
			{
				...
				UE::FMyUsdPrim SkelRootPrim = UsdUtils::GetClosestParentSkelRoot(UsdPrim);
				if (UE::FMyUsdPrim SkelAnimationPrim = UsdUtils::FindAnimationSource(SkelRootPrim, UsdPrim))
				{
					UE::FMyUsdAttribute TranslationsAttr = SkelAnimationPrim.GetAttribute(TEXT("translations"));
					UE::FMyUsdAttribute RotationsAttr = SkelAnimationPrim.GetAttribute(TEXT("rotations"));
					UE::FMyUsdAttribute ScalesAttr = SkelAnimationPrim.GetAttribute(TEXT("scales"));
		...
		else
		{
			...
			if (PropertyTrack)
			{
				...
				else if (Writer.TwoVectorWriter)
				{
					...
					if (Track.GetTrackName() == GET_MEMBER_NAME_CHECKED(UMyUsdDrawModeComponent, BoundsMin))
					{
...
FMyUsdLevelSequenceHelperImpl::FLayerTimeInfo& FMyUsdLevelSequenceHelperImpl::FindOrAddLayerTimeInfo(const UE::FSdfLayer& Layer)
{
...
FMyUsdLevelSequenceHelperImpl::FLayerTimeInfo* FMyUsdLevelSequenceHelperImpl::FindLayerTimeInfo(const UE::FSdfLayer& Layer)
{
	const FString Identifier = Layer.GetIdentifier();
	return LayerTimeInfosByLayerIdentifier.Find(Identifier);
}

void FMyUsdLevelSequenceHelperImpl::UpdateLayerTimeInfoFromLayer(FLayerTimeInfo& LayerTimeInfo, const UE::FSdfLayer& Layer)
{
	if (!Layer)
	{
		return;
...
ULevelSequence* FMyUsdLevelSequenceHelperImpl::FindSequenceForIdentifier(const FString& SequenceIdentitifer) const
{
	ULevelSequence* Sequence = nullptr;
...
public:
	...
	EMyUsdRootMotionHandling GetRootMotionHandling() const
	{
		return EMyUsdRootMotionHandling::NoAdditionalRootMotion;
	}
	void SetRootMotionHandling(EMyUsdRootMotionHandling NewValue) {};
	void OnStageActorRenamed() {};

	void AddPrim(UMyUsdPrimTwin& PrimTwin, bool bForceVisibilityTracks, TOptional<bool> HasAnimatedBounds)
	{
	}
	void RemovePrim(const UMyUsdPrimTwin& PrimTwin)
	{
...
FMyUsdLevelSequenceHelper::FMyUsdLevelSequenceHelper()
{
	UsdSequencerImpl = MakeUnique<FMyUsdLevelSequenceHelperImpl>();
}

FMyUsdLevelSequenceHelper::~FMyUsdLevelSequenceHelper() = default;

FMyUsdLevelSequenceHelper::FMyUsdLevelSequenceHelper(const FMyUsdLevelSequenceHelper& Other)
	: FMyUsdLevelSequenceHelper()
{
}

FMyUsdLevelSequenceHelper& FMyUsdLevelSequenceHelper::operator=(const FMyUsdLevelSequenceHelper& Other)
{
...
FMyUsdLevelSequenceHelper::FMyUsdLevelSequenceHelper(FMyUsdLevelSequenceHelper&& Other) = default;
FMyUsdLevelSequenceHelper& FMyUsdLevelSequenceHelper::operator=(FMyUsdLevelSequenceHelper&& Other) = default;

ULevelSequence* FMyUsdLevelSequenceHelper::Init(const UE::FMyUsdStage& UsdStage)
{
...
bool FMyUsdLevelSequenceHelper::Serialize(FArchive& Ar)
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::OnStageActorRenamed()
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::SetInfoCache(TSharedPtr<FMyUsdInfoCache> InInfoCache)
{
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void FMyUsdLevelSequenceHelper::SetPrimLinkCache(UMyUsdPrimLinkCache* PrimLinkCache)
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::SetBBoxCache(TSharedPtr<UE::FMyUsdGeomBBoxCache> InBBoxCache)
{
	if (UsdSequencerImpl.IsValid())
	{
...
bool FMyUsdLevelSequenceHelper::HasData() const
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::Clear()
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::BindToUsdStageActor(AMyUsdStageActor* StageActor)
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::UnbindFromUsdStageActor()
{
	if (UsdSequencerImpl.IsValid())
	{
...
EMyUsdRootMotionHandling FMyUsdLevelSequenceHelper::GetRootMotionHandling() const
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::SetRootMotionHandling(EMyUsdRootMotionHandling NewValue)
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::AddPrim(UMyUsdPrimTwin& PrimTwin, bool bForceVisibilityTracks, TOptional<bool> HasAnimatedBounds)
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::RemovePrim(const UMyUsdPrimTwin& PrimTwin)
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::UpdateControlRigTracks(UMyUsdPrimTwin& PrimTwin)
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::StartMonitoringChanges()
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::StopMonitoringChanges()
{
	if (UsdSequencerImpl.IsValid())
	{
...
void FMyUsdLevelSequenceHelper::BlockMonitoringChangesForThisTransaction()
{
	if (UsdSequencerImpl.IsValid())
	{
...
ULevelSequence* FMyUsdLevelSequenceHelper::GetMainLevelSequence() const
{
	if (UsdSequencerImpl.IsValid())
	{
...
TArray<ULevelSequence*> FMyUsdLevelSequenceHelper::GetSubSequences() const
{
	if (UsdSequencerImpl.IsValid())
	{
...
FMyUsdLevelSequenceHelper::FOnSkelAnimationBaked& FMyUsdLevelSequenceHelper::GetOnSkelAnimationBaked()
{
#if USE_USD_SDK
	if (UsdSequencerImpl.IsValid())
	{
...
FScopedBlockMonitoringChangesForTransaction::FScopedBlockMonitoringChangesForTransaction(FMyUsdLevelSequenceHelper& InHelper)
	: FScopedBlockMonitoringChangesForTransaction(*InHelper.UsdSequencerImpl.Get())
{
}

FScopedBlockMonitoringChangesForTransaction::FScopedBlockMonitoringChangesForTransaction(FMyUsdLevelSequenceHelperImpl& InHelperImpl)
