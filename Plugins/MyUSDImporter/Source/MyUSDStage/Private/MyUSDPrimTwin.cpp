...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UMyUsdPrimTwin::AddChild);
	...
	TObjectPtr<UMyUsdPrimTwin>& ChildPrim = Children.Add(ChildPrimName);

	// Needs public because this will mostly live on the transient package (c.f. AMyUsdStageActor::GetRootPrimTwin())
	ChildPrim = NewObject<UMyUsdPrimTwin>(this, NAME_None, GetFlags() | RF_Public);
...
void UMyUsdPrimTwin::RemoveChild(const TCHAR* InPrimPath)
{
	FScopedUnrealAllocs UnrealAllocs;

	Modify();

	for (TMap<FString, TObjectPtr<UMyUsdPrimTwin>>::TIterator ChildIt = Children.CreateIterator(); ChildIt; ++ChildIt)
	{
...
void UMyUsdPrimTwin::Clear()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UMyUsdPrimTwin::Clear);

	FScopedUnrealAllocs UnrealAllocs;

	Modify();

	for (const TPair<FString, TObjectPtr<UMyUsdPrimTwin>>& Pair : Children)
	{
	...
	if (ActorToDestroy && !ActorToDestroy->IsA<AMyUsdStageActor>() && !ActorToDestroy->IsActorBeingDestroyed() && ActorToDestroy->GetWorld())
	{
...
UMyUsdPrimTwin* UMyUsdPrimTwin::Find(const FString& InPrimPath)
{
...
UMyUsdPrimTwin* UMyUsdPrimTwin::Find(const USceneComponent* InSceneComponent)
{
	...
	for (const TPair<FString, TObjectPtr<UMyUsdPrimTwin>>& Child : Children)
	{
		UMyUsdPrimTwin* FoundPrimTwin = Child.Value->Find(InSceneComponent);
		if (FoundPrimTwin)
		{
...
USceneComponent* UMyUsdPrimTwin::GetSceneComponent() const
{
...
void UMyUsdPrimTwin::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
	...
	// AMyUsdStageActor::OnObjectsReplaced)
