...
class FMyUsdStageModule : public IMyUsdStageModule
{
public:
	virtual void StartupModule() override
	{
		...
		UolEditorModule.RegisterLocatorEditor("UsdPrim", MakeShared<FMyUsdPrimLocatorEditor>());
		...
		PropertyModule.RegisterCustomClassLayout(
			TEXT("UsdStageActor"),
			FOnGetDetailCustomizationInstance::CreateStatic(&FMyUsdStageActorCustomization::MakeInstance)
		);
	...
	virtual void ShutdownModule() override
	{
		...
		{
			UolModule->UnregisterFragmentType(FMyUsdPrimLocatorFragment::FragmentType);
		}
	...
	virtual AMyUsdStageActor& GetUsdStageActor(UWorld* World) override
	{
		if (AMyUsdStageActor* UsdStageActor = FindUsdStageActor(World))
		{
			return *UsdStageActor;
		}
		else
		{
			return *(World->SpawnActor<AMyUsdStageActor>());
		}
	}

	virtual AMyUsdStageActor* FindUsdStageActor(UWorld* World) override
	{
		for (FActorIterator ActorIterator(World); ActorIterator; ++ActorIterator)
		{
			if (AMyUsdStageActor* UsdStageActor = Cast<AMyUsdStageActor>(*ActorIterator))
			{
				return UsdStageActor;
			}
		}
...
IMPLEMENT_MODULE_USD(FMyUsdStageModule, MyUSDStage);
