...
#include "MyUSDTransactor.h"
#include "MyUSDTypesConversion.h"

#include "MyUsdWrappers/SdfLayer.h"
#include "UsdWrappers/MyUsdGeomBBoxCache.h"
#include "UsdWrappers/MyUsdGeomXformable.h"
#include "UsdWrappers/MyUsdRelationship.h"
...
#define LOCTEXT_NAMESPACE "MyUSDStageActor"
...
AMyUsdStageActor::FOnActorLoaded AMyUsdStageActor::OnActorLoaded;
AMyUsdStageActor::FOnOpenStageEditorClicked AMyUsdStageActor::OnOpenStageEditorClicked;

struct FMyUsdStageActorImpl
{
	static TSharedRef<FMyUsdSchemaTranslationContext> CreateUsdSchemaTranslationContext(AMyUsdStageActor* StageActor, const FString& PrimPath)
	{
		TSharedRef<FMyUsdSchemaTranslationContext> TranslationContext = MakeShared<FMyUsdSchemaTranslationContext>(
		...
		TranslationContext->PurposesToLoad = (EMyUsdPurpose)StageActor->PurposesToLoad;
		...
		// Its more convenient to toggle between variants using the MyUSDStage window, as opposed to parsing LODs
		...
		TranslationContext->KindsToCollapse = (EMyUsdDefaultKind)StageActor->KindsToCollapse;
		...
		UMyUsdPrimTwin* ParentUsdPrimTwin = StageActor->GetRootPrimTwin()->Find(UsdPrimPath.GetParentPath().GetString());
	...
	static void DeselectActorsAndComponents(AMyUsdStageActor* StageActor)
	{
#if WITH_EDITOR
		...
		if (GEditor && !IsGarbageCollecting() && StageActor->RootUsdTwin && !StageActor->RootUsdTwin->HasAnyFlags(RF_BeginDestroyed))
		{
			...
			StageActor->GetRootPrimTwin()->Iterate(
				[&ActorsToDeselect, &ComponentsToDeselect](UMyUsdPrimTwin& PrimTwin)
				{
	...
	static void DiscardStage(const UE::FMyUsdStage& Stage, AMyUsdStageActor* DiscardingActor)
	{
		...
		AMyUsdStageActor::StaticClass()->GetDefaultObject()->GetArchetypeInstances(Instances);
		for (UObject* Instance : Instances)
		{
			...
			if (const AMyUsdStageActor* Actor = Cast<const AMyUsdStageActor>(Instance))
			{
				const UE::FMyUsdStage& OtherStage = Actor->GetUsdStage();
	...
	static bool ObjectNeedsMultiUserTag(UObject* Object, AMyUsdStageActor* StageActor)
	{
	 ...
	 */
	static void MakeVisible(UMyUsdPrimTwin& UsdPrimTwin, const UE::FMyUsdStage& Stage)
	{
		// Find the highest invisible prim parent: Nothing above this can possibly change with what we're doing
		UMyUsdPrimTwin* Iter = &UsdPrimTwin;
		UMyUsdPrimTwin* HighestInvisibleParent = nullptr;
		while (Iter)
		{
		...
		UE::FMyUsdPrim Prim = Stage.GetPrimAtPath(UE::FSdfPath(*UsdPrimTwin.PrimPath));
		if (!Prim)
		{
			return;
		}
		...
		TFunction<void(UMyUsdPrimTwin&, bool)> RecursiveResyncVisibility;
		RecursiveResyncVisibility = [&Stage, &RecursiveResyncVisibility](UMyUsdPrimTwin& PrimTwin, bool bPrimHasInvisibleParent)
		{
			...
			UE::FMyUsdPrim CurrentPrim = Stage.GetPrimAtPath(UE::FSdfPath(*PrimTwin.PrimPath));
			if (!CurrentPrim)
			{
				return;
			}
			...
			for (const TPair<FString, TObjectPtr<UMyUsdPrimTwin>>& ChildPair : PrimTwin.GetChildren())
			{
				if (UMyUsdPrimTwin* ChildTwin = ChildPair.Value)
				{
	 ...
	 * Note that bHiddenInGame corresponds to computed visibility, and the component tags correspond to individual prim-level visibilities
	 */
	static void MakeInvisible(UMyUsdPrimTwin& UsdPrimTwin)
	{
	...
	)
	{
		...
		if (FEngineAnalytics::IsAvailable())
		{
			...
			IMyUsdClassesModule::AddAssetCountAttributes(ActiveAssets, EventAttributes);

			const bool bAutomated = false;
			IMyUsdClassesModule::SendAnalytics(MoveTemp(EventAttributes), TEXT("Open"), bAutomated, ElapsedSeconds, NumberOfFrames, Extension);
		}
	}
	...
	{
#if WITH_EDITOR
		IMyUsdStageModule& UsdStageModule = FModuleManager::Get().LoadModuleChecked<IMyUsdStageModule>(TEXT("UsdStage"));
	...
	static void ShowTransformOnCameraComponentWarning(const UActorComponent* Component)
	{
		...
		if (Settings && Settings->bShowTransformOnCameraComponentWarning)
		{
			...
			Toast.CheckBoxStateChanged = FOnCheckStateChanged::CreateStatic(
				...
				{
					if (UMyUsdProjectSettings* Settings = GetMutableDefault<UMyUsdProjectSettings>())
					{
						// This is flipped because the default checkbox message is "Don't prompt again"
	...
	static void WriteOutAssetMetadataChange(
		const AMyUsdStageActor* StageActor,
		const UMyUsdAssetUserData* ChangedUserData,
	...
	)
	{
		...
		const FMyUsdCombinedPrimMetadata* StageMetadata = ChangedUserData->StageIdentifierToMetadata.Find(StageIdentifier);
		...
		for (const TPair<FString, FMyUsdPrimMetadata>& PrimPathToMetadata : StageMetadata->PrimPathToMetadata)
		{
			const FString& PrimPath = PrimPathToMetadata.Key;
			const FMyUsdPrimMetadata& PrimMetadata = PrimPathToMetadata.Value;

			const UE::FMyUsdPrim& Prim = Stage.GetPrimAtPath(UE::FSdfPath{*PrimPath});
			...
			if (bChangeWasRemoval)
			{
				...
				if (bHasMetadataFilters)
				{
					...
					FMyUsdCombinedPrimMetadata TempInvertedMetadata;
	...
	static TSet<FString> GetPointInstancerPrototypes(const UE::FMyUsdPrim& Prim)
	{
		TSet<FString> PrototypePaths;

#if USE_USD_SDK
		static FString PrototypesStr = UsdToUnreal::ConvertToken(pxr::UsdGeomTokens->prototypes);
		if (UE::FMyUsdRelationship Relationship = Prim.GetRelationship(*PrototypesStr))
		{
...
/**
 * Class that helps us know when a blueprint that derives from AMyUsdStageActor is being compiled.
...
struct FRecompilationTracker
{
	...
	static void OnCompilationStarted(UBlueprint* BP)
	{
		...
		// level
		if (!BP || BP->bIsRegeneratingOnLoad || !BP->GeneratedClass || !BP->GeneratedClass->IsChildOf(AMyUsdStageActor::StaticClass())
...
AMyUsdStageActor::AMyUsdStageActor()
	: StageState(EMyUsdStageState::OpenedAndLoaded)
	, InitialLoadSet(EMyUsdInitialLoadSet::LoadAll)
	, InterpolationType(EMyUsdInterpolationType::Linear)
	...
	, PurposesToLoad((int32)EMyUsdPurpose::Proxy)
	...
	, RootMotionHandling(EMyUsdRootMotionHandling::NoAdditionalRootMotion)
	, FallbackCollisionType(EMyUsdCollisionType::ConvexHull)
	, SubdivisionLevel(0)
	...
	, bIsUndoRedoing(false)
{
	...
	const FName RootTwinName = MakeUniqueObjectName(this, UMyUsdPrimTwin::StaticClass(), TEXT("RootUsdTwin"));
	const FName InfoCacheName = MakeUniqueObjectName(this, UMyUsdInfoCache::StaticClass(), TEXT("InfoCache"));
	const FName PrimLinkCacheName = MakeUniqueObjectName(this, UMyUsdPrimLinkCache::StaticClass(), TEXT("LinkCache"));
	const FName TransactorName = MakeUniqueObjectName(this, UMyUsdTransactor::StaticClass(), TEXT("Transactor"));
	const bool bTransient = true;

	RootUsdTwin = CreateDefaultSubobject<UMyUsdPrimTwin>(RootTwinName, bTransient);
	UsdInfoCache = CreateDefaultSubobject<UMyUsdInfoCache>(InfoCacheName, bTransient);
	PrimLinkCache = CreateDefaultSubobject<UMyUsdPrimLinkCache>(PrimLinkCacheName, bTransient);
	Transactor = CreateDefaultSubobject<UMyUsdTransactor>(TransactorName, bTransient);
	...
	if (!IsTemplate())
	{
		...
		for (TFieldIterator<FProperty> PropertyIterator(AMyUsdStageActor::StaticClass()); PropertyIterator; ++PropertyIterator)
		{
			FProperty* Property = *PropertyIterator;
			if (Property && Property->GetFName() == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, RootLayer))
			{
		...
		FEditorDelegates::BeginPIE.AddUObject(this, &AMyUsdStageActor::OnBeginPIE);
		FEditorDelegates::PostPIEStarted.AddUObject(this, &AMyUsdStageActor::OnPostPIEStarted);

		FMyUsdDelegates::OnPostUsdImport.AddUObject(this, &AMyUsdStageActor::OnPostUsdImport);
		FMyUsdDelegates::OnPreUsdImport.AddUObject(this, &AMyUsdStageActor::OnPreUsdImport);

		GEngine->OnLevelActorDeleted().AddUObject(this, &AMyUsdStageActor::OnLevelActorDeleted);
		...
		{
			TransBuffer->OnTransactionStateChanged().AddUObject(this, &AMyUsdStageActor::HandleTransactionStateChanged);
			...
			OnRedoHandle = TransBuffer->OnRedo().AddLambda(
				[this](const FTransactionContext& TransactionContext, bool bSucceeded)
				{
						...
						&& !RootLayer.FilePath.IsEmpty())
					{
						...
						// Note that now our UMyUsdTransactor may have already caused the stage itself to be loaded, but we may still need to call
						// LoadUsdStage on our end.
						else if (ObjectsToWatch.Num() == 0 && (!AssetCache || AssetCache->GetNumAssets() == 0))
						{
							this->LoadUsdStage();
							AMyUsdStageActor::OnActorLoaded.Broadcast(this);
						}
					}
				}
			);
		...
		// Note that we subscribe to some additional events from PostInitProperties()

		LevelSequenceHelper.GetOnSkelAnimationBaked().AddUObject(this, &AMyUsdStageActor::OnSkelAnimationBaked);

#endif	  // WITH_EDITOR

		OnTimeChanged.AddUObject(this, &AMyUsdStageActor::AnimatePrims);

		UsdListener.GetOnObjectsChanged().AddUObject(this, &AMyUsdStageActor::OnUsdObjectsChanged);

		UsdListener.GetOnSdfLayersChanged().AddLambda(
			[&, this](const UsdUtils::FLayerToSdfChangeList& LayersToChangeList)
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::OnSdfLayersChanged);
				...
				const UE::FMyUsdStage& Stage = GetUsdStage();
				if (!Stage)
				{
					return;
				}
...
void AMyUsdStageActor::NewStage()
{
#if USE_USD_SDK
	UE::FMyUsdStage NewStage = UnrealUSDWrapper::NewStage();
	if (!NewStage)
	{
		return;
	}
	...
	UE::FMyUsdPrim RootPrim = NewStage.DefinePrim(UE::FSdfPath{TEXT("/Root")}, TEXT("Xform"));
	ensure(UsdUtils::SetDefaultKind(RootPrim, EMyUsdDefaultKind::Assembly));
	...
	if (StageState == EMyUsdStageState::Closed)
	{
...
void AMyUsdStageActor::SetIsolatedRootLayer(const FString& IsolatedStageRootLayer)
{
...
FString AMyUsdStageActor::GetIsolatedRootLayer() const
{
	return IsolatedStage ? IsolatedStage.GetRootLayer().GetIdentifier() : FString{};
}

void AMyUsdStageActor::IsolateLayer(const UE::FSdfLayer& Layer, bool bLoadUsdStage)
{
	...
	TGuardValue<EMyUsdSaveDialogBehavior> DisableDialogGuard{Settings->ShowSaveLayersDialogWhenClosing, EMyUsdSaveDialogBehavior::NeverSave};

	OnPreStageChanged.Broadcast();

	UE::FMyUsdStage NewIsolatedStage;
	UE::FMyUsdStage StageToListenTo;

	// Stop isolating
	if (!Layer || Layer == UsdStage.GetRootLayer())
	{
		NewIsolatedStage = UE::FMyUsdStage{};
	...
	else if (UsdStage)
	{
		...
		UE::FMyUsdStage FreshCurrentStage;
		if (!Layer.IsAnonymous() && !ValidLayers.Contains(Layer))
		{
			...
			FreshCurrentStage = UnrealUSDWrapper::OpenMaskedStage(*UsdStage.GetRootLayer().GetIdentifier(), EMyUsdInitialLoadSet::LoadNone, {});
		...
		NewIsolatedStage = UnrealUSDWrapper::OpenStage(Layer, {}, EMyUsdInitialLoadSet::LoadAll, bUseStageCache);
...
void AMyUsdStageActor::OnUsdObjectsChanged(const UsdUtils::FObjectChangesByPath& InfoChanges, const UsdUtils::FObjectChangesByPath& ResyncChanges)
{
#if USE_USD_SDK
	...
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::OnUsdObjectsChanged);
	...
	if (StageState == EMyUsdStageState::Opened)
	{
	...
	else if (StageState == EMyUsdStageState::Closed)
	{
...
void AMyUsdStageActor::HandleAccumulatedNotices()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::HandleAccumulatedNotices);
	...
	const UE::FMyUsdStage& Stage = GetOrOpenUsdStage();
	if (!Stage)
	{
		return;
	}
	...
	// and the UE::FMyUsdStage will still be opened and valid (even though we intend on closing/changing it when we transact). It could be
	...
	// have the new value
	{
		const UE::FMyUsdStage& BaseStage = GetBaseUsdStage();
		...
		if (!FMyUsdStageActorImpl::DoesPathPointToLayer(RootLayer.FilePath, StageRoot))
		{
			return;
		}
	}
	...
	UMyUsdAssetCache3::FMyUsdScopedReferencer ScopedReferencer{AssetCache, this};
		...
		for (const TPair<FString, TArray<UsdUtils::FSdfChangeListEntry>>& InfoChange : AccumulatedInfoChanges)
		{
			...
			for (const UsdUtils::FSdfChangeListEntry& ObjectChange : InfoChange.Value)
			{
				...
				if (ObjectChange.Flags.bDidChangeRelationshipTargets)
				{
					...
					UE::FMyUsdPrim ChangedPrim = Stage.GetPrimAtPath(PrimPath);
	...
	FScopedSlowTask RefreshStageTask(4, LOCTEXT("ProcessingMyUSDStageUpdates", "Processing USD Stage updates"));
	...
	if (UsdInfoCache && bHasResync)
	{
		...
		TSharedRef<FMyUsdSchemaTranslationContext> TranslationContext = FMyUsdStageActorImpl::CreateUsdSchemaTranslationContext(this, TEXT("/"));
	...
	if (bHasResync)
	{
		FMyUsdStageActorImpl::DeselectActorsAndComponents(this);
	}
	...
	FScopedSlowTask CleanUpComponentsTask(PrimsToUpdate.Num(), LOCTEXT("CleaningUpComponents", "Cleaning up actors and components"));
	{
		...
		for (const TPair<UE::FSdfPath, EPrimUpdateType>& PrimChangedInfo : PrimsToUpdate)
		{
			...
			{
				if (UMyUsdPrimTwin* UsdPrimTwin = GetRootPrimTwin()->Find(PrimPath.GetString()))
				{
					UsdPrimTwin->Clear();
				}
			}
	...
	FScopedSlowTask RegenerateAssetsTask(PrimsToUpdate.Num(), LOCTEXT("RegeneratingAssets", "Regenerating assets"));
	{
		...
		for (const TPair<UE::FSdfPath, EPrimUpdateType>& PrimChangedInfo : PrimsToUpdate)
		{
			...
			UE::FMyUsdPrim PrimToUpdate = Stage.GetPrimAtPath(PrimPath);
			...
			TSharedRef<FMyUsdSchemaTranslationContext> TranslationContext = FMyUsdStageActorImpl::CreateUsdSchemaTranslationContext(
				this,
				PrimPath.GetString()
			);
	...
	if (bHasResync)
	{
		FMyUsdStageActorImpl::RepairExternalSequencerBindings();
	}
...
MYUSDSTAGE_API void AMyUsdStageActor::Reset()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::Reset);
...
void AMyUsdStageActor::StopListeningToUsdNotices()
{
	IsBlockedFromUsdNotices.Increment();
}

void AMyUsdStageActor::ResumeListeningToUsdNotices()
{
	IsBlockedFromUsdNotices.Decrement();
}

bool AMyUsdStageActor::IsListeningToUsdNotices() const
{
	return IsBlockedFromUsdNotices.GetValue() == 0;
}

void AMyUsdStageActor::StopMonitoringLevelSequence()
{
	LevelSequenceHelper.StopMonitoringChanges();
}

void AMyUsdStageActor::ResumeMonitoringLevelSequence()
{
	LevelSequenceHelper.StartMonitoringChanges();
}

void AMyUsdStageActor::BlockMonitoringLevelSequenceForThisTransaction()
{
	LevelSequenceHelper.BlockMonitoringChangesForThisTransaction();
}

UMyUsdPrimTwin* AMyUsdStageActor::GetOrCreatePrimTwin(const UE::FSdfPath& UsdPrimPath)
{
	const FString PrimPath = UsdPrimPath.GetString();
	const FString ParentPrimPath = UsdPrimPath.GetParentPath().GetString();

	UMyUsdPrimTwin* RootTwin = GetRootPrimTwin();
	UMyUsdPrimTwin* UsdPrimTwin = RootTwin->Find(PrimPath);
	UMyUsdPrimTwin* ParentUsdPrimTwin = RootTwin->Find(ParentPrimPath);

	const UE::FMyUsdPrim Prim = GetOrOpenUsdStage().GetPrimAtPath(UsdPrimPath);
	...
	if (!UsdPrimTwin)
	{
		...
		UsdPrimTwin->OnDestroyed.AddUObject(this, &AMyUsdStageActor::OnUsdPrimTwinDestroyed);
	}

	return UsdPrimTwin;
}

UMyUsdPrimTwin* AMyUsdStageActor::ExpandPrim(
...
{
	UMyUsdPrimTwin* UsdPrimTwin = nullptr;
#if USE_USD_SDK
	...
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::ExpandPrim);
	...
	{
		if (TSharedPtr<FMyUsdSchemaTranslator> SchemaTranslator = FMyUsdSchemaTranslatorRegistry::Get()
																	.CreateTranslatorForSchema(TranslationContext.AsShared(), UE::FMyUsdTyped(Prim)))
		{
			...
			else
			{
				...
				{
					TSet<FString> PrototypePaths = FMyUsdStageActorImpl::GetPointInstancerPrototypes(Prim);
	...
	if (!bParentHasAnimatedVisibility.IsSet())
	{
		...
		UE::FMyUsdPrim ParentPrim = Prim.GetParent();
	...
	if (bExpandChildren)
	{
		...
		// We can't just have this prim's FMyUsdGeomXformableTranslator::UpdateComponents ->
		...
		const TArray<UE::FMyUsdPrim> PrimChildren = Prim.GetFilteredChildren(bTraverseInstanceProxies);

		for (const UE::FMyUsdPrim& ChildPrim : PrimChildren)
		{
	...
	{
#if WITH_EDITOR
		if (TwinSceneComponent->IsA<UGeometryCacheComponent>() || TwinSceneComponent->IsA<USkyLightComponent>())
		{
		...
		else if (Prim.IsA(TEXT("PointInstancer")))
		{
			...
			for (const TObjectPtr<USceneComponent>& Child : ChildComponents)
			{
				...
				UMyUsdAssetUserData* UserData = UsdUnreal::ObjectUtils::GetAssetUserData(ISMMesh);
				if (!UserData)
				{
					continue;
				}
	...
	if (EMyUsdDrawMode DrawMode = UsdUtils::GetAppliedDrawMode(Prim); DrawMode != EMyUsdDrawMode::Default)
	{
		const bool bUseExtentsHint = true;
		const bool bIgnoreVisibility = false;
		bHasAnimatedBounds = UsdUtils::HasAnimatedBounds(Prim, static_cast<EMyUsdPurpose>(PurposesToLoad), bUseExtentsHint, bIgnoreVisibility);
	...
	{
		UE::FMyUsdPrim PrimWithSchema;
		if (UsdUtils::PrimHasSchema(Prim, UnrealIdentifiers::ControlRigAPI))
		{
			PrimWithSchema = Prim;
		}
		else if (UE::FMyUsdPrim ParentSkelRoot = UsdUtils::GetClosestParentSkelRoot(Prim))
		{
		...
		if (PrimWithSchema)
		{
			...
			// Prevent register/unregister spam when calling FMyUsdGeomXformableTranslator::UpdateComponents later
...
void AMyUsdStageActor::UpdatePrim(const UE::FSdfPath& InUsdPrimPath, bool bResync, FMyUsdSchemaTranslationContext& TranslationContext)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::UpdatePrim);
	...
	if (UsdPrimPath.IsAbsoluteRootOrPrimPath())
	{
		UE::FMyUsdPrim PrimToExpand = GetOrOpenUsdStage().GetPrimAtPath(UsdPrimPath);
		ExpandPrim(PrimToExpand, bResync, TranslationContext);
	}
}

const UE::FMyUsdStage& AMyUsdStageActor::GetUsdStage() const
{
	return IsolatedStage ? IsolatedStage : UsdStage;
}

const UE::FMyUsdStage& AMyUsdStageActor::GetBaseUsdStage() const
{
	return UsdStage;
}

const UE::FMyUsdStage& AMyUsdStageActor::GetIsolatedUsdStage() const
{
	return IsolatedStage;
}

void AMyUsdStageActor::SetUsdStage(const UE::FMyUsdStage& NewStage)
{
...
UE::FMyUsdStage& AMyUsdStageActor::GetOrLoadUsdStage()
{
	return GetOrOpenUsdStage();
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

UE::FMyUsdStage& AMyUsdStageActor::GetOrOpenUsdStage()
{
	OpenUsdStage();
...
void AMyUsdStageActor::SetRootLayer(const FString& RootFilePath)
{
	...
	if (UsdStage)
	{
		...
		if (StageRootLayer)
		{
			if (FMyUsdStageActorImpl::DoesPathPointToLayer(RelativeFilePath, StageRootLayer))
			{
				return;
			}
		}
...
void AMyUsdStageActor::SetStageState(EMyUsdStageState NewState)
{
	...
	else if (StageState == EMyUsdStageState::Opened)
	{
		UnloadUsdStage();
		OpenUsdStage();
	}
	else if (StageState == EMyUsdStageState::OpenedAndLoaded)
	{
		// Don't call OpenUsdStage directly so that we can abort opening the stage in case the user cancels
		// out of the missing asset cache dialog
		const bool bOpenIfNeeded = true;
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void AMyUsdStageActor::SetAssetCache(UMyUsdAssetCache2* NewCache)
{
	...
	if (!UsdAssetCache && UsdStage)
	{
		...
		UsdAssetCache = NewObject<UMyUsdAssetCache2>(GetTransientPackage(), NAME_None, GetMaskedFlags(RF_PropagateToSubObjects));
	}
...
void AMyUsdStageActor::SetUsdAssetCache(UMyUsdAssetCache3* NewCache)
{
...
void AMyUsdStageActor::SetInitialLoadSet(EMyUsdInitialLoadSet NewLoadSet)
{
...
void AMyUsdStageActor::SetInterpolationType(EMyUsdInterpolationType NewType)
{
...
void AMyUsdStageActor::SetGeometryCacheImport(EGeometryCacheImport ImportOption)
{
...
void AMyUsdStageActor::SetUsePrimKindsForCollapsing(bool bUse)
{
...
void AMyUsdStageActor::SetKindsToCollapse(int32 NewKindsToCollapse)
{
	const EMyUsdDefaultKind NewEnum = (EMyUsdDefaultKind)NewKindsToCollapse;
	EMyUsdDefaultKind Result = NewEnum;

	// If we're collapsing all 'model's, then we must collapse all of its derived kinds
	if (EnumHasAnyFlags(NewEnum, EMyUsdDefaultKind::Model))
	{
		Result |= (EMyUsdDefaultKind::Component | EMyUsdDefaultKind::Group | EMyUsdDefaultKind::Assembly);
	}

	// If we're collapsing all 'group's, then we must collapse all of its derived kinds
	if (EnumHasAnyFlags(NewEnum, EMyUsdDefaultKind::Group))
	{
		Result |= (EMyUsdDefaultKind::Assembly);
	}
...
void AMyUsdStageActor::SetMergeIdenticalMaterialSlots(bool bMerge)
{
...
void AMyUsdStageActor::SetShareAssetsForIdenticalPrims(bool bShare)
{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void AMyUsdStageActor::SetReuseIdenticalAssets(bool bReuse)
{
...
void AMyUsdStageActor::SetCollapseTopLevelPointInstancers(bool bCollapse)
{
}

void AMyUsdStageActor::SetPurposesToLoad(int32 NewPurposesToLoad)
{
	...
	{
		BBoxCache->SetIncludedPurposes(static_cast<EMyUsdPurpose>(PurposesToLoad));
	}
	LoadUsdStage();
}

void AMyUsdStageActor::SetNaniteTriangleThreshold(int32 NewNaniteTriangleThreshold)
{
...
void AMyUsdStageActor::SetRenderContext(const FName& NewRenderContext)
{
...
void AMyUsdStageActor::SetMaterialPurpose(const FName& NewMaterialPurpose)
{
...
void AMyUsdStageActor::SetRootMotionHandling(EMyUsdRootMotionHandling NewHandlingStrategy)
{
...
void AMyUsdStageActor::SetFallbackCollisionType(EMyUsdCollisionType NewCollisionType)
{
...
void AMyUsdStageActor::SetSubdivisionLevel(int32 NewSubdivisionLevel)
{
...
void AMyUsdStageActor::SetCollectMetadata(bool bNewCollectValue)
{
...
void AMyUsdStageActor::SetCollectFromEntireSubtrees(bool bNewCollectValue)
{
...
void AMyUsdStageActor::SetCollectOnComponents(bool bNewCollectValue)
{
...
void AMyUsdStageActor::SetBlockedPrefixFilters(const TArray<FString>& NewFilters)
{
...
void AMyUsdStageActor::SetInvertFilters(bool bNewInvertValue)
{
...
float AMyUsdStageActor::GetTime() const
{
	return Time;
}

void AMyUsdStageActor::SetTime(float InTime)
{
	if (InTime == Time)
	{
		return;
...
ULevelSequence* AMyUsdStageActor::GetLevelSequence() const
{
	return LevelSequence;
}

USceneComponent* AMyUsdStageActor::GetGeneratedComponent(const FString& PrimPath) const
{
	const UE::FMyUsdStage& CurrentStage = static_cast<const AMyUsdStageActor*>(this)->GetUsdStage();
	if (!CurrentStage || StageState != EMyUsdStageState::OpenedAndLoaded)
	{
		return nullptr;
	}
	...
	if (UMyUsdPrimTwin* UsdPrimTwin = GetRootPrimTwin()->Find(UncollapsedPath))
	{
...
TArray<UObject*> AMyUsdStageActor::GetGeneratedAssets(const FString& PrimPath) const
{
	const UE::FMyUsdStage& CurrentStage = static_cast<const AMyUsdStageActor*>(this)->GetUsdStage();
	if (!CurrentStage || StageState != EMyUsdStageState::OpenedAndLoaded)
	{
...
FString AMyUsdStageActor::GetSourcePrimPath(const UObject* Object) const
{
	UMyUsdPrimTwin* RootUsdPrimTwin = GetRootPrimTwin();
	...
	{
		if (UMyUsdPrimTwin* UsdPrimTwin = RootUsdPrimTwin->Find(Component))
		{
...
void AMyUsdStageActor::OpenUsdStage()
{
	// Early exit if stage is already opened, or if we shouldn't be opening anything anyway
	if (UsdStage || RootLayer.FilePath.IsEmpty() || StageState == EMyUsdStageState::Closed)
	{
		return;
	}

	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::OpenUsdStage);
	...
	{
		// The RootLayer property is marked as RelativeToGameDir, and FMyUsdStageViewModel::OpenStage will also give us paths relative to the project's
		// directory
...
void AMyUsdStageActor::CloseUsdStage(bool bUnloadIfNeeded)
{
	TOptional<TGuardValue<ITransaction*>> SuppressTransaction;
	if (GDiscardUndoBufferOnStageOpenClose)
	{
	...
	FMyUsdStageActorImpl::DiscardStage(UsdStage, this);
	UsdStage = UE::FMyUsdStage();
	IsolatedStage = UE::FMyUsdStage();			  // We don't keep our isolated stages on the stage cache
	LevelSequenceHelper.Init(UE::FMyUsdStage());	  // Drop the helper's reference to the stage
...
#if WITH_EDITOR
void AMyUsdStageActor::OnBeginPIE(bool bIsSimulating)
{
...
void AMyUsdStageActor::OnPostPIEStarted(bool bIsSimulating)
{
	...
	// StartTimeCode state. We have to do this here (after duplicating) as we need the calls to FMyUsdSkelSkeletonTranslator::UpdateComponents
...
void AMyUsdStageActor::OnObjectsReplaced(const TMap<UObject*, UObject*>& ObjectReplacementMap)
{
	...
	if (AMyUsdStageActor* NewActor = Cast<AMyUsdStageActor>(ObjectReplacementMap.FindRef(this)))
	{
...
void AMyUsdStageActor::OnLevelActorDeleted(AActor* DeletedActor)
{
	...
	if (RootLayer.FilePath == OldRootLayer.FilePath && FMyUsdStageActorImpl::ObjectNeedsMultiUserTag(DeletedActor, this))
	{
		// DeletedActor is already detached from our hierarchy, so we must tag it directly
		TSet<UObject*> VisitedObjects;
		FMyUsdStageActorImpl::AllowListComponentHierarchy(DeletedActor->GetRootComponent(), VisitedObjects);
	}
}

#endif	  // WITH_EDITOR

void AMyUsdStageActor::LoadUsdStage(bool bOpenIfNeeded)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::LoadUsdStage);

	// We should always have an asset cache when opening a stage now, as IMyUsdClassesModule::GetAssetCacheForProject
	// should never fail
	...
	if (StageState != EMyUsdStageState::OpenedAndLoaded)
	{
		return;
	}
	...
	// the movie scene and tracks will all trigger OnObjectTransacted. We listen for those on FMyUsdLevelSequenceHelperImpl::OnObjectTransacted,
	...
	UE::FMyUsdStage StageToLoad = GetUsdStage();
	...
	TSharedRef<FMyUsdSchemaTranslationContext> TranslationContext = FMyUsdStageActorImpl::CreateUsdSchemaTranslationContext(this, RootTwin->PrimPath);
	...
	if (StageToLoad.GetRootLayer() && IsActorInitialized())
	{
		...
		if (UBlueprintGeneratedClass* BPClass = Cast<UBlueprintGeneratedClass>(GetClass()))
		{
			...
			if (World && World->WorldType == EWorldType::EditorPreview)
			{
				...
				// AMyUsdStageActor's CDO
				if (AMyUsdStageActor* CDO = Cast<AMyUsdStageActor>(GetClass()->GetDefaultObject()))
				{
					CDO->SetTime(GetTime());
				}
			}
	...
	FMyUsdStageActorImpl::SendAnalytics(
		this,
		ElapsedSeconds,
...
void AMyUsdStageActor::UnloadUsdStage()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::UnloadUsdStage);
...
void AMyUsdStageActor::SetupAssetCacheIfNeeded()
{
	if (!AssetCache)
	{
		...
		AssetCache = IMyUsdClassesModule::GetAssetCacheForProject();
	}
}

void AMyUsdStageActor::SetupBBoxCacheIfNeeded()
{
	if (BBoxCache.IsValid())
	{
		return;
	...
	const bool bUseExtentsHint = true;
	const bool bIgnoreVisibility = false;
	BBoxCache = MakeShared<UE::FMyUsdGeomBBoxCache>(Time, static_cast<EMyUsdPurpose>(PurposesToLoad), bUseExtentsHint, bIgnoreVisibility);
}

void AMyUsdStageActor::RebuildInfoCacheFromStoredChanges()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::RebuildInfoCacheFromStoredChanges);
...
UMyUsdPrimTwin* AMyUsdStageActor::GetRootPrimTwin()
{
	return RootUsdTwin;
}

UMyUsdPrimTwin* AMyUsdStageActor::GetRootPrimTwin() const
{
	return RootUsdTwin;
}

void AMyUsdStageActor::Refresh() const
{
	OnTimeChanged.Broadcast();
}

PRAGMA_DISABLE_DEPRECATION_WARNINGS
void AMyUsdStageActor::ReloadAnimations()
{
	RegenerateLevelSequence();
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

bool AMyUsdStageActor::RegenerateLevelSequence()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::RegenerateLevelSequence);
	...
	const UE::FMyUsdStage& CurrentStage = GetOrOpenUsdStage();
	if (!CurrentStage)
	{
		return false;
	}
...
void AMyUsdStageActor::RepopulateLevelSequence()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::RepopulateLevelSequence);
...
void AMyUsdStageActor::OpenLevelSequence()
{
#if WITH_EDITOR
	if (!LevelSequence)
	{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
TSharedPtr<FMyUsdInfoCache> AMyUsdStageActor::GetInfoCache()
{
	return InfoCache;
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

TSharedPtr<UE::FMyUsdGeomBBoxCache> AMyUsdStageActor::GetBBoxCache()
{
	return BBoxCache;
}

TMap<FString, TMap<FString, int32>> AMyUsdStageActor::GetMaterialToPrimvarToUVIndex()
{
	return MaterialToPrimvarToUVIndex;
}

const UsdUtils::FBlendShapeMap& AMyUsdStageActor::GetBlendShapeMap()
{
	return BlendShapesByPath;
}

FMyUsdListener& AMyUsdStageActor::GetUsdListener()
{
	return UsdListener;
}

const FMyUsdListener& AMyUsdStageActor::GetUsdListener() const
{
	return UsdListener;
}

#if WITH_EDITOR
void AMyUsdStageActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void AMyUsdStageActor::PostTransacted(const FTransactionObjectEvent& TransactionEvent)
{
	...
	if (TransactionEvent.GetEventType() == ETransactionObjectEventType::UndoRedo)
	{
		...
		if (ChangedProperties.Contains(GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, RootLayer)))
		{
			...
			if (StageState != EMyUsdStageState::Closed && UsdInfoCache)
			{
				TSharedRef<FMyUsdSchemaTranslationContext> TranslationContext = FMyUsdStageActorImpl::CreateUsdSchemaTranslationContext(this, TEXT("/"));
				UsdInfoCache->GetInner().RebuildCacheForSubtrees({UE::FSdfPath::AbsoluteRootPath()}, TranslationContext.Get());
			}
		...
		else if (ChangedProperties.Contains(GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, StageState)))
		{
		...
		else if (ChangedProperties.Contains(GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, Time)))
		{
			Refresh();
...
void AMyUsdStageActor::PreEditChange(FProperty* PropertyThatWillChange)
{
...
void AMyUsdStageActor::PreEditUndo()
{
	bIsUndoRedoing = true;
...
void AMyUsdStageActor::HandleTransactionStateChanged(
...
)
{
		...
		&& (InTransactionState == ETransactionStateEventType::UndoRedoStarted || InTransactionState == ETransactionStateEventType::UndoRedoFinalized))
	{
		...
		{
			static TMap<AMyUsdStageActor*, TArray<TWeakPtr<ISequencer>>> ActorsToSequencers;
			...
			if (const FTransaction* Transaction = Trans->GetTransaction(CurrentTransactionIndex))
			{
				...
				if (bTransactionContainsLevelSequence && bTransactionContainsThis)
				{
					...
					{
						if (IMyUsdStageModule* UsdStageModule = FModuleManager::Get().GetModulePtr<IMyUsdStageModule>(TEXT("UsdStage")))
						{
...
void AMyUsdStageActor::PostDuplicate(bool bDuplicateForPIE)
{
...
void AMyUsdStageActor::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
void AMyUsdStageActor::Destroyed()
{
	...
	// on AMyUsdStageActor::Reset (called from PostTransacted), we would Modify() these actors, but if their first modify is in
...
void AMyUsdStageActor::PostActorCreated()
{
	Super::PostActorCreated();
}

void AMyUsdStageActor::PostRename(UObject* OldOuter, const FName OldName)
{
...
void AMyUsdStageActor::BeginDestroy()
{
#if WITH_EDITOR
	if (!IsEngineExitRequested() && HasAuthorityOverStage())
	{
		...
		FMyUsdDelegates::OnPostUsdImport.RemoveAll(this);
		FMyUsdDelegates::OnPreUsdImport.RemoveAll(this);
		if (UTransBuffer* TransBuffer = GUnrealEd ? Cast<UTransBuffer>(GUnrealEd->Trans) : nullptr)
		{
	...
	// This clears the SMyUSDStage window whenever the level we're currently in gets destroyed.
...
void AMyUsdStageActor::PostInitProperties()
{
	Super::PostInitProperties();

#if WITH_EDITOR
	// Don't register FCoreUObjectDelegates from the constructor as that can lead to a race condition when async loading (see UE-294491)
	if (!IsTemplate())
	{
		FCoreUObjectDelegates::OnObjectPropertyChanged.RemoveAll(this);
		FCoreUObjectDelegates::OnObjectPropertyChanged.AddUObject(this, &AMyUsdStageActor::OnObjectPropertyChanged);
		...
		if (GIsEditor && GEditor)
		{
			if (UBlueprintGeneratedClass* BPClass = Cast<UBlueprintGeneratedClass>(GetClass()))
			{
				...
				FCoreUObjectDelegates::OnObjectsReplaced.RemoveAll(this);
				FCoreUObjectDelegates::OnObjectsReplaced.AddUObject(this, &AMyUsdStageActor::OnObjectsReplaced);
			}
		}
	}
#endif	  // WITH_EDITOR
...
void AMyUsdStageActor::PostRegisterAllComponents()
{
	...
	// Send this before we load the stage so that we know SMyUSDStage is synced to a potential OnStageChanged broadcast
	OnActorLoaded.Broadcast(this);

	LoadUsdStage();
}

void AMyUsdStageActor::UnregisterAllComponents(bool bForReregister)
{
...
void AMyUsdStageActor::PostUnregisterAllComponents()
{
	Super::PostUnregisterAllComponents();
}

void AMyUsdStageActor::OnPreUsdImport(FString FilePath)
{
	const UE::FMyUsdStage& CurrentStage = static_cast<const AMyUsdStageActor*>(this)->GetUsdStage();
...
void AMyUsdStageActor::OnPostUsdImport(FString FilePath)
{
	const UE::FMyUsdStage& CurrentStage = static_cast<const AMyUsdStageActor*>(this)->GetUsdStage();
	if (!CurrentStage || !HasAuthorityOverStage())
	{
...
void AMyUsdStageActor::UpdateSpawnedObjectsTransientFlag(bool bTransient)
{
	if (!RootUsdTwin)
	{
		return;
	...
	TFunction<void(UMyUsdPrimTwin&)> UpdateTransient = [=](UMyUsdPrimTwin& PrimTwin)
	{
...
void AMyUsdStageActor::RequestDelayedTransactorReset()
{
#if WITH_EDITOR
	...
	TWeakObjectPtr<AMyUsdStageActor> WeakThis{this};
	ExecuteOnGameThread(
		UE_SOURCE_LOCATION,
		[WeakThis]()
		{
			if (AMyUsdStageActor* Actor = WeakThis.Get())
			{
...
void AMyUsdStageActor::OnUsdPrimTwinDestroyed(const UMyUsdPrimTwin& UsdPrimTwin)
{
	PrimsToAnimate.Remove(UsdPrimTwin.PrimPath);
...
void AMyUsdStageActor::OnObjectPropertyChanged(UObject* ObjectBeingModified, FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (RootLayer.FilePath == OldRootLayer.FilePath && FMyUsdStageActorImpl::ObjectNeedsMultiUserTag(ObjectBeingModified, this))
	{
		TSet<UObject*> VisitedObjects;
		FMyUsdStageActorImpl::AllowListComponentHierarchy(GetRootComponent(), VisitedObjects);
	}
	...
	if (UMyUsdAssetUserData* UserData = Cast<UMyUsdAssetUserData>(ObjectBeingModified))
	{
		if (AssetCache->IsAssetTrackedByCache(ObjectBeingModified->GetOuter()->GetPathName()))
		{
			...
			FMyUsdStageActorImpl::WriteOutAssetMetadataChange(this, UserData, PropertyChangedEvent);
			return;
		}
	}
		...
		for (const TWeakPtr<ISequencer>& ExistingSequencer : UsdStageModule.GetExistingSequencers())
		{
			if (TSharedPtr<ISequencer> PinnedSequencer = ExistingSequencer.Pin())
			{
				if (UMovieSceneSequence* RootSequence = PinnedSequencer->GetRootMovieSceneSequence())
				{
					...
					FMyUsdStageActorImpl::GetDescendantMovieSceneSequences(RootSequence, AllSequences);
					...
					for (UMovieSceneSequence* Sequence : AllSequences)
					{
						...
						if (bIsRootComponent)
						{
							...
							// suppress a root component animation in case the property is just mirrored on the actor,
	...
	{
		FMyUsdStageActorImpl::ShowTransformOnCameraComponentWarning(ComponentBeingModified);
		return;
	}
	...
	UMyUsdPrimTwin* UsdPrimTwin = GetRootPrimTwin()->Find(PrimPath);
	if (UsdPrimTwin)
	{
	...
	if (PrimSceneComponent && CurrentStage)
	{
		...
		// independently saved to the transaction via the UMyUsdTransactor, which would be duplication
		FScopedBlockNoticeListening BlockNotices(this);

		UE::FMyUsdPrim UsdPrim = CurrentStage.GetPrimAtPath(UE::FSdfPath(*PrimPath));
		...
		if (UsdPrimTwin && PropertyChangedEvent.GetPropertyName() == UnrealIdentifiers::HiddenInGamePropertyName)
		{
			...
			{
				FMyUsdStageActorImpl::MakeInvisible(*UsdPrimTwin);
			}
			else
			{
				FMyUsdStageActorImpl::MakeVisible(*UsdPrimTwin, CurrentStage);
			}
		}
		...
		else
		{
			...
			if (UMyUsdDrawModeComponent* DrawModeComponent = Cast<UMyUsdDrawModeComponent>(PrimSceneComponent))
			{
				const static TSet<FName> BoundsProperties = {
					GET_MEMBER_NAME_CHECKED(UMyUsdDrawModeComponent, BoundsMin),
					GET_MEMBER_NAME_CHECKED(UMyUsdDrawModeComponent, BoundsMax),
				};
...
void AMyUsdStageActor::HandlePropertyChangedEvent(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, RootLayer))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, StageState))
	{
		const EMyUsdStageState CorrectState = StageState;
		StageState = (EMyUsdStageState) !((uint8)StageState);
		SetStageState(CorrectState);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, Time))
	{
		const float CorrectTime = Time;
		Time = Time + 1.0f;
		SetTime(CorrectTime);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, InitialLoadSet))
	{
		const EMyUsdInitialLoadSet CorrectLoadSet = InitialLoadSet;
		InitialLoadSet = (EMyUsdInitialLoadSet) !((uint8)InitialLoadSet);
		SetInitialLoadSet(CorrectLoadSet);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, InterpolationType))
	{
		const EMyUsdInterpolationType CorrectInterpolationType = InterpolationType;
		InterpolationType = (EMyUsdInterpolationType) !((uint8)InterpolationType);
		SetInterpolationType(CorrectInterpolationType);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, GeometryCacheImport))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, bUsePrimKindsForCollapsing))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, KindsToCollapse))
	{
		const int32 CorrectKindsToCollapse = KindsToCollapse;
		KindsToCollapse += 1;
		SetKindsToCollapse(CorrectKindsToCollapse);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, bMergeIdenticalMaterialSlots))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, bShareAssetsForIdenticalPrims))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, PurposesToLoad))
	{
		const int32 CorrectPurposesToLoad = PurposesToLoad;
		PurposesToLoad += 1;
		SetPurposesToLoad(CorrectPurposesToLoad);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, NaniteTriangleThreshold))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, RenderContext))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, MaterialPurpose))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, RootMotionHandling))
	{
		const EMyUsdRootMotionHandling CorrectHandling = RootMotionHandling;
		RootMotionHandling = (EMyUsdRootMotionHandling) !((uint8)RootMotionHandling);
		SetRootMotionHandling(CorrectHandling);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, FallbackCollisionType))
	{
		const EMyUsdCollisionType CorrectCollisionType = FallbackCollisionType;
		FallbackCollisionType = (EMyUsdCollisionType) !((uint8)FallbackCollisionType);
		SetFallbackCollisionType(CorrectCollisionType);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, SubdivisionLevel))
	{
		int32 CorrectSubdivisionLevel = SubdivisionLevel;
		SubdivisionLevel = !SubdivisionLevel;
		SetSubdivisionLevel(CorrectSubdivisionLevel);
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(FMyUsdMetadataImportOptions, bCollectMetadata))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(FMyUsdMetadataImportOptions, bCollectFromEntireSubtrees))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(FMyUsdMetadataImportOptions, bCollectOnComponents))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(FMyUsdMetadataImportOptions, BlockedPrefixFilters))
	{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(FMyUsdMetadataImportOptions, bInvertFilters))
	{
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, UsdAssetCache))
	{
		UMyUsdAssetCache2* CorrectCache = UsdAssetCache.Get();
		UsdAssetCache = UsdAssetCache ? nullptr : NewObject<UMyUsdAssetCache2>();
		SetAssetCache(CorrectCache);
	}
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, AssetCache))
	{
		UMyUsdAssetCache3* CorrectCache = AssetCache.Get();
		AssetCache = AssetCache ? nullptr : NewObject<UMyUsdAssetCache3>();

		SetUsdAssetCache(CorrectCache);
	}
}

bool AMyUsdStageActor::HasAuthorityOverStage() const
{
#if WITH_EDITOR
...
void AMyUsdStageActor::OnSkelAnimationBaked(const FString& SkeletonPrimPath)
{
#if USE_USD_SDK
	const UE::FMyUsdStage& CurrentStage = static_cast<const AMyUsdStageActor*>(this)->GetUsdStage();
	...
	UE::FMyUsdPrim SkeletonPrim = CurrentStage.GetPrimAtPath(UE::FSdfPath{*SkeletonPrimPath});
	...
	UMyUsdPrimTwin* Twin = RootTwin->Find(SkeletonPrimPath);
	if (!Twin)
	{
		return;
	}
	...
	TSharedRef<FMyUsdSchemaTranslationContext> TranslationContext = FMyUsdStageActorImpl::CreateUsdSchemaTranslationContext(this, SkeletonPrimPath);
	...
	if (TSharedPtr<FMyUsdSchemaTranslator> SchemaTranslator = FMyUsdSchemaTranslatorRegistry::Get()
																.CreateTranslatorForSchema(TranslationContext, UE::FMyUsdTyped(SkeletonPrim)))
	{
		if (TSharedPtr<FMyUsdSkelSkeletonTranslator> SkelRootTranslator = StaticCastSharedPtr<FMyUsdSkelSkeletonTranslator>(SchemaTranslator))
		{
...
bool AMyUsdStageActor::UnloadAssets(const UE::FSdfPath& StartPrimPath, bool bForEntireSubtree)
{
...
bool AMyUsdStageActor::LoadAsset(FMyUsdSchemaTranslationContext& TranslationContext, const UE::FMyUsdPrim& Prim)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::LoadAsset);
	...
	PrimLinkCache->Modify();
	{
		...
		// c.f. the big comment on the analogous position within AMyUsdStageActor::LoadAssets
		TGuardValue<ITransaction*> SuppressTransaction{GUndo, nullptr};

		if (TSharedPtr<FMyUsdSchemaTranslator> SchemaTranslator = FMyUsdSchemaTranslatorRegistry::Get().CreateTranslatorForSchema(	  //
				TranslationContext.AsShared(),
				UE::FMyUsdTyped(Prim)
			))
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::CreateAssetsForPrim);
...
bool AMyUsdStageActor::LoadAssets(FMyUsdSchemaTranslationContext& TranslationContext, const UE::FMyUsdPrim& StartPrim)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::LoadAssets);
	...
	auto CreateAssetsForPrims = [&TranslationContext](const TArray<UE::FMyUsdPrim>& AllPrimAssets, FSlowTask& Progress)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::CreateAssetsForPrims);

		for (const UE::FMyUsdPrim& UsdPrim : AllPrimAssets)
		{
			Progress.EnterProgressFrame(1.f);

			if (TSharedPtr<FMyUsdSchemaTranslator> SchemaTranslator = FMyUsdSchemaTranslatorRegistry::Get().CreateTranslatorForSchema(
					TranslationContext.AsShared(),
					UE::FMyUsdTyped(UsdPrim)
				))
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::CreateAssetsForPrim);
				SchemaTranslator->CreateAssets();
	...
	auto PruneChildren = [&TranslationContext](const UE::FMyUsdPrim& UsdPrim) -> bool
	{
		if (TSharedPtr<FMyUsdSchemaTranslator> SchemaTranslator = FMyUsdSchemaTranslatorRegistry::Get()
																	.CreateTranslatorForSchema(TranslationContext.AsShared(), UE::FMyUsdTyped(UsdPrim)))
		{
	...
	PrimLinkCache->Modify();
	{
		...
		TArray<UE::FMyUsdPrim> AllPrimAssets = UsdUtils::GetAllPrimsOfType(StartPrim, TEXT("UsdShadeMaterial"));
		{
...
void AMyUsdStageActor::AnimatePrims()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(AMyUsdStageActor::AnimatePrims);
	...
	for (const FString& PrimToAnimate : PrimsToAnimate)
	{
		...
		if (TSharedPtr<FMyUsdSchemaTranslator> SchemaTranslator = FMyUsdSchemaTranslatorRegistry::Get().CreateTranslatorForSchema(
				TranslationContext,
				UE::FMyUsdTyped(CurrentStage.GetPrimAtPath(PrimPath))
			))
		{
			if (UMyUsdPrimTwin* UsdPrimTwin = GetRootPrimTwin()->Find(PrimToAnimate))
			{
...
FScopedBlockNoticeListening::FScopedBlockNoticeListening(AMyUsdStageActor* InStageActor)
{
...
{
	if (AMyUsdStageActor* StageActorPtr = StageActor.Get())
	{
