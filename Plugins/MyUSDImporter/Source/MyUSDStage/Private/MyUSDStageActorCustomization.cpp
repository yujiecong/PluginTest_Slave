...
FMyUsdStageActorCustomization::FMyUsdStageActorCustomization()
{
}

TSharedRef<IDetailCustomization> FMyUsdStageActorCustomization::MakeInstance()
{
	return MakeShared<FMyUsdStageActorCustomization>();
}

void FMyUsdStageActorCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayoutBuilder)
{
	...
	CurrentActor = Cast<AMyUsdStageActor>(SelectedObject.Get());
	if (!CurrentActor)
	{
		return;
	}

	CurrentActor->OnStageChanged.AddSP(this, &FMyUsdStageActorCustomization::ForceRefreshDetails);
	...
	// clang-format off
	TWeakObjectPtr<AMyUsdStageActor> WeakActor = CurrentActor;
	...
	[
		...
		[
			...
			{
				if(AMyUsdStageActor* StageActor = WeakActor.Get())
				{
					AMyUsdStageActor::OnOpenStageEditorClicked.Broadcast(StageActor);
				}
	...
	if (TSharedPtr<IPropertyHandle> RenderContextProperty = DetailLayoutBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, RenderContext)))
	{
		...
		[
			...
			.OnSelectionChanged(this, &FMyUsdStageActorCustomization::OnComboBoxSelectionChanged)
			...
			[
				SNew(STextBlock)
				.Text(this, &FMyUsdStageActorCustomization::GetComboBoxSelectedOptionText)
				.Font(FAppStyle::GetFontStyle(TEXT("PropertyWindow.NormalFont")))
			]
		];
	}

	if (TSharedPtr<IPropertyHandle> MaterialPurposeProperty = DetailLayoutBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(AMyUsdStageActor, MaterialPurpose)))
	{
...
void FMyUsdStageActorCustomization::CustomizeDetails(const TSharedPtr<IDetailLayoutBuilder>& DetailBuilder)
{
	DetailBuilderWeakPtr = DetailBuilder;
	CustomizeDetails(*DetailBuilder);
}

void FMyUsdStageActorCustomization::OnComboBoxSelectionChanged(TSharedPtr<FString> NewContext, ESelectInfo::Type SelectType)
{
...
FText FMyUsdStageActorCustomization::GetComboBoxSelectedOptionText() const
{
...
void FMyUsdStageActorCustomization::ForceRefreshDetails()
{
