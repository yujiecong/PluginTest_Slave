...
namespace UsdUtils
{
	...
	{
	public:
		FMyUsdTransactorImpl();
		~FMyUsdTransactorImpl();

		void Update(FTransactorRecordedEdits&& NewEdits);
		void Serialize(FArchive& Ar);
		void PreEditUndo(AMyUsdStageActor* StageActor);
		void PostEditUndo(AMyUsdStageActor* StageActor);
	...
	private:
		...
		// This is because when we receive a ConcertSync transaction the UMyUsdTransactor's values will be overwritten with the received data.
	...
	FMyUsdTransactorImpl::FMyUsdTransactorImpl()
	{
#if WITH_EDITOR
		if (GEditor)
		{
			...
			{
				Transactor->OnTransactionStateChanged().AddRaw(this, &FMyUsdTransactorImpl::HandleTransactionStateChanged);
				Transactor->OnBeforeRedoUndo().AddRaw(this, &FMyUsdTransactorImpl::HandleBeforeOnRedoUndo);
				Transactor->OnRedo().AddRaw(this, &FMyUsdTransactorImpl::HandleOnRedo);
			}
		}
#endif	  // WITH_EDITOR
	}
	...
	void FMyUsdTransactorImpl::Update(FTransactorRecordedEdits&& NewEdits)
	{
		if (!GUndo)
		{
			return;
	...
	void FMyUsdTransactorImpl::Serialize(FArchive& Ar)
	{
		Ar << Values;
	...
	void FMyUsdTransactorImpl::PreEditUndo(AMyUsdStageActor* StageActor)
	{
		if (!StageActor)
		{
			return;
	...
	void FMyUsdTransactorImpl::PostEditUndo(AMyUsdStageActor* StageActor)
	{
		...
		{
			// If we're just redoing it's a bit of a waste to let the AMyUsdStageActor respond to notices from the fields that we'll set,
	...
	bool FMyUsdTransactorImpl::IsTransactionUndoing()
	{
#if WITH_EDITOR
		if (UTransBuffer* Transactor = Cast<UTransBuffer>(GEditor->Trans))
		{
	...
	bool FMyUsdTransactorImpl::IsTransactionRedoing()
	{
#if WITH_EDITOR
		if (UTransBuffer* Transactor = Cast<UTransBuffer>(GEditor->Trans))
		{
	...
	void FMyUsdTransactorImpl::HandleTransactionStateChanged(
	...
	void FMyUsdTransactorImpl::HandleBeforeOnRedoUndo(const FTransactionContext& TransactionContext)
	{
	...
	void FMyUsdTransactorImpl::HandleOnRedo(const FTransactionContext& TransactionContext, bool bSucceeded)
	{
...
UMyUsdTransactor::UMyUsdTransactor(FVTableHelper& Helper)
	: Super(Helper)
{
}
UMyUsdTransactor::UMyUsdTransactor()
{
#if USE_USD_SDK
	Impl = MakeUnique<UsdUtils::FMyUsdTransactorImpl>();
#endif
}
UMyUsdTransactor::~UMyUsdTransactor() = default;

void UMyUsdTransactor::Initialize(AMyUsdStageActor* InStageActor)
{
	StageActor = InStageActor;
}

void UMyUsdTransactor::Update(const UsdUtils::FObjectChangesByPath& NewInfoChanges, const UsdUtils::FObjectChangesByPath& NewResyncChanges)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UMyUsdTransactor::Update);
...
void UMyUsdTransactor::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
void UMyUsdTransactor::PreEditUndo()
{
	if (Impl.IsValid())
	{
...
void UMyUsdTransactor::PostEditUndo()
{
	if (Impl.IsValid())
	{
