...
UENUM()
enum class EMyUsdStageState : uint8
{
	Closed,
	Opened,
	OpenedAndLoaded
...
UCLASS(MinimalAPI, config = MyUSDImporter)
class AMyUsdStageActor : public AActor
{
	GENERATED_BODY()
...
public:
	...
	TObjectPtr<UMyUsdInfoCache> UsdInfoCache;
...
public:
	...
	MYUSDSTAGE_API static FOnActorLoaded OnActorLoaded;

	DECLARE_EVENT(AMyUsdStageActor, FOnStageActorEvent);
	...
	DECLARE_EVENT_TwoParams(AMyUsdStageActor, FOnPrimChanged, const FString&, bool);
	FOnPrimChanged OnPrimChanged;

	DECLARE_MULTICAST_DELEGATE(FOnUsdStageTimeChanged);
	FOnUsdStageTimeChanged OnTimeChanged;

	DECLARE_EVENT_OneParam(AMyUsdStageActor, FOnOpenStageEditorClicked, AMyUsdStageActor*);
	MYUSDSTAGE_API static FOnOpenStageEditorClicked OnOpenStageEditorClicked;

public:
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetRootLayer(const FString& RootFilePath);
	...
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetUsePrimKindsForCollapsing(bool bUse);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetKindsToCollapse(int32 NewKindsToCollapse);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetMergeIdenticalMaterialSlots(bool bMerge);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetShareAssetsForIdenticalPrims(bool bShare);

	UE_DEPRECATED(5.5, "Use 'SetShareAssetsForIdenticalPrims' instead.")
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetReuseIdenticalAssets(bool bReuse);

	UE_DEPRECATED(5.2, "This option is now controlled via the cvar 'USD.CollapseTopLevelPointInstancers'")
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetCollapseTopLevelPointInstancers(bool bCollapse);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetPurposesToLoad(int32 NewPurposesToLoad);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetNaniteTriangleThreshold(int32 NewNaniteTriangleThreshold);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetRenderContext(const FName& NewRenderContext);
	...
	MYUSDSTAGE_API void SetCollectMetadata(bool bNewCollectValue);

	UFUNCTION(BlueprintCallable, Category = "USD|Metadata", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetCollectFromEntireSubtrees(bool bNewCollectValue);

	UFUNCTION(BlueprintCallable, Category = "USD|Metadata", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetCollectOnComponents(bool bNewCollectValue);

	UFUNCTION(BlueprintCallable, Category = "USD|Metadata", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetBlockedPrefixFilters(const TArray<FString>& NewFilters);

	UFUNCTION(BlueprintCallable, Category = "USD|Metadata", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetInvertFilters(bool bNewInvertValue);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API float GetTime() const;

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetTime(float InTime);

	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API ULevelSequence* GetLevelSequence() const;

	/**
	 * Gets the transient component that was generated for a prim with a given prim path.
	 * Warning: The lifetime of the component is managed by the AMyUsdStageActor, and it may be force-destroyed at any time (e.g. when closing the
	 * stage)
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API USceneComponent* GetGeneratedComponent(const FString& PrimPath) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API TArray<UObject*> GetGeneratedAssets(const FString& PrimPath) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API FString GetSourcePrimPath(const UObject* Object) const;

	// Creates a brand new, memory-only USD stage and opens it
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void NewStage();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API void SetIsolatedRootLayer(const FString& IsolatedStageRootLayer);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "USD", meta = (CallInEditor = "true"))
	MYUSDSTAGE_API FString GetIsolatedRootLayer() const;
...
public:
	 ...
	 */
	MYUSDSTAGE_API void IsolateLayer(const UE::FSdfLayer& Layer, bool bLoadUsdStage = true);

	UE_DEPRECATED(5.5, "This function has been renamed into 'RegenerateLevelSequence', which better describes what it does")
	MYUSDSTAGE_API void ReloadAnimations();
	...
	MYUSDSTAGE_API bool RegenerateLevelSequence();
	 ...
	 */
	MYUSDSTAGE_API void RepopulateLevelSequence();
	...
	MYUSDSTAGE_API void ResumeListeningToUsdNotices();
	MYUSDSTAGE_API bool IsListeningToUsdNotices() const;

	/** Prevents writing back data to the USD stage whenever our LevelSequences are modified */
	MYUSDSTAGE_API void StopMonitoringLevelSequence();
	MYUSDSTAGE_API void ResumeMonitoringLevelSequence();
	MYUSDSTAGE_API void BlockMonitoringLevelSequenceForThisTransaction();

	// Begin UObject interface
	MYUSDSTAGE_API virtual void PostDuplicate(bool bDuplicateForPIE) override;
	MYUSDSTAGE_API virtual void Serialize(FArchive& Ar) override;
	MYUSDSTAGE_API virtual void Destroyed() override;
	MYUSDSTAGE_API virtual void PostActorCreated() override;
	MYUSDSTAGE_API virtual void PostRename(UObject* OldOuter, const FName OldName) override;
	MYUSDSTAGE_API virtual void BeginDestroy() override;
	MYUSDSTAGE_API virtual void PostInitProperties() override;
#if WITH_EDITOR
	MYUSDSTAGE_API virtual void PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent) override;
	MYUSDSTAGE_API virtual void PostTransacted(const FTransactionObjectEvent& TransactionEvent) override;
	MYUSDSTAGE_API virtual void PreEditChange(FProperty* PropertyThatWillChange) override;
	MYUSDSTAGE_API virtual void PreEditUndo() override;
#endif	  // WITH_EDITOR
	// End UObject interface

	// Begin AActor interface
	MYUSDSTAGE_API virtual void Reset() override;
	MYUSDSTAGE_API virtual void PostRegisterAllComponents() override;
	MYUSDSTAGE_API virtual void UnregisterAllComponents(bool bForReregister = false) override;
	MYUSDSTAGE_API virtual void PostUnregisterAllComponents() override;
	// End AActor interface

	MYUSDSTAGE_API AMyUsdStageActor();
...
protected:
	...
	UMyUsdPrimTwin* GetRootPrimTwin();
	UMyUsdPrimTwin* GetRootPrimTwin() const;
	...
	MYUSDSTAGE_API void RequestDelayedTransactorReset();
	...
	void OnUsdPrimTwinDestroyed(const UMyUsdPrimTwin& UsdPrimTwin);
...
protected:
	friend class SMyUsdStage;	   // So that it can call RequestDelayedTransactorReset
	friend struct FMyUsdStageActorImpl;
	friend class FMyUsdLevelSequenceHelperImpl;
	friend class UsdUtils::FMyUsdTransactorImpl;
	 ...
	 */
	UPROPERTY(Transient, Instanced)
	TObjectPtr<UMyUsdPrimTwin> RootUsdTwin;

	UPROPERTY(Transient, Instanced)
	TObjectPtr<UMyUsdTransactor> Transactor;
...
{
public:
	UE_API explicit FScopedBlockNoticeListening(AMyUsdStageActor* InStageActor);
