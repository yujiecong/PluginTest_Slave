...
namespace UsdGeomMeshTranslatorImpl
{
	...
	)
	{
		...
		// (c.f. FMyUsdGeomXformableTranslator::UpdateComponents), and will instead expect it to be baked into the mesh.
		// So here we do that
		bool bSuccess = true;
		FTransform MeshTransform = FTransform::Identity;
...
void FMyUsdGeomMeshTranslator::CreateAssets()
{
...
USceneComponent* FMyUsdGeomMeshTranslator::CreateComponents()
{
...
void FMyUsdGeomMeshTranslator::UpdateComponents(USceneComponent* SceneComponent)
{
	...
	if (IsMeshPrim())
	{
		...
		if (UsdUtils::IsAnimatedMesh(GetPrim()))
		{
			...
			// reassign our new static meshes when we get to FMyUsdGeomXformableTranslator::UpdateComponents
			CreateAssets();
...
bool FMyUsdGeomMeshTranslator::CollapsesChildren(ECollapsingType CollapsingType) const
{
...
bool FMyUsdGeomMeshTranslator::CanBeCollapsed(ECollapsingType CollapsingType) const
{
	if (!IsMeshPrim())
	{
...
TSet<UE::FSdfPath> FMyUsdGeomMeshTranslator::CollectAuxiliaryPrims() const
{
	if (!IsMeshPrim())
	{
...
bool FMyUsdGeomMeshTranslator::ShouldSkipSkinnablePrim(bool bCheckForComponent) const
{
