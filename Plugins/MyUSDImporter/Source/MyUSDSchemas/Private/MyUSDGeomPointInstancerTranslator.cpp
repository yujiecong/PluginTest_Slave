...
void FMyUsdGeomPointInstancerCreateAssetsTaskChain::SetupTasks()
{
...
void FMyUsdGeomPointInstancerTranslator::CreateAssets()
{
	...
	// If another FMyUsdGeomXformableTranslator is collapsing the point instancer prim, it will do so by calling
	...
	if (GCollapseTopLevelPointInstancers)
	{
		...
		Context->TranslatorTasks.Add(MakeShared<FMyUsdGeomPointInstancerCreateAssetsTaskChain>(
			Context,
			PrimPath,
...
USceneComponent* FMyUsdGeomPointInstancerTranslator::CreateComponents()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMyUsdGeomPointInstancerTranslator::CreateComponents);

	// If we're collapsing ourselves, we're really just a collapsed Xform prim, so let that translator handle it
	if (GCollapseTopLevelPointInstancers)
	{
		return FMyUsdGeomXformableTranslator::CreateComponents();
	}
...
void FMyUsdGeomPointInstancerTranslator::UpdateComponents(USceneComponent* PointInstancerRootComponent)
{
...
bool FMyUsdGeomPointInstancerTranslator::CollapsesChildren(ECollapsingType CollapsingType) const
{
	return true;
}

bool FMyUsdGeomPointInstancerTranslator::CanBeCollapsed(ECollapsingType CollapsingType) const
{
	return GCollapseTopLevelPointInstancers;
}

TSet<UE::FSdfPath> FMyUsdGeomPointInstancerTranslator::CollectAuxiliaryPrims() const
{
