...
namespace UsdGeometryCacheTranslatorImpl
{
	...
	)
	{
		...
		// Use new feedback context to override the one coming from the MyUSDImporter to be able to show the ScopedSlowTask below
...
void FMyUsdGeometryCacheTranslator::CreateAssets()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMyUsdGeometryCacheTranslator::CreateAssets);
...
USceneComponent* FMyUsdGeometryCacheTranslator::CreateComponents()
{
	if (!IsPotentialGeometryCacheRoot())
	{
...
void FMyUsdGeometryCacheTranslator::UpdateComponents(USceneComponent* SceneComponent)
{
	...
	if (GeometryCacheComponent)
	{
		...
		// Don't go through FMyUsdGeomMeshTranslator::UpdateComponents as it will want to create a static mesh if PrimPath is an animated mesh prim
		// (which is likely, given that we're running this FMyUsdGeometryCacheTranslator for it)
		if (!bIsImporting)
		{
			FMyUsdGeomXformableTranslator::UpdateComponents(GeometryCacheComponent);
		}
...
bool FMyUsdGeometryCacheTranslator::CollapsesChildren(ECollapsingType CollapsingType) const
{
...
bool FMyUsdGeometryCacheTranslator::CanBeCollapsed(ECollapsingType CollapsingType) const
{
...
TSet<UE::FSdfPath> FMyUsdGeometryCacheTranslator::CollectAuxiliaryPrims() const
{
	if (!IsPotentialGeometryCacheRoot())
	{
