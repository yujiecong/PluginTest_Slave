...
#include "MyUSDGroomConversion.h"
...
protected:
	void SetupTasks()
	{
		...
		Then(
			...
			[this]()
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FMyUsdGroomCreateAssetsTaskChain::Build);
...
bool FMyUsdGroomTranslator::IsGroomPrim() const
{
	return UsdUtils::PrimHasSchema(GetPrim(), UnrealIdentifiers::GroomAPI);
}

void FMyUsdGroomTranslator::CreateAssets()
{
	...
	Context->TranslatorTasks.Add(MakeShared<FMyUsdGroomCreateAssetsTaskChain>(Context, PrimPath));
}

USceneComponent* FMyUsdGroomTranslator::CreateComponents()
{
	if (!Context->bAllowParsingGroomAssets || !IsGroomPrim())
	{
...
void FMyUsdGroomTranslator::UpdateComponents(USceneComponent* SceneComponent)
{
	if (Context->bAllowParsingGroomAssets && IsGroomPrim())
	{
...
bool FMyUsdGroomTranslator::CollapsesChildren(ECollapsingType CollapsingType) const
{
	if (!Context->bAllowParsingGroomAssets || !IsGroomPrim())
	{
...
bool FMyUsdGroomTranslator::CanBeCollapsed(ECollapsingType CollapsingType) const
{
	if (!Context->bAllowParsingGroomAssets || !IsGroomPrim())
	{
...
TSet<UE::FSdfPath> FMyUsdGroomTranslator::CollectAuxiliaryPrims() const
{
	if (!Context->bAllowParsingGroomAssets || !IsGroomPrim())
	{
