...
namespace UE::USDMaterialXTranslator::Private
{
	...
	)
	{
		...
		FString TempPackagePath = FString::Printf(TEXT("/Engine/MyUSDImporter/Transient/%d"), PackageSuffix++);
		...
		// Make sure that content folder doesn't have any packages already in it
		{
			...
			{
				TempPackagePath = FString::Printf(TEXT("/Engine/MyUSDImporter/Transient/%d"), PackageSuffix++);
			}
		}
...
void FMaterialXUsdShadeMaterialTranslator::CreateAssets()
{
	...
	// prims within our info cache. This way, the rest or our MyUSDImporter plugin doesn't need to care or know where
