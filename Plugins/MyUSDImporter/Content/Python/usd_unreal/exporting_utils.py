...
def add_relative_reference(prim: Usd.Prim, path: str):
    edit_layer = prim.GetStage().GetEditTarget().GetLayer()
    prim.GetReferences().AddReference(try_making_path_relative_to_layer(edit_layer, path))


def add_relative_payload(prim: Usd.Prim, path: str):
    edit_layer = prim.GetStage().GetEditTarget().GetLayer()
    prim.GetPayloads().AddPayload(try_making_path_relative_to_layer(edit_layer, path))


def try_making_path_relative_to_layer(layer: Sdf.Layer, path: str):
    layer_path = layer.realPath
    relative_path = unreal.UsdConversionLibrary.make_path_relative_to_layer(layer_path, path)

    if Path(Path(layer_path).parent / relative_path).exists():
...
def get_schema_name_for_component(component):
    ...
    if isinstance(component, unreal.SkinnedMeshComponent):
        return 'SkelRoot'
    elif isinstance(component, unreal.InstancedStaticMeshComponent):
...
def should_export_actor(actor):
    ...
    pcg_enabled = hasattr(unreal, "PCGVolume")

    for unreal_class in actor_classes_to_ignore:
        # PCGVolumes are Brushes but we allow those as they are the attach parents of all PCG outputs, which we do want to export
        if isinstance(actor, unreal_class) and (not pcg_enabled or not isinstance(actor, unreal.PCGVolume)):
            return False
...
def get_unique_export_file_path(context, asset, extension_with_dot=None):
    extension = extension_with_dot if extension_with_dot is not None else context.scene_file_extension
    root_folder = context.options.inner.asset_folder.path

    if context.options.inner.export_to_nested_sub_folders:
        return get_filename_to_export_to(root_folder, asset, extension)
    else:
        return unreal.UsdConversionLibrary.get_unique_file_path_for_export(os.path.join(root_folder, asset.get_name() + extension))
...
def create_a_sublayer_for_each_level(context, levels):
    ...
    for level in levels:
        ...
        sublayer_path = unreal.UsdConversionLibrary.get_unique_file_path_for_export(sublayer_path)
        ...
        root_prim = sublayer_stage.DefinePrim('/' + context.options.inner.root_prim_name, 'Xform')
...
class UsdUniquePathScope:
    """ RAII-style wrapper around Start/EndUniquePathScope functions to ensure we always close the scope.
    ...
    def __enter__(self):
        unreal.UsdConversionLibrary.begin_unique_path_scope()

    def __exit__(self, type, value, traceback):
        unreal.UsdConversionLibrary.end_unique_path_scope()
