...
from pxr import Usd, UsdUtils, UsdGeom, Sdf, UsdLux, Gf, UsdSkel, UsdShade, Tf, UsdMedia
import os
import re
import sys
import shutil
...
def collect_geometry_caches(actors):
    """ Collects all geometry caches that are used by unreal.GeometryCacheComponents of actors

    :param actors: Iterable of unreal.Actor
    :returns: Set of unreal.GeometryCache objects
    """
    geocaches = set()

    for actor in actors:
        for comp in actor.get_components_by_class(unreal.GeometryCacheComponent.static_class()):
            # Really we want to check bIsVisualizationComponent, so that we can skip exporting sprites
            # and camera meshes and directional light arrows and so on, but that's not exposed to blueprint
            # so this is the best we can do
            if comp.is_editor_only:
            ...
            geocache = comp.get_editor_property('geometry_cache')
            if geocache:
...
def assign_ism_component_assets(component, prim, context, material_overrides_layer, converter):
    """ Assigns the reference, exported static mesh asset for this instanced static mesh component

    :param component: unreal.InstancedStaticMeshComponent to export
    ...
    proxy_prim = stage.DefinePrim(prototypes.GetPath().AppendPath(proxy_prim_name))
...
def assign_geometry_cache_component_assets(component, prim, exported_assets):
    """ Assigns the reference, exported geometry cache asset for this geometry cache component

    :param component: unreal.GeometryCacheComponent to export
    :param prim: Usd.Prim that will reference the Mesh prim asset usd file
    :param exported_assets: Maps from unreal.Object to exported filename (e.g. "C:/MyFolder/Mesh.usda")
                            Used to retrieve the exported filename of the unreal.GeometryCache used by the component, if any
    :returns: None
    """
    geocache = component.get_editor_property('geometry_cache')
    if not geocache:
        return

    asset_file_path = exported_assets[geocache]
    if asset_file_path is None:
        unreal.log_warning(f"Failed to find an exported usd file to use for geometry cache asset '{geocache.get_name()}'")
        return
...
def assign_instanced_foliage_actor_assets(actor, prim, context, material_overrides_layer, converter):
    ...
    for foliage_type in types:
        ...
        if isinstance(source, unreal.StaticMesh) and source in context.exported_assets:
            ...
            exported_type_sources.append(None)
    ...
    for foliage_type, source in zip(exported_types, exported_type_sources):
        proxy_prim_name = str(foliage_type.get_name())
        proxy_prim_name = Tf.MakeValidIdentifier(proxy_prim_name)
        proxy_prim_name = get_unique_name(unique_child_prims, proxy_prim_name)
        unique_child_prims.add(proxy_prim_name)

        proxy_prim_path = prototypes.GetPath().AppendPath(proxy_prim_name)
        proxy_prim = stage.DefinePrim(proxy_prim_path)

        # Sometimes we fail to export a mesh for one of the foliage. We still need to emit a prototype
        # prim for it though, because our protoIndices already match the original foliage actor's FoliageTypes order
        if source:
...
def convert_component(context, converter, component, visited_components=None, parent_prim_path=""):
    ...
    prim_path = unreal.UsdConversionLibrary.get_prim_path_for_object(
        ...
        context.options.inner.root_prim_name
    )
    ...
    if isinstance(component, unreal.InstancedStaticMeshComponent):
        ...
        child_prim_path = get_unique_name(context.exported_prim_paths, prim_path + "/ISMInstance")
        ...
        prim_for_metadata = child_prim

        assign_ism_component_assets(component, child_prim, context, material_overrides_layer, converter)
        converter.convert_ism_component(component, child_prim_path)
        ...
        try:
            ...
            # We append 'Skel' because our prim here is a SkelRoot, but ConvertMaterialOverrides now expects
            # a Skeleton for the skeletal case. We always export skeletons named "Skel" though, so we can just
            # use that
            converter.convert_material_overrides(
                component.get_skinned_asset(),
                component.get_editor_property('override_materials'),
                str(Sdf.Path(prim_path).AppendChild("Skel")),
                context.options.inner.asset_options.lowest_mesh_lod,
                context.options.inner.asset_options.highest_mesh_lod
            )
        finally:
    ...
    elif isinstance(component, unreal.GeometryCacheComponent):
        assign_geometry_cache_component_assets(component, prim, context.exported_assets)
        ...
        try:
            converter.set_edit_target(unreal.FilePath(material_overrides_layer.realPath))
            converter.convert_material_overrides(
                component.get_editor_property('geometry_cache'),
                component.get_editor_property('override_materials'),
                str(prim_path),
                lowest_lod=0,
                highest_lod=0
    ...
    elif isinstance(component, unreal.UsdDrawModeComponent):
        converter.convert_draw_mode_component(component, prim_path)

    elif isinstance(component, unreal.AudioComponent):
        converter.convert_audio_component(component, prim_path)

    # Author metadata if we have any for this component
    if context.options.inner.metadata_options.export_component_metadata:
        asset_user_data = unreal.UsdConversionLibrary.get_usd_asset_user_data(component)
        if asset_user_data:
            converter.set_prim_metadata_from_user_data(
                str(prim_for_metadata.GetPrimPath()),
                asset_user_data,
                blocked_prefix_filter=context.options.inner.metadata_options.blocked_prefix_filters,
                invert_filter=context.options.inner.metadata_options.invert_filters
            )
    ...
    if isinstance(component, unreal.SceneComponent):
        ...
        # Export other stage actors with simple references to the stages that they're loading, if any
            ...
            if opened_root_layer and opened_root_layer.file_path:
                ...
                prim.ClearTypeName()
                add_relative_payload(prim, file_path)
...
def export_level(context, actors):
    ...
    with unreal.ScopedSlowTask(3, f"Exporting main root layer") as slow_task:
        ...
        root_prim = context.stage.DefinePrim('/' + context.options.inner.root_prim_name, 'Xform')
        ...
        if context.options.inner.metadata_options.export_asset_info:
            with UsdConversionContext(context.root_layer_path) as converter:
        ...
        # Write metadata stored on the actual ULevels, if any
        if context.options.inner.metadata_options.export_asset_metadata:
            for level in unreal.EditorLevelUtils.get_levels(context.world):

                layer_path = ""
                sublayer = context.level_to_sublayer.get(level)
                if sublayer:
                    layer_path = str(sublayer.identifier)
                elif level and level.get_outer() == context.world:
                    layer_path = context.root_layer_path

                if layer_path:
                    with UsdConversionContext(layer_path) as converter:
                        asset_user_data = unreal.UsdConversionLibrary.get_usd_asset_user_data(level)
                        if asset_user_data:
                            # We always define root_prim_name as default prim on every layer we export
                            converter.set_prim_metadata_from_user_data(
                                str('/' + context.options.inner.root_prim_name),
                                asset_user_data,
                                blocked_prefix_filter=context.options.inner.metadata_options.blocked_prefix_filters,
                                invert_filter=context.options.inner.metadata_options.invert_filters
                            )
...
def export_material(context, material):
    ...
    """
    material_file = get_unique_export_file_path(context, material)
    ...
    options.metadata_options = context.options.inner.metadata_options
...
def export_mesh(context, mesh, mesh_type):
    ...
    :returns: (Bool, String) containing True if the export was successful, and the output filename that was used
    """
    mesh_file = get_unique_export_file_path(context, mesh)
    ...
    if mesh_type == 'static':
        ...
        options = unreal.GeometryCacheExporterUSDOptions()
...
def export_cubemap(context, cubemap):
    ...
    :param context: UsdExportContext object describing the export
    :param cubemap: unreal.TextureCube object to export
    :returns: (Bool, String) containing True if the export was successful, and the output filename that was used
    """
    if not cubemap:
        return

    hdr_file = get_unique_export_file_path(context, cubemap, extension_with_dot=".hdr")
    ...
    with unreal.ScopedSlowTask(num_cubemaps, 'Exporting/copying cubemaps') as slow_task:
        ...
        for cubemap in cubemaps:
            ...
            if os.path.exists(existing_filename) and os.path.isfile(existing_filename):
                target_filename = get_unique_export_file_path(context, cubemap, extension_with_dot=".hdr")
...
def export_landscape(context, landscape_actor):
    ...
    if context.options.inner.export_to_nested_sub_folders:
        ...
        folder_path = context.options.inner.asset_folder.path
        actor_name = landscape_actor.get_actor_label()
    ...
    if not context.options.inner.export_to_nested_sub_folders:
        asset_path = unreal.UsdConversionLibrary.get_unique_file_path_for_export(asset_path)
        mesh_path = unreal.UsdConversionLibrary.get_unique_file_path_for_export(mesh_path)
        mat_path = unreal.UsdConversionLibrary.get_unique_file_path_for_export(mat_path)
    ...
    with Usd.StageCacheContext(cache):
        ...
        if context.options.inner.asset_options.use_payload:
            ...
            num_lods = max(abs(context.options.inner.highest_landscape_lod - context.options.inner.lowest_landscape_lod + 1), 1)
            asset_prim = asset_stage.DefinePrim('/' + Tf.MakeValidIdentifier(actor_name))
            ...
            add_relative_payload(asset_prim, mesh_path)
            ...
            mat_proxy_prim = asset_stage.DefinePrim(mat_scope_prim.GetPath().AppendChild('Material'))
        ...
        # Just export mesh and material files
            ...
            mat_proxy_prim = mesh_stage.DefinePrim(mat_scope_prim.GetPath().AppendChild('Material'))
            add_relative_reference(mat_proxy_prim, mat_path)
...
def export(context):
    ...
    try:
        ...
        with UsdUniquePathScope():
            context.root_layer_path = unreal.UsdConversionLibrary.get_unique_file_path_for_export(context.root_layer_path)
            ...
            with unreal.ScopedSlowTask(7, f"Exporting level to '{context.root_layer_path}'") as slow_task:
                slow_task.make_dialog(True)
                ...
                with UsdScopedAnalyticsBlocker():
                    ...
                    slow_task.enter_progress_frame(1)
                    export_meshes(context, geometry_caches, 'animated')
                ...
                # Send analytics for the level export itself
                send_analytics(context, actors, static_meshes, skeletal_meshes, geometry_caches, materials, cubemaps)
